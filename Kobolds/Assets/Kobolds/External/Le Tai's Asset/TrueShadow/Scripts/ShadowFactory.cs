...
namespace LeTai.TrueShadow
{
	public class ShadowFactory
	{
		...
		private readonly Dictionary<int, ShadowContainer> shadowCache = new();
    ...
    public int ReleasedContainerCount
    {
		...
		internal void Get(ShadowSettingSnapshot snapshot, ref ShadowContainer container)
		{
			if (
				float.IsNaN(snapshot.dimensions.x) || snapshot.dimensions.x < 1
													...
													|| (snapshot.size == 0 &&
														snapshot.canvasRelativeOffset.sqrMagnitude == 0)
			)
			{
        ...
        if (snapshot.shadow.alwaysRender)
			...
			var requestHash = snapshot.GetHashCode();
		...
		private static readonly Rect UNIT_RECT = new(0, 0, 1, 1);
		private static readonly Vector4 ALPHA8_TEXTURE_BIAS = new(1, 1, 1, 0);
		...
		private ShadowContainer GenerateShadow(ShadowSettingSnapshot snapshot)
		{
			...
			var blurConfig = snapshot.algorithm == BlurAlgorithmSelection.Fast ? blurConfigFast : blurConfigAccurate;
			...
			var padding = snapshot.shadow.Inset ?
				System.Math.Max(
					Mathf.CeilToInt(
						Mathf.Max(
							Mathf.Abs(snapshot.canvasRelativeOffset.x),
			...
			var needProcessImprint = snapshot.shadow.IgnoreCasterColor || snapshot.shadow.Inset;
			...
			materialProps.Clear();
			materialProps.SetVector(
				ShaderId.CLIP_RECT, new Vector4(
					float.NegativeInfinity, float.NegativeInfinity,
					float.PositiveInfinity, float.PositiveInfinity));
			materialProps.SetInt(
				ShaderId.COLOR_MASK, (int) ColorWriteMask.All); // Render shadow even if mask hide graphic
			...
			materialProps.SetVector(
				ShaderId.TIME, new Vector4(
					baseTime / 20,
					baseTime,
					baseTime * 2,
					baseTime * 3));
			materialProps.SetVector(
				ShaderId.SIN_TIME, new Vector4(
					Mathf.Sin(baseTime / 8),
					Mathf.Sin(baseTime / 4),
					Mathf.Sin(baseTime / 2),
					Mathf.Sin(baseTime)));
			materialProps.SetVector(
				ShaderId.COS_TIME, new Vector4(
					Mathf.Cos(baseTime / 8),
					Mathf.Cos(baseTime / 4),
					Mathf.Cos(baseTime / 2),
					Mathf.Cos(baseTime)));
			materialProps.SetVector(
				ShaderId.UNITY_DELTA_TIME, new Vector4(
					Time.deltaTime,
					1 / Time.deltaTime,
...
#if TMP_PRESENT
			if (snapshot.shadow.Graphic is TextMeshProUGUI
				|| snapshot.shadow.Graphic is TMP_SubMeshUI)
			{
				...
				materialProps.SetVector(
					ShaderId.SCREEN_PARAMS, new Vector4(
						tw, th,
						1f + 1f / tw,
						1f + 1f / th));
			}
			...
			var blurSrc = needProcessImprint ? imprintTexProcessed : imprintTex;
			...
			var relativeOffset = new Vector2(
				snapshot.canvasRelativeOffset.x / tw,
			...
			if (needPostProcess)
			{
				...
				ShadowPostProcessMaterial.SetFloat(
					ShaderId.ALPHA_MULTIPLIER,
