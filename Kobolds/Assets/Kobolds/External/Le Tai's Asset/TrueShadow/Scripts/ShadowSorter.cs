...
namespace LeTai.TrueShadow
{
	...
	public class ShadowSorter : MonoBehaviour
	{
		...
		private readonly IndexedSet<TrueShadow> shadows = new();
		private readonly IndexedSet<SortGroup> sortGroups = new();
		...
		public static ShadowSorter Instance
		{
			get
			{
				if (!instance)
				{
					...
					for (var i = existings.Length - 1; i > 0; i--) Destroy(existings[i]);
		...
		public void Sort()
		{
			for (var i = 0; i < sortGroups.Count; i++)
			{
				...
				foreach (var entry in group.sortEntries) entry.shadow.ForgetSiblingIndexChanges();
			}
