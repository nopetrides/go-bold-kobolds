...
namespace LeTai.TrueShadow
{
	...
	{
		private static readonly Color FADED_COLOR = new(.5f, .5f, .5f, .5f);
		...
		[Header("Color")]
		public Color selectedColor = new(0, 0, 0, .25f);

		public Color hoverColor = new(0, 0, 0, .20f);
		public Color clickedColor = new(0, 0, 0, .25f);
		...
		private void Reset()
		{
			...
			if (shadow)
			{
				...
				var selectedIsNormal = Input.mousePresent || Input.touchSupported;
		...
		private void Update()
		{
			...
			if (!Approximately(targetDistance, shadow.OffsetDistance))
				shadow.OffsetDistance = SmoothDamp(
					shadow.OffsetDistance,
					targetDistance,
