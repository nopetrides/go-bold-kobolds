...
namespace LeTai.TrueShadow
{
	...
	{
		private static readonly Color DEFAULT_COLOR = new(0, 0, 0, .6f);

		[Tooltip(
			"Accurate algorithm doesn't miss small features, but can be much slower for large or dynamic shadows. " +
			"Fast is recommended in most cases")]
		[SerializeField]
		private BlurAlgorithmSelection algorithm = BlurAlgorithmSelection.Fast;
		...
		[SerializeField] private bool useGlobalAngle;
		...
		[SerializeField]
		private float offsetDistance = 12;
...
#pragma warning restore 0649
		...
		[SerializeField] private bool modifiedFromInspector;
		...
		public float Size
		{
			...
			set
			{
				...
				size = newSize;
				if (Inset && OffsetDistance > Size) OffsetDistance = Size;
			}
		}
		...
		/// <summary>
		///     The area where the alpha channel = 0 can be either 0, or the color of the edge of the texture, depend on how the
		///     texture was authored.
		...
		///     You can provide custom clear color by implementing
		///     <see cref="PluginInterfaces.ITrueShadowCasterClearColorProvider" />, and set this to Plugin
		/// </summary>
		/// <exception cref="ArgumentOutOfRangeException"></exception>
		public Color ClearColor => casterClearColorProvider?.GetTrueShadowCasterClearColor() ?? Color.clear;

		/// <summary>
		///     Can't be implemented due to
		///     <see
		///         href="https://issuetracker.unity3d.com/issues/prefab-instances-sibling-index-is-not-updated-when-a-lower-index-sibling-is-deleted">
		///         Unity's
		///         bug 1280465
		///     </see>
		///     . Do not use!
    ...
    public bool alwaysRender;
		...
		[SerializeField] private List<Sprite> bakedShadows;
...
#if TMP_PRESENT
					case TextMeshProUGUI tmp: return tmp.materialForRendering.mainTexture;
					case TMP_SubMeshUI stmp: return stmp.materialForRendering.mainTexture;
#endif
...
#if UNITY_EDITOR
			Undo.undoRedoPerformed += ApplySerializedData;
		...
		{
			var areCanvasRenderersCulled = CanvasRenderer && CanvasRenderer.cull &&
		...
		public void CopyToTMPSubMeshes()
		{
			...
			for (var i = 0; i < submeshes.Length; i++) CopyTo(submeshes[i].gameObject);
		}
#endif
	}
}
