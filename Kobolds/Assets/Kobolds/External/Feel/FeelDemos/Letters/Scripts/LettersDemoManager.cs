...
namespace MoreMountains.Feel
{
	...
	public class LettersDemoManager : MonoBehaviour
	{
		...
		{
			if (FeelDemosInputHelper.CheckAlphaInputPressedThisFrame(1)) PlayF();
			if (FeelDemosInputHelper.CheckAlphaInputPressedThisFrame(2)) PlayE1();
			if (FeelDemosInputHelper.CheckAlphaInputPressedThisFrame(3)) PlayE2();
			if (FeelDemosInputHelper.CheckAlphaInputPressedThisFrame(4)) PlayL();

			if (FeelDemosInputHelper.CheckMouseDown())
			{
				RaycastHit hit;
				var ray = Camera.main.ScreenPointToRay(FeelDemosInputHelper.MousePosition());
