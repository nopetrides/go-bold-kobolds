...
namespace Lofelt.NiceVibrations
{
	...
	public class MMProgressBar : MonoBehaviour
	{
		...
		public AnimationCurve BumpAnimationCurve = new(
			...
			if (ForegroundBar != null)
				switch (FillMode)
				{
					case FillModes.LocalScale:
						...
						if (LerpForegroundBar)
							_newScale = Vector3.Lerp(
								ForegroundBar.localScale, _targetLocalScale, currentDeltaTime * LerpForegroundBarSpeed);
						else
					...
					case FillModes.Width:
						...
						newSizeX = Mathf.Lerp(
							_foregroundImage.rectTransform.sizeDelta.x, newSizeX,
							currentDeltaTime * LerpForegroundBarSpeed);
						_foregroundImage.rectTransform.SetSizeWithCurrentAnchors(
							RectTransform.Axis.Horizontal, newSizeX);
						break;
					...
					case FillModes.Height:
						...
						newSizeY = Mathf.Lerp(
							_foregroundImage.rectTransform.sizeDelta.x, newSizeY,
							currentDeltaTime * LerpForegroundBarSpeed);
					...
					case FillModes.FillAmount:
						...
						if (LerpForegroundBar)
							_foregroundImage.fillAmount = Mathf.Lerp(
								_foregroundImage.fillAmount, _targetFill, currentDeltaTime * LerpForegroundBarSpeed);
						else
			...
			if (DelayedBar != null)
				if (currentTime - _lastUpdateTimestamp > Delay)
				{
					if (FillMode == FillModes.LocalScale)
					{
						...
						if (LerpDelayedBar)
							_newScale = Vector3.Lerp(
								DelayedBar.localScale, _targetLocalScale, currentDeltaTime * LerpDelayedBarSpeed);
						else
							_newScale = _targetLocalScale;
					...
					{
						if (LerpDelayedBar)
							_delayedImage.fillAmount = Mathf.Lerp(
								_delayedImage.fillAmount, _targetFill, currentDeltaTime * LerpDelayedBarSpeed);
						else
