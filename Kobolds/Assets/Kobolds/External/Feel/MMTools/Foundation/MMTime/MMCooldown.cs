	...
	public class MMCooldown
	{
		...
		public float Progress
		{
			get
			{
				...
				if (CooldownState == CooldownStates.Consuming || CooldownState == CooldownStates.Stopped) return 0f;

				if (CooldownState == CooldownStates.Refilling) return CurrentDurationLeft / RefillDuration;
		...
		{
			if (Ready()) ChangeState(CooldownStates.Consuming);
		}
		...
		{
			if (CooldownState == CooldownStates.Consuming) ChangeState(CooldownStates.Stopped);
		}
