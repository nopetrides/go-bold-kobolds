...
namespace MoreMountains.Tools
{
	...
	public class MMSquashAndStretch : MonoBehaviour
	{
		...
		protected virtual void ComputeVelocityAndDirection()
		{
			...
			if (AutoSquashOnStop)
			{
				...
				else if (_movementStarted)
				{
					...
					_squashing = true;
					var duration = MMMaths.Remap(
						_lastVelocity, 0f, MaximumVelocity, SquashDuration.x, SquashDuration.y);
					var intensity = MMMaths.Remap(
						_lastVelocity, 0f, MaximumVelocity, SquashIntensity.x, SquashIntensity.y);
		...
		{
			if (!RotateToMatchDirection) return;
		...
		protected virtual void ComputeNewLocalScale()
		{
			if (_squashing)
			{
				...
				var curveValue = SquashCurve.Evaluate(elapsed);
			...
			{
				MMMaths.Spring(
					ref _springScale, _newLocalScale, ref _springVelocity, SpringDamping, SpringFrequency,
					Time.deltaTime);
				_newLocalScale = _springScale;
			}

			if (!RescaleX) _newLocalScale.x = _initialScale.x;
			if (!RescaleY) _newLocalScale.y = _initialScale.y;
			if (!RescaleZ) _newLocalScale.z = _initialScale.z;
