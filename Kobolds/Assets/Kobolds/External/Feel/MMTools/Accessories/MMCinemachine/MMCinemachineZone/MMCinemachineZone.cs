...
namespace MoreMountains.Tools
{
	...
	public abstract class MMCinemachineZone : MonoBehaviour
	{
		...
		protected virtual void Initialization()
		{
			...
			if (VirtualCamera == null)
				Debug.LogWarning(
					"[MMCinemachineZone2D] " + name +
					" : no virtual camera is attached to this zone. Set one in its inspector.");
#endif

			if (SetupConfinerOnStart) SetupConfinerGameObject();
		...
		protected virtual void Start()
		{
			...
			if (SetupConfinerOnStart) SetupConfiner();
		...
		protected virtual IEnumerator EnableCamera(bool state, int frames)
		{
			...
			{
				var settings = VirtualCamera.Priority;
		...
		{
			if (!RoomVisited) OnEnterZoneForTheFirstTimeEvent.Invoke();
		...
		protected virtual void ExitZone()
		{
			...
			if (gameObject.activeInHierarchy) StartCoroutine(EnableCamera(false, 0));
			foreach (var go in ActivationList) go.SetActive(false);
		}
