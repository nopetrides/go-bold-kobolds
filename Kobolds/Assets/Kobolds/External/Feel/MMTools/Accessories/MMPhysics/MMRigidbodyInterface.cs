...
namespace MoreMountains.Tools
{
	...
	public class MMRigidbodyInterface : MonoBehaviour
	{
		...
		public Vector3 Velocity
		{
			get
			{
				if (_mode == "2D") return _rigidbody2D.linearVelocity;

				if (_mode == "3D") return _rigidbody.linearVelocity;

				return new Vector3(0, 0, 0);
			}
			set
			{
				if (_mode == "2D") _rigidbody2D.linearVelocity = value;
				if (_mode == "3D") _rigidbody.linearVelocity = value;
			}
		}
		...
		public bool isKinematic
		{
			get
			{
				if (_mode == "2D") return _rigidbody2D.bodyType == RigidbodyType2D.Kinematic;
		...
		{
			if (_mode == "2D") _rigidbody2D.bodyType = status ? RigidbodyType2D.Kinematic : RigidbodyType2D.Dynamic;
