...
namespace MoreMountains.Tools
{
	public class MMTilemapGridRenderer
	{
		...
		public static void RenderGrid(
			int[,] grid, MMTilemapGeneratorLayer layer, bool slowRender = false, float slowRenderDuration = 1f,
			MMTweenType slowRenderTweenType = null, MonoBehaviour slowRenderSupport = null)
		{
			...
			if (!slowRender || !Application.isPlaying)
				...
				slowRenderSupport.StartCoroutine(
					SlowRenderGrid(grid, layer.TargetTilemap, tile, slowRenderDuration, slowRenderTweenType, 60));
		...
		/// <returns></returns>
		public static IEnumerator SlowRenderGrid(
			int[,] grid, Tilemap tilemap, TileBase tile, float slowRenderDuration, MMTweenType slowRenderTweenType,
			int frameRate)
		{
		...
		/// <returns></returns>
		private static int DrawGrid(
			int[,] grid, Tilemap tilemap, TileBase tile, int startIndex, int numberOfTilesToDraw)
		{
			var width = grid.GetUpperBound(0);
			var height = grid.GetUpperBound(1);
