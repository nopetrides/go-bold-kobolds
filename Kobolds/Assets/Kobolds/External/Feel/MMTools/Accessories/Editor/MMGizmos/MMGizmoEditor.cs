	...
	public class MMGizmoEditor : Editor
	{
		...
		private static bool TestDistance(MMGizmo mmGizmo, float viewDistance)
		{
			...
			distanceToCamera = Vector3.Distance(
				mmGizmo.transform.position, SceneView.currentDrawingSceneView.camera.transform.position);
			return distanceToCamera < viewDistance;
		}
		...
		private static void Initialization(MMGizmo mmGizmo)
		{
			...
			mmGizmo._textGUIStyle.padding = new RectOffset(
				(int) mmGizmo.TextPadding.x, (int) mmGizmo.TextPadding.y, (int) mmGizmo.TextPadding.z,
				(int) mmGizmo.TextPadding.w);
		...
		private static void DrawColliderGizmo(MMGizmo mmGizmo)
		{
			if (mmGizmo._sphereColliderNotNull)
			{
				if (mmGizmo.ColliderRenderType == MMGizmo.ColliderRenderTypes.Full)
					Gizmos.DrawSphere(
						ComputeGizmoPosition(mmGizmo, mmGizmo._sphereCollider.center), mmGizmo._sphereCollider.radius);
				else
			...
			if (mmGizmo._boxColliderNotNull)
			{
				if (mmGizmo.ColliderRenderType == MMGizmo.ColliderRenderTypes.Full)
					Gizmos.DrawCube(
						ComputeGizmoPosition(mmGizmo, mmGizmo._boxCollider.center), mmGizmo._boxCollider.size);
				else
					Gizmos.DrawWireCube(
						ComputeGizmoPosition(mmGizmo, mmGizmo._boxCollider.center), mmGizmo._boxCollider.size);
			}
			...
			if (mmGizmo._circleCollider2DNotNull)
			{
				if (mmGizmo.ColliderRenderType == MMGizmo.ColliderRenderTypes.Full)
					Gizmos.DrawSphere(
						ComputeGizmoPosition(mmGizmo, mmGizmo._circleCollider2D.offset),
		...
		private static void DrawPositionGizmo(MMGizmo mmGizmo)
		{
			switch (mmGizmo.PositionMode)
			{
				case MMGizmo.PositionModes.Point:
					MMDebug.DrawGizmoPoint(
						ComputeGizmoPosition(mmGizmo, mmGizmo._vector3Zero), mmGizmo.GizmoColor, mmGizmo.PositionSize);
					break;
				case MMGizmo.PositionModes.Cube:
					Gizmos.DrawCube(
						ComputeGizmoPosition(mmGizmo, mmGizmo._vector3Zero), Vector3.one * mmGizmo.PositionSize);
					break;
				...
				case MMGizmo.PositionModes.Texture:
					...
					{
						Handles.BeginGUI();
						mmGizmo._worldToGUIPosition = HandleUtility.WorldToGUIPoint(
							ComputeGizmoPosition(mmGizmo, mmGizmo.transform.position, false));
						mmGizmo._textureRect = new Rect(
							mmGizmo._worldToGUIPosition.x - mmGizmo.TextureSize.x / 2f,
				...
				case MMGizmo.PositionModes.Arrows:
					...
					Handles.ArrowHandleCap(
						0, ComputeGizmoPosition(mmGizmo, mmGizmo.transform.position, false),
		...
		private static void DrawText(MMGizmo mmGizmo)
		{
			...
			if (mmGizmo._textToDisplay != "")
				Handles.Label(
					mmGizmo.transform.position + mmGizmo.TextOffset, mmGizmo._textToDisplay, mmGizmo._textGUIStyle);
		}
