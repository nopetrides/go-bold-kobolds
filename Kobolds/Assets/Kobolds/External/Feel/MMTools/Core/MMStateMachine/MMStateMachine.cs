...
namespace MoreMountains.Tools
{
	...
	public class MMStateMachine<T> : MMIStateMachine where T : struct, IComparable, IConvertible, IFormattable
	{
		...
		/// let's assume in some class we have a public MMStateMachine
		/// <CharacterStates.MovementStates>
		///     MovementState, and we use that to track the state of a moving character (idle, walking, running etc)
		///     in any other class, we could do :
		///     public class TestListener : MonoBehaviour, MMEventListener<MMStateChangeEvent
		///     <CharacterStates.MovementStates>
		///         >
		///         {
		///         // triggered every time a state change event occurs
		///         public void OnMMEvent(MMStateChangeEvent
		///         <CharacterStates.MovementStates>
		///             stateChangeEvent)
		///             {
		...
		///             {
		///             MMEventManager.AddListener<MMStateChangeEvent
		///             <CharacterStates.MovementStates>
		///                 >(this);
		///                 }
		///                 private void OnDisable() // on disable we stop listening for these events
		///                 {
		///                 MMEventManager.RemoveListener<MMStateChangeEvent
		///                 <CharacterStates.MovementStates>
		///                     >(this);
		///                     }
		///                     }
		///                     Now every time this character's movement state changes, the OnMMEvent method will be called, and
