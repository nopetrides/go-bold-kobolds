...
namespace MoreMountains.Tools
{
	public class MonoAttribute
	{
		...
		public MonoAttribute(
			MonoBehaviour targetObject, MemberTypes type, PropertyInfo propertyInfo, FieldInfo fieldInfo,
			string memberName)
		{
		...
		{
			if (MemberType == MemberTypes.Property) return (float) MemberPropertyInfo.GetValue(TargetObject);
	...
	public class FloatController : MMMonoBehaviour
	{
		...
		public AnimationCurve ToDestinationCurve = new(
			new Keyframe(0, 0), new Keyframe(0.5f, 0.6f), new Keyframe(1f, 1f));
		...
		protected virtual void Update()
		{
			...
			switch (ControlMode)
			{
				...
				case ControlModes.OneTime:
					if (!_shaking) return;
					_remappedTimeSinceStart = MMMaths.Remap(
						GetTime() - _shakeStartTimestamp, 0f, OneTimeDuration, 0f, 1f);
				...
				case ControlModes.ToDestination:
					if (!_shaking) return;
					_remappedTimeSinceStart = MMMaths.Remap(
						GetTime() - _shakeStartTimestamp, 0f, ToDestinationDuration, 0f, 1f);
		...
		public virtual bool FindAttribute(string propertyName)
		{
			...
			if (propInfo != null)
				TargetAttribute = new MonoAttribute(
					TargetObject, MonoAttribute.MemberTypes.Property, propInfo, null, propertyName);
			if (fieldInfo != null)
				TargetAttribute = new MonoAttribute(
					TargetObject, MonoAttribute.MemberTypes.Field, null, fieldInfo, propertyName);
			...
			{
				Debug.LogError(
					"FloatController " + name + " couldn't find any property or field named " + propertyName + " on " +
					TargetObject.name);
				return false;
			}
