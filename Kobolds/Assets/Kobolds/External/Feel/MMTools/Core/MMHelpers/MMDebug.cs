...
#region Commands
			...
			get
			{
				if (_commands == null)
					_commands = AppDomain.CurrentDomain.GetAssemblies()
						.SelectMany(m => m.GetTypes().SelectMany(n => n
		...
		public static void DebugLogCommand(string command)
		{
			...
			else
			{
				...
				{
					LogCommand(
						"A version of command " + commandFirst + " without arguments could not be found.", "#ff2a00");
					return;
				}
			}
...
#region DebugLog
		...
		public struct DebugLogItem
		{
			...
			public DebugLogItem(
				object message, string color, int framecount, float time, int timePrecision, bool displayFrameCount)
			{
				Message = message;
				Color = color;
		...
		public static string LogHistoryText
		{
			get
			{
				...
				for (var i = 0; i < LogHistory.Count; i++)
				{
					...
					log.Append(
						"<color=#f9a682>[" + MMTime.FloatToTimeString(LogHistory[i].Time, false, true, true, true) +
						"]</color> ");
		...
		public static void DebugLogInfo(
			object message, string color = "", int timePrecision = 3, bool displayFrameCount = true)
		{
		...
		public static void DebugLogTime(
			object message, string color = "", int timePrecision = 3, bool displayFrameCount = true)
		{
			...
			var item = LogDebugToConsole(message, color, timePrecision, displayFrameCount);
		}
		...
		/// <param name="displayFrameCount"></param>
		/// <returns></returns>
		public static DebugLogItem LogDebugToConsole(
...
#region Casts
		...
		public static RaycastHit2D RayCast(
			Vector2 rayOriginPoint, Vector2 rayDirection, float rayDistance, LayerMask mask, Color color,
		...
		public static RaycastHit2D BoxCast(
			Vector2 origin, Vector2 size, float angle, Vector2 direction, float length, LayerMask mask, Color color,
			bool drawGizmo = false)
		{
		...
		public static RaycastHit2D MonoRayCastNonAlloc(
			RaycastHit2D[] array, Vector2 rayOriginPoint, Vector2 rayDirection, float rayDistance, LayerMask mask,
		...
		public static RaycastHit Raycast3D(
			Vector3 rayOriginPoint, Vector3 rayDirection, float rayDistance, LayerMask mask, Color color,
			bool drawGizmo = false, QueryTriggerInteraction queryTriggerInteraction = QueryTriggerInteraction.UseGlobal)
		{
			if (drawGizmo && DebugDrawEnabled) Debug.DrawRay(rayOriginPoint, rayDirection * rayDistance, color);
			RaycastHit hit;
...
#region DebugDraw
		...
		/// <param name="color">Color.</param>
		public static void DrawGizmoArrow(
			Vector3 origin, Vector3 direction, Color color, float arrowHeadLength = 3f, float arrowHeadAngle = 25f)
		{
		...
		public static void DebugDrawArrow(
			Vector3 origin, Vector3 direction, Color color, float arrowHeadLength = 0.2f, float arrowHeadAngle = 35f)
		{
			if (!DebugDrawEnabled) return;
		...
		public static void DebugDrawArrow(
			Vector3 origin, Vector3 direction, Color color, float arrowLength, float arrowHeadLength = 0.20f,
		...
		private static void DrawArrowEnd(
			bool drawGizmos, Vector3 arrowEndPosition, Vector3 direction, Color color, float arrowHeadLength = 0.25f,
		...
		public static void DrawHandlesBounds(Bounds bounds, Color color)
		{
			...
			var v3FrontTopLeft = new Vector3(
				boundsCenter.x - boundsExtents.x, boundsCenter.y + boundsExtents.y,
