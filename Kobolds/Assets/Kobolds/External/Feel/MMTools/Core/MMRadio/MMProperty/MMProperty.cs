...
namespace MoreMountains.Tools
{
	public class MMProperty
	{
		...
		public MMProperty(
			Component targetComponent, MemberTypes type, PropertyInfo propertyInfo, FieldInfo fieldInfo,
		...
		public static MMProperty FindProperty(
			string propertyName, Component targetComponent, GameObject source, ScriptableObject scriptable)
		{
			...
			if (propInfo != null)
				TargetProperty = new MMProperty(
					targetComponent, MemberTypes.Property, propInfo, null, propertyName, scriptable);
			if (fieldInfo != null)
				TargetProperty = new MMProperty(
					targetComponent, MemberTypes.Field, null, fieldInfo, propertyName, scriptable);
			if (propertyName == "")
			{
				if (source != null)
					Debug.LogError(
						"The MMProperty on " + source.name + " : you need to pick a property from the Property list");
				return null;
			}
			...
			if (propInfo == null && fieldInfo == null)
			{
				if (source != null)
					Debug.LogError(
						"The MMProperty on " + source.name + " couldn't find any property or field named " +
