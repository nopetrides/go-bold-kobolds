...
namespace MoreMountains.Tools
{
	...
	public abstract class MMRadioSignal : MonoBehaviour
	{
		...
		public virtual float TimescaleTime => TimeScale == TimeScales.Scaled ? Time.time : Time.unscaledTime;

		/// the delta time, unscaled or not
		public virtual float TimescaleDeltaTime =>
			TimeScale == TimeScales.Scaled ? Time.deltaTime : Time.unscaledDeltaTime;
		...
		protected virtual void Update()
		{
			...
			if (_levelLastFrame != Level) ApplyLevel(Level);
		...
		{
			if (OnValueChange != null) OnValueChange.Invoke(level);
		}
