...
namespace MoreMountains.Tools
{
		...
		MMEventListener<MMSoundManagerTrackFadeEvent>
	{
		...
		{
			if (MMSoundManager.HasInstance) UpdateSliderValueWithTrackVolume();
		}
		...
		protected virtual void LateUpdate()
		{
			if (Mode == Modes.Read)
			{
				var trackVolume = MMSoundManager.Instance.GetTrackVolume(Track, false);
		...
		/// <summary>
		///     When we get an event letting us know the settings have been loaded, we update our slider to reflect the current
		///     track volume
		/// </summary>
		...
		public void OnMMEvent(MMSoundManagerTrackEvent trackEvent)
		{
			switch (trackEvent.TrackEventType)
			{
				case MMSoundManagerTrackEventTypes.MuteTrack:
					if (ChangeModeOnMute) ChangeModeToRead(ModeSwitchBufferTime);
					break;
				case MMSoundManagerTrackEventTypes.UnmuteTrack:
					if (ChangeModeOnUnmute) ChangeModeToRead(ModeSwitchBufferTime);
					break;
				case MMSoundManagerTrackEventTypes.SetVolumeTrack:
					if (ChangeModeOnTrackVolumeChange) ChangeModeToRead(ModeSwitchBufferTime);
					break;
			}
		}
		...
		{
			if (ChangeModeOnTrackFade) ChangeModeToRead(fadeEvent.FadeDuration + ModeSwitchBufferTime);
		}
		...
		public virtual void UpdateVolume(float newValue)
		{
			...
			var newVolume = MMMaths.Remap(newValue, 0f, 1f, MinVolume, MaxVolume);
		...
		{
			_slider.value = MMMaths.Remap(
				MMSoundManager.Instance.GetTrackVolume(Track, false), 0f, 1f, MinVolume, MaxVolume);
		}
	}
}
#endif
