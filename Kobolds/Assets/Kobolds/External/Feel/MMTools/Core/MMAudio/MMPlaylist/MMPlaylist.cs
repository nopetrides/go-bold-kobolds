...
namespace MoreMountains.Tools
{
	...
	public struct MMPlaylistVolumeMultiplierEvent
	{
		...
		public delegate void Delegate(
			int channel, float newVolumeMultiplier, bool applyVolumeMultiplierInstantly = false);
	...
	public struct MMPlaylistPitchMultiplierEvent
	{
		...
		public delegate void Delegate(
			int channel, float newPitchMultiplier, bool applyPitchMultiplierInstantly = false);
	...
	public class MMPlaylist : MMMonoBehaviour
	{
		...
		{
			if (!AutoHandleApplicationPause) return;
		...
		protected virtual IEnumerator PlaySong(int index)
		{
			...
			while (Songs[index].TargetAudioSource.isPlaying || PlaylistState.CurrentState == PlaylistStates.Paused ||
					_shouldResumeOnApplicationPause) yield return null;
		...
		protected virtual IEnumerator Fade(
			int index, float duration, float initialVolume, float endVolume, bool stopAtTheEnd)
		{
			if (index < 0 || index >= Songs.Count) yield break;
		...
		protected virtual int PickNextIndex()
		{
			...
			if (_queuedSong != -1)
			{
				var newRequestedIndex = _queuedSong;
				_queuedSong = -1;
		...
		protected virtual void OnMMPlaylistVolumeMultiplierEvent(
			int channel, float newVolumeMultiplier, bool applyVolumeMultiplierInstantly = false)
		{
			if (channel != Channel) return;
