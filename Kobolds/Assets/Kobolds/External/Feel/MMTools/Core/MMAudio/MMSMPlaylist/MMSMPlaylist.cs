...
namespace MoreMountains.Tools
{
	...
	public class MMSMPlaylist : ScriptableObject
	{
		...
		public virtual void Initialization()
		{
			...
			foreach (var song in Songs) song.Initialization();
		}
		...
		{
			var newIndex = currentSongIndex;

			if (Songs.Count == 0) return -1;

			if (queuedSongIndex != -1)
			{
				var newRequestedIndex = queuedSongIndex;
				queuedSongIndex = -1;
				return newRequestedIndex;
			}

			if (PlayCount >= Songs.Count && PlayMode == PlayModes.PlayOnce) return -2;

			if (PlayMode == PlayModes.PlayXTimes && PlayCount >= MaxAmountOfPlays) return -2;

			if (currentSongIndex >= 0 && currentSongIndex < Songs.Count && Songs[currentSongIndex].Options.Loop &&
				!bypassLoop) return currentSongIndex;

			switch (PlayOrder)
			{
				case PlayOrders.Random:
					while (newIndex == currentSongIndex) newIndex = Random.Range(0, Songs.Count);
					return newIndex;

				case PlayOrders.RandomUnique:

					var allPlayed = true;
					var lowestPlayCount = int.MaxValue;
					...
					{
						var random = Random.Range(0, _randomUniqueCandidates.Count);
		...
		{
			PlayCount = 0;
			foreach (var song in Songs) song.PlayCount = 0;
		}
	}
}
