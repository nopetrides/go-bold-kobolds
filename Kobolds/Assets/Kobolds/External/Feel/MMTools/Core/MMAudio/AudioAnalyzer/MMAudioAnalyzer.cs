...
namespace MoreMountains.Tools
{
	...
	public class Beat
	{
		...
		[MMEnumCondition(
			"Mode", (int) Modes.Raw, (int) Modes.Normalized, (int) Modes.BufferedRaw, (int) Modes.BufferedNormalized)]
	...
	public class MMAudioAnalyzer : MonoBehaviour
	{
		...
		protected virtual void HandleBuffer()
		{
			...
			{
				BufferedBandLevels[i] = Mathf.Max(
					BufferedBandLevels[i] * Mathf.Exp(-BufferSpeed * Time.deltaTime), BandLevels[i]);
		...
		protected virtual IEnumerator RemapBeat(Beat beat)
		{
			...
			while (Time.time - remapStartedAt < beat.RemappedAttack + beat.RemappedDecay)
			{
				...
				if (Time.time - remapStartedAt > beat.RemappedAttack)
					beat.CurrentValue = Mathf.Lerp(
						1f, 0f, (Time.time - remapStartedAt - beat.RemappedAttack) / beat.RemappedDecay);
				yield return null;
			}
