	...
	public class MMPropertyPickerDrawer : PropertyDrawer
	{
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			...
			if (viewData._TargetObject != null)
			{
				...
				if (viewData._selectedComponentIndex != 0) viewData._numberOfLines = 4;
			}

			if (viewData._targetIsScriptableObject) viewData._numberOfLines = 4;
		...
		{
			if (viewData._initialized) return;
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			// rectangles
			var targetLabelRect = new Rect(position.x, position.y, position.width, PropertyPickerViewData._lineHeight);
			var targetObjectRect = new Rect(
				position.x, position.y + (PropertyPickerViewData._lineHeight + PropertyPickerViewData._lineMargin),
				position.width, PropertyPickerViewData._lineHeight);
			var targetComponentRect = new Rect(
				position.x, position.y + (PropertyPickerViewData._lineHeight + PropertyPickerViewData._lineMargin) * 2,
				position.width, PropertyPickerViewData._lineHeight);
			var targetPropertyRect = new Rect(
				position.x, position.y + (PropertyPickerViewData._lineHeight + PropertyPickerViewData._lineMargin) * 3,
				position.width, PropertyPickerViewData._lineHeight);
			...
			EditorGUI.PropertyField(
				targetObjectRect, property.FindPropertyRelative("TargetObject"), new GUIContent("Target Object"), true);
			if (EditorGUI.EndChangeCheck())
			{
				...
				if (viewData._targetIsScriptableObject) FillPropertyList(property, viewData);
			}
			...
			{
				EditorGUI.BeginChangeCheck();
				viewData._selectedComponentIndex = EditorGUI.Popup(
					targetComponentRect, "Component", viewData._selectedComponentIndex, viewData._componentNames);
				if (EditorGUI.EndChangeCheck())
				{
					SetTargetComponent(property, viewData);
					viewData._selectedPropertyIndex = 0;
			...
			{
				EditorGUI.BeginChangeCheck();
				viewData._selectedPropertyIndex = EditorGUI.Popup(
					targetPropertyRect, "Property", viewData._selectedPropertyIndex, viewData._propertiesNames);
				if (EditorGUI.EndChangeCheck()) SetTargetProperty(property, viewData);
			}
		...
		protected virtual void DisplayAdditionalProperties(
			Rect position, SerializedProperty property, GUIContent label, PropertyPickerViewData viewData)
		{
		}

		protected virtual void DrawLevelProgressBar(
			Rect position, float level, Color frontColor, Color negativeColor, PropertyPickerViewData viewData)
		{
			var levelLabelRect = new Rect(
				position.x,
			...
			var levelValueRect = new Rect(
				position.x - 15 + EditorGUIUtility.labelWidth + 4,
				position.y + (PropertyPickerViewData._lineHeight + PropertyPickerViewData._lineMargin) *
				(viewData._numberOfLines - 1), position.width, PropertyPickerViewData._lineHeight);

			var progressX = position.x - 5 + EditorGUIUtility.labelWidth + 60;
			var progressY = position.y + (PropertyPickerViewData._lineHeight + PropertyPickerViewData._lineMargin) *
		...
		protected virtual void FillComponentsList(SerializedProperty property, PropertyPickerViewData viewData)
		{
			...
			var components = viewData._TargetGameObject.GetComponents(typeof(Component));
		...
		{
			if (viewData._TargetObject == null) return;
			...
			if (!viewData._targetIsScriptableObject)
			{
				...
				foreach (var thisFieldInfo in fieldsList)
				{
					var newEntry = thisFieldInfo.Name + " [Field - " + thisFieldInfo.FieldType.Name + "]";
		...
		protected virtual void GetComponentIndex(SerializedProperty property, PropertyPickerViewData viewData)
		{
			...
			foreach (var component in viewData._componentList)
			{
			...
			if (!found) viewData._selectedComponentIndex = 0;
		}
		...
		protected virtual void GetPropertyIndex(SerializedProperty property, PropertyPickerViewData viewData)
		{
			...
			foreach (var prop in viewData._propertiesList)
			{
			...
			if (!found) viewData._selectedPropertyIndex = 0;
		}

		protected virtual Type GetPropertyType(SerializedProperty property, PropertyPickerViewData viewData)
		{
			if (viewData._selectedPropertyIndex == 0) return null;

			MMProperty tempProperty;

			tempProperty = MMProperty.FindProperty(
				viewData._propertiesList[viewData._selectedPropertyIndex],
		...
		public class PropertyPickerViewData
		{
			...
			public int _selectedComponentIndex;
			public int _selectedPropertyIndex;
