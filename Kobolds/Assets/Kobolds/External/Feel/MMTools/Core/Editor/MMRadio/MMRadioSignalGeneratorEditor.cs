	...
	public class MMRadioSignalGeneratorEditor : MMRadioSignalEditor
	{
		...
		protected override void DrawProperties()
		{
			DrawPropertiesExcluding(
				serializedObject, "AnimatedPreview", "SignalList", "GlobalMultiplier", "CurrentLevel", "Clamp",
				"Clamps");
		...
		protected virtual void DrawRawSpectrum()
		{
			...
			// box
			GUILayout.Box(
				"", GUILayout.Width(_inspectorWidth - _externalMargin), GUILayout.Height(_rawSpectrumBoxHeight));
			...
			if (_animatedPreview.boolValue)
			{
				...
				zeroX = maxX - MMMaths.Remap(
					_normalizedTime, 0f, 1f, _spectrumBoxPosition.x + _externalMargin,
			...
			if (_duration.floatValue > 0f)
			{
				...
				if (Application.isPlaying)
					boxSpectrumValue = MMMaths.Remap(
						_radioSignal.GraphValue(_driverTime.floatValue), 0f, 1f, 0f, _spectrumBoxSize.y);
				else
					boxSpectrumValue = MMMaths.Remap(
						_radioSignal.GraphValue(_normalizedTime), 0f, 1f, 0f, _spectrumBoxSize.y);
			...
			{
				var xPosition = _spectrumBoxPosition.x + _externalMargin + MMMaths.Remap(
					i, 0, _spectrumPointsCount, 0f, _spectrumBoxSize.x - _externalMargin * 2);
				...
				var spectrumValuePrevious = MMMaths.Remap(
					_radioSignal.GraphValue(timePrevious), 0f, 1f, 0f, _spectrumBoxSize.y);
