...
namespace MoreMountains.Tools
{
	...
	public class MMAudioAnalyzerEditor : Editor
	{
		...
		protected virtual void DrawBandTable()
		{
			...
			if (_bandValuesFoldout)
			{
				...
				for (var i = 0; i < _numberOfBands; i++)
				{
					...
					GUILayout.Label(
						NormalizedBandLevels.GetArrayElementAtIndex(i).floatValue.ToString(), GUILayout.Width(w4));
					GUILayout.Label(
						NormalizedBufferedBandLevels.GetArrayElementAtIndex(i).floatValue.ToString(),
						GUILayout.Width(w5));
					GUILayout.FlexibleSpace();
					GUILayout.EndHorizontal();
				}
		...
		protected virtual void DrawBeats()
		{
			...
			var boxesPerLine = (int) Mathf.Round(
				(_inspectorWidth - margin - 3 * _externalMargin) / (beatsBoxSquareSize + margin));
		...
		protected virtual void DrawBandVisualization()
		{
			...
			// box
			GUILayout.Box(
				"", GUILayout.Width(_inspectorWidth - _externalMargin), GUILayout.Height(_bandsValuesBoxHeight));
			_boxPosition = GUILayoutUtility.GetLastRect().position;
			_boxSize = GUILayoutUtility.GetLastRect().size;
		...
		protected virtual void DrawRawSpectrum()
		{
			...
			if (Active)
			{
				...
				for (var i = 1; i < RawSpectrum.arraySize - 1; i++)
				{
					var xPosition = _spectrumBoxPosition.x + _externalMargin + MMMaths.Remap(
						i, 0, RawSpectrum.arraySize, 0f, _spectrumBoxSize.x - _externalMargin * 2);
			...
			else
			{
				...
				{
					var xPosition = _spectrumBoxPosition.x + _externalMargin + MMMaths.Remap(
						i, 0, points, 0f, _spectrumBoxSize.x - _externalMargin * 2);
					var yPosition = _spectrumBoxPosition.y + _spectrumBoxSize.y / 2;
