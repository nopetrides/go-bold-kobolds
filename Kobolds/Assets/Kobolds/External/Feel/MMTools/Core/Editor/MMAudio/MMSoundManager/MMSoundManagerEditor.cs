...
namespace MoreMountains.Tools
{
	...
	public class MMSoundManagerEditor : Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			{
				DrawTrack(
					"Master Track", _mmSoundManager.settingsSo.Settings.MasterOn,
				...
				DrawTrack(
					"Music Track", _mmSoundManager.settingsSo.Settings.MusicOn,
				...
				DrawTrack(
					"SFX Track", _mmSoundManager.settingsSo.Settings.SfxOn, MMSoundManager.MMSoundManagerTracks.Sfx,
					_sfxColorMute, _sfxColorUnmute, _sfxColorPause, _sfxColorStop, _sfxColorPlay, _sfxColorFree);
				DrawTrack(
					"UI Track", _mmSoundManager.settingsSo.Settings.UIOn, MMSoundManager.MMSoundManagerTracks.UI,
		...
		protected virtual void DrawTrack(
			string title, bool mute, MMSoundManager.MMSoundManagerTracks track, Color muteColor, Color unmuteColor,
			Color pauseColor, Color stopColor, Color playColor, Color freeColor)
		{
			GUILayout.Space(10);
			...
			switch (track)
			{
				case MMSoundManager.MMSoundManagerTracks.Master:
					newVolume = EditorGUILayout.Slider(
						_masterVolume, MMSoundManagerSettings._minimalVolume, MMSoundManagerSettings._maxVolume);
					if (newVolume != _masterVolume)
						_mmSoundManager.settingsSo.SetTrackVolume(
							MMSoundManager.MMSoundManagerTracks.Master, newVolume);
					break;
				case MMSoundManager.MMSoundManagerTracks.Music:
					newVolume = EditorGUILayout.Slider(
						_musicVolume, MMSoundManagerSettings._minimalVolume, MMSoundManagerSettings._maxVolume);
				...
				case MMSoundManager.MMSoundManagerTracks.Sfx:
					newVolume = EditorGUILayout.Slider(
						_sfxVolume, MMSoundManagerSettings._minimalVolume, MMSoundManagerSettings._maxVolume);
				...
				case MMSoundManager.MMSoundManagerTracks.UI:
					newVolume = EditorGUILayout.Slider(
						_uiVolume, MMSoundManagerSettings._minimalVolume, MMSoundManagerSettings._maxVolume);
			...
			EditorGUILayout.BeginHorizontal();
			{
				if (mute)
					...
					DrawColoredButton(
						"Unmute", unmuteColor, track, _mmSoundManager.UnmuteTrack, EditorStyles.miniButtonMid);
		...
		public virtual void DrawColoredButton(
			string buttonLabel, Color buttonColor, MMSoundManager.MMSoundManagerTracks track,
			Action<MMSoundManager.MMSoundManagerTracks> action, GUIStyle styles)
		{
