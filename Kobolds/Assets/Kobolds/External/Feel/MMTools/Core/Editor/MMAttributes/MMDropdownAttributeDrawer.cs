		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			if (_dropdownAttribute == null)
			{
				...
				for (var i = 0; i < _dropdownValues.Length; i++)
				{
					...
					if (_propertyType == typeof(float) && Mathf.Approximately(
							property.floatValue, Convert.ToSingle(_dropdownValues[i])))
					{
						_selectedDropdownValueIndex = i;
						found = true;
						break;
			...
			_selectedDropdownValueIndex = EditorGUI.Popup(
				position, label.text, _selectedDropdownValueIndex, _dropdownValues);
