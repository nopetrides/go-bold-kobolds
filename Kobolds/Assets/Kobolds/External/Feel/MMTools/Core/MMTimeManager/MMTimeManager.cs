...
namespace MoreMountains.Feedbacks
{
	...
	/// <summary>
	///     The possible modes with which to interpolate time scale. Speed is a legacy mode, the recommended mode if you want
	///     to lerp timescale would be Duration, which offers the most options and accuracy
	/// </summary>
	...
	public struct MMTimeScaleEvent
	{
		...
		public delegate void Delegate(
			MMTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp, float lerpSpeed,
			bool infinite,
		...
		{
			OnEvent?.Invoke(
				timeScaleMethod, timeScale, duration, lerp, lerpSpeed, infinite, timeScaleLerpMode, timeScaleLerpCurve,
				timeScaleLerpDuration, timeScaleLerpOnReset, timeScaleLerpCurveOnReset, timeScaleLerpDurationOnReset);
		}
	...
	public class MMTimeManager : MMSingleton<MMTimeManager>
	{
		...
		protected virtual void Update()
		{
			...
			while (_timeScaleProperties.Count > 0)
			{
				...
				if (_currentProperty.Duration > 0f || _currentProperty.Infinite) break; // keep current property values

				Unfreeze(); // pop current property
			}

			if (_timeScaleProperties.Count == 0) TargetTimeScale = NormalTimeScale;
			...
			if (_currentProperty.TimeScaleLerp)
			{
				if (_currentProperty.TimeScaleLerpMode == MMTimeScaleLerpModes.Speed)
				{
					...
					ApplyTimeScale(
						Mathf.Lerp(
							Time.timeScale, TargetTimeScale, Time.unscaledDeltaTime * _currentProperty.LerpSpeed));
				}
				else if (_currentProperty.TimeScaleLerpMode == MMTimeScaleLerpModes.Duration)
				{
					var timeSinceStart = Time.unscaledTime - _startedAt;
					var progress = MMMaths.Remap(timeSinceStart, 0f, _currentProperty.TimeScaleLerpDuration, 0f, 1f);
					var delta = _currentProperty.TimeScaleLerpCurve.Evaluate(progress);
		...
		protected virtual void ApplyTimeScale(float newValue)
		{
			...
			if (newValue == _timeScaleLastTime) return;

			if (UpdateTimescale) Time.timeScale = newValue;

			if (UpdateFixedDeltaTime && newValue != 0) Time.fixedDeltaTime = _initialFixedDeltaTime * newValue;

			if (UpdateMaximumDeltaTime) Time.maximumDeltaTime = _initialMaximumDeltaTime * newValue;
		...
		protected virtual void SetTimeScale(TimeScaleProperties timeScaleProperties)
		{
			...
			{
				timeScaleProperties.Duration = Mathf.Max(
					timeScaleProperties.Duration, timeScaleProperties.TimeScaleLerpDuration);
				timeScaleProperties.Duration = Mathf.Max(
					timeScaleProperties.Duration, timeScaleProperties.TimeScaleLerpDurationOnReset);
			}
			...
			if (_timeScaleProperties.Count == 0)
			{
					...
					_resetProperty.TimeScaleLerpOnReset)
				{
					...
					MMTimeScaleEvent.Trigger(
						MMTimeScaleMethods.For, NormalTimeScale, _resetProperty.TimeScaleLerpDuration,
						_resetProperty.TimeScaleLerp,
		...
		public virtual void OnTimeScaleEvent(
			MMTimeScaleMethods timeScaleMethod, float timeScale, float duration, bool lerp, float lerpSpeed,
			bool infinite,
			MMTimeScaleLerpModes timeScaleLerpMode = MMTimeScaleLerpModes.Speed, MMTweenType timeScaleLerpCurve = null,
			float timeScaleLerpDuration = 0.2f,
