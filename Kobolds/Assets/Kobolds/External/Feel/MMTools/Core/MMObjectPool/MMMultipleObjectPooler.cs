...
namespace MoreMountains.Tools
{
	...
	public class MMMultipleObjectPooler : MMObjectPooler
	{
		...
		{
			if (MutualizedPoolName == null || MutualizedPoolName == "") return "[MultipleObjectPooler] " + name;
		...
		protected virtual GameObject AddOneObjectToThePool(GameObject typeOfObject)
		{
			...
			var initialStatus = typeOfObject.activeSelf;
		...
		protected virtual GameObject GetPooledGameObjectOriginalOrder()
		{
			...
			{
				var findObject = FindInactiveObject(
					_objectPool.PooledGameObjects[_currentIndex].gameObject.name, _objectPool.PooledGameObjects);
				if (findObject != null)
				{
					_currentIndex++;
					return findObject;
		...
		protected virtual GameObject GetPooledGameObjectOriginalOrderSequential()
		{
			...
			{
				var findObject = FindInactiveObject(
					Pool[_currentIndex].GameObjectToPool.name, _objectPool.PooledGameObjects);
				if (findObject != null)
				{
					_currentCount++;
		...
		protected virtual GameObject GetPooledGameObjectPoolSizeBased()
		{
			...
			if (_objectPool.PooledGameObjects[randomIndex].gameObject.activeInHierarchy)
			{
				...
				var pulledObject = FindInactiveObject(
					_objectPool.PooledGameObjects[randomIndex].gameObject.name, _objectPool.PooledGameObjects);
		...
		protected virtual GameObject GetPooledGameObjectRandomBetweenObjects()
		{
					...
					&& overflowCounter < _objectPool.PooledGameObjects.Count)
			{
				...
				var newGameObject = FindInactiveObject(
					Pool[randomIndex].GameObjectToPool.name, _objectPool.PooledGameObjects);
