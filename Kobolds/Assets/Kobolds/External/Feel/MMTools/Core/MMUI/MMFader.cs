...
namespace MoreMountains.Tools
{
	...
	public struct MMFadeEvent
	{
		...
		public MMFadeEvent(
			float duration, float targetAlpha, MMTweenType tween, int id = 0,
	...
	public struct MMFadeInEvent
	{
		...
		public MMFadeInEvent(
			float duration, MMTweenType tween, int id = 0,
			bool ignoreTimeScale = true, Vector3 worldPosition = new())
		{
			ID = id;
			Duration = duration;
		...
		MMEventListener<MMFadeOutEvent>, MMEventListener<MMFadeStopEvent>
	{
		...
		public virtual void OnMMEvent(MMFadeStopEvent fadeStopEvent)
		{
			...
			{
				_fading = false;
				if (fadeStopEvent.Restore) _canvasGroup.alpha = _initialAlpha;
			}
		}
		...
		protected virtual void Fade()
		{
			...
			{
				var result = MMTween.Tween(
					currentTime, _fadeStartedAt, endTime, _initialAlpha, _currentTargetAlpha, _currentCurve);
		...
		protected virtual void StartFading(
			float initialAlpha, float endAlpha, float duration, MMTweenType curve, bool ignoreTimeScale)
		{
