...
namespace MoreMountains.Tools
{
		...
		MMEventListener<MMFadeOutEvent>, MMEventListener<MMFadeStopEvent>
	{
		...
		[MMInspectorButtonBar(
			new[] {"FadeIn1Second", "FadeOut1Second", "DefaultFade", "ResetFader"},
			new[] {"FadeIn1Second", "FadeOut1Second", "DefaultFade", "ResetFader"},
		...
		public virtual void OnMMEvent(MMFadeEvent fadeEvent)
		{
			...
			StartCoroutine(
				StartFading(
					status, fadeEvent.Duration, fadeEvent.Curve, fadeEvent.ID,
		...
		public virtual void OnMMEvent(MMFadeInEvent fadeEvent)
		{
			StartCoroutine(
				StartFading(
					true, fadeEvent.Duration, fadeEvent.Curve, fadeEvent.ID,
					fadeEvent.IgnoreTimeScale, fadeEvent.WorldPosition));
		}
		...
		public virtual void OnMMEvent(MMFadeOutEvent fadeEvent)
		{
			StartCoroutine(
				StartFading(
					false, fadeEvent.Duration, fadeEvent.Curve, fadeEvent.ID,
					fadeEvent.IgnoreTimeScale, fadeEvent.WorldPosition));
		}
		...
		public virtual void OnMMEvent(MMFadeStopEvent fadeStopEvent)
		{
			...
			{
				_fading = false;
				if (fadeStopEvent.Restore) _rectTransform.anchoredPosition = _initialPosition;
			}
		}
		...
		protected virtual void Fade()
		{
			...
			{
				_newPosition = MMTween.Tween(
					currentTime, _fadeStartedAt, endTime, _fromPosition, _toPosition, _currentCurve);
		...
		protected virtual IEnumerator StartFading(
			bool fadingIn, float duration, MMTweenType curve, int id,
