...
{
	/// <summary>
	///     Add this component to a scene and it'll let you save and load the state of objects that implement the IMMPersistent
	///     interface
	///     You can create your own classes that implement this interface, or use the MMPersistent class that comes with this
	///     package
...
#region SAVE_AND_LOAD
		...
		public virtual void SaveToMemory()
		{
			...
			var sceneData = new MMPersistenceSceneData();
			sceneData.ObjectDatas = new DictionaryStringString();

			var persistents = FindAllPersistentObjects();
		...
		public virtual void LoadFromMemory()
		{
			...
			if (!SceneDatas.TryGetValue(_currentSceneName, out var sceneData)) return;

			if (sceneData.ObjectDatas == null) return;

			var persistents = FindAllPersistentObjects();
			foreach (var persistent in persistents)
				if (sceneData.ObjectDatas.TryGetValue(persistent.GetGuid(), out var data))
		...
		{
			var saveData = new MMPersistenceManagerData();
		...
		{
			var saveData = (MMPersistenceManagerData) MMSaveLoadManager.Load(
				typeof(MMPersistenceManagerData), DetermineSaveName(), _saveFolderName);
		...
		{
			SaveToMemory();
			if (SaveToFileOnSaveEvents) SaveFromMemoryToFile();
		}
		...
		{
			if (LoadFromFileOnLoadEvents) LoadFromFileToMemory();
			LoadFromMemory();
		}
...
#region EVENTS
		...
		{
			if (gameEvent.EventName == "Save" && ListenForSaveEvents) Save();
			if (gameEvent.EventName == "Load" && ListenForLoadEvents) Load();
			if (gameEvent.EventName == "SaveToMemory" && ListenForSaveToMemoryEvents) SaveToMemory();
			if (gameEvent.EventName == "LoadFromMemory" && ListenForLoadFromMemoryEvents) LoadFromMemory();
			if (gameEvent.EventName == "SaveToFile" && ListenForSaveToFileEvents) SaveFromMemoryToFile();
			if (gameEvent.EventName == "LoadFromFile" && ListenForLoadFromFileEvents) LoadFromFileToMemory();
		}
