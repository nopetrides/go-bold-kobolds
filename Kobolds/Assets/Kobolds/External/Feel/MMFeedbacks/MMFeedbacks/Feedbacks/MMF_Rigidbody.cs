	...
	public class MMF_Rigidbody : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (ExtraTargetRigidbodies != null)
				foreach (var rb in ExtraTargetRigidbodies)
					ApplyForce(rb);
		}
		...
		{
			if (ResetVelocityOnPlay) rb.linearVelocity = Vector3.zero;

			if (ForwardForce) _force = _force.magnitude * rb.transform.forward;
