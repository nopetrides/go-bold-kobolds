...
{
	/// <summary>
	///     This feedback allows you to chain any number of target MMF Players and play them in sequence, with optional delays
	///     before and after
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp(
	...
	public class MMF_PlayerChain : MMF_Feedback
	{
		...
		public override float FeedbackDuration
		{
			get
			{
				...
				{
					if (item == null || item.TargetPlayer == null || item.Inactive) continue;
		...
		protected virtual IEnumerator PlayChain()
		{
			...
			foreach (var item in Players)
			{
				...
				if (item.WaitUntilComplete)
				{
					...
					while (item.TargetPlayer.IsPlaying) yield return null;
				}
				else
				{
					item.TargetPlayer.PlayFeedbacks();
			...
			while (FeedbacksStillPlaying()) yield return null;
			IsPlaying = false;
		}

		protected virtual bool FeedbacksStillPlaying()
		{
			var feedbacksStillPlaying = false;
			foreach (var item in Players)
		...
		protected override void CustomRestoreInitialValues()
		{
			...
			foreach (var item in Players) item.TargetPlayer.RestoreInitialValues();
		}
