...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Rotation : MMF_Feedback
	{
		...
		public MMTweenType AnimateRotationTweenX = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)), "AnimateX");
		...
		public MMTweenType AnimateRotationTweenY = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)), "AnimateY");
		...
		public MMTweenType AnimateRotationTweenZ = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)), "AnimateZ");
		...
		/// how the x part of the rotation should animate over time, in degrees
		[Tooltip("how the x part of the rotation should animate over time, in degrees")]
		public MMTweenType ToDestinationTween = new(
			MMTween.MMTweenCurve.EaseInQuintic, "", "Mode", (int) Modes.ToDestination);
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (Active || Owner.AutoPlayOnEnable)
			{
				if (Mode == Modes.Absolute || Mode == Modes.Additive)
				{
					...
					ClearCoroutine();
					_coroutine = Owner.StartCoroutine(
						AnimateRotation(
							AnimateRotationTarget, Vector3.zero, FeedbackDuration, AnimateRotationTweenX,
							AnimateRotationTweenY, AnimateRotationTweenZ, RemapCurveZero * intensityMultiplier,
		...
		protected virtual IEnumerator RotateToDestination()
		{
			...
			if (AnimateRotationTweenX == null || AnimateRotationTweenY == null ||
				AnimateRotationTweenZ == null) yield break;

			if (FeedbackDuration == 0f) yield break;

			var tempAngles = DestinationAngles;
			if (ToDestinationTransform != null) tempAngles = ToDestinationTransform.eulerAngles;

			var destinationAngles = NormalPlayDirection ? tempAngles : _initialToDestinationAngles;
		...
		protected virtual void ApplyRotation(
			Transform targetTransform, float remapZero, float remapOne, MMTweenType curveX, MMTweenType curveY,
			MMTweenType curveZ, float percent)
		{
			...
			if (AnimateX)
			{
				var x = MMTween.Tween(percent, 0f, 1f, remapZero, remapOne, curveX);
				targetTransform.Rotate(Vector3.right, x, RotationSpace);
			}

			if (AnimateY)
			{
				var y = MMTween.Tween(percent, 0f, 1f, remapZero, remapOne, curveY);
				targetTransform.Rotate(Vector3.up, y, RotationSpace);
			}

			if (AnimateZ)
			{
				var z = MMTween.Tween(percent, 0f, 1f, remapZero, remapOne, curveZ);
		...
		public override string RequiresSetupText =>
			"This feedback requires that you set an AnimateRotationTarget to be able to work properly. You can set one below.";
