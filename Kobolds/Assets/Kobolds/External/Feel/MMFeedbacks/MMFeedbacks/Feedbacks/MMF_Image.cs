...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Image : MMF_Feedback
	{
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			if (Active)
			{
				if (DisableOnInit) Turn(false);

				if (BoundImage == null)
					Debug.LogWarning(
						"[Image Feedback] The image feedback on " + Owner.name +
						" doesn't have a BoundImage, it won't work. You need to specify an Image in its inspector.");
				else
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (EnableOnPlay) Turn(true);
			switch (Mode)
			{
				case Modes.Instant:
					if (ModifyColor) BoundImage.color = NormalPlayDirection ? InstantColor : _initialInstantColor;
					break;
				case Modes.OverTime:
		...
		protected virtual IEnumerator ImageSequence()
		{
			...
			if (DisableOnSequenceEnd) Turn(false);
			IsPlaying = false;
			_coroutine = null;
			yield return null;
		}
