...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Scale : MMF_Feedback
	{
		...
		public MMTweenType AnimateScaleTweenX = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)), "AnimateX");

		/// the y scale animation definition
		[Tooltip("the y scale animation definition")]
		public MMTweenType AnimateScaleTweenY = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)), "AnimateY");

		/// the z scale animation definition
		[Tooltip("the z scale animation definition")]
		public MMTweenType AnimateScaleTweenZ = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1.5f), new Keyframe(1, 0)), "AnimateZ");
			...
			if (Active || Owner.AutoPlayOnEnable)
			{
				if (Mode == Modes.Absolute || Mode == Modes.Additive)
				{
					...
					_coroutine = Owner.StartCoroutine(
						AnimateScale(
							AnimateScaleTarget, Vector3.zero, FeedbackDuration, AnimateScaleTweenX, AnimateScaleTweenY,
							AnimateScaleTweenZ, RemapCurveZero * intensityMultiplier,
		...
		protected virtual IEnumerator ScaleToDestination()
		{
			...
			if (AnimateScaleTweenX == null || AnimateScaleTweenY == null || AnimateScaleTweenZ == null) yield break;
			...
			while (journey >= 0 && journey <= FeedbackDuration && FeedbackDuration > 0)
			{
				...
				if (AnimateX)
				{
					_newScale.x = Mathf.LerpUnclamped(
						_initialScale.x, DestinationScale.x, AnimateScaleTweenX.Evaluate(percent) + Offset);
					_newScale.x = MMFeedbacksHelpers.Remap(_newScale.x, 0f, 1f, RemapCurveZero, RemapCurveOne);
				}

				if (AnimateY)
				{
					_newScale.y = Mathf.LerpUnclamped(
						_initialScale.y, DestinationScale.y, AnimateScaleTweenY.Evaluate(percent) + Offset);
					_newScale.y = MMFeedbacksHelpers.Remap(_newScale.y, 0f, 1f, RemapCurveZero, RemapCurveOne);
				}

				if (AnimateZ)
				{
					_newScale.z = Mathf.LerpUnclamped(
						_initialScale.z, DestinationScale.z, AnimateScaleTweenZ.Evaluate(percent) + Offset);
		...
		/// <returns></returns>
		protected virtual IEnumerator AnimateScale(
			Transform targetTransform, Vector3 vector, float duration, MMTweenType curveX, MMTweenType curveY,
			MMTweenType curveZ, float remapCurveZero = 0f, float remapCurveOne = 1f)
		{
