...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Animation : MMF_Feedback
	{
		...
		[MMFEnumCondition(
			"FloatValueMode", (int) ValueModes.Constant, (int) ValueModes.Random, (int) ValueModes.Incremental)]
		...
		[MMFEnumCondition(
			"IntValueMode", (int) ValueModes.Constant, (int) ValueModes.Random, (int) ValueModes.Incremental)]
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			if (RandomTriggerParameterNames == null) RandomTriggerParameterNames = new List<string>();
			if (RandomBoolParameterNames == null) RandomBoolParameterNames = new List<string>();
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			{
				Debug.LogWarning(
					"[Animation Feedback] The animation feedback on " + Owner.name +
					" doesn't have a BoundAnimator, it won't work. You need to specify one in its inspector.");
				return;
			}
			...
			foreach (var animator in ExtraBoundAnimators) ApplyValue(animator, intensityMultiplier);
		}
		...
		protected virtual void ApplyValue(Animator targetAnimator, float intensityMultiplier)
		{
			if (UpdateTrigger)
			{
				if (TriggerMode == TriggerModes.SetTrigger) targetAnimator.SetTrigger(_triggerParameter);
				if (TriggerMode == TriggerModes.ResetTrigger) targetAnimator.ResetTrigger(_triggerParameter);
			}
			...
			if (UpdateRandomTrigger)
			{
				...
				if (RandomTriggerMode == TriggerModes.SetTrigger) targetAnimator.SetTrigger(randomParameter);
				if (RandomTriggerMode == TriggerModes.ResetTrigger) targetAnimator.ResetTrigger(randomParameter);
			}

			if (UpdateBool) targetAnimator.SetBool(_boolParameter, BoolParameterValue);
			...
			switch (IntValueMode)
			{
				...
				case ValueModes.Incremental:
					var newValue = targetAnimator.GetInteger(_intParameter) + IntIncrement;
			...
			switch (FloatValueMode)
			{
				case ValueModes.Constant:
					...
					break;
				case ValueModes.Incremental:
					var newValue = targetAnimator.GetFloat(_floatParameter) + FloatIncrement * intensityMultiplier;
			...
			if (SetLayerWeight) targetAnimator.SetLayerWeight(TargetLayerIndex, NewWeight);
		}
		...
		protected override void CustomStopFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			foreach (var animator in ExtraBoundAnimators) animator.SetBool(_boolParameter, false);
		}
