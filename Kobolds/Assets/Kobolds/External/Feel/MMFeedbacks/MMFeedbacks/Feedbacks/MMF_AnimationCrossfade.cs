...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_AnimationCrossfade : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			{
				Debug.LogWarning(
					"[Animation Crossfade Feedback] The animation crossfade feedback on " + Owner.name +
					" doesn't have a BoundAnimator, it won't work. You need to specify one in its inspector.");
				return;
			}

			CrossFade(BoundAnimator);
			foreach (var animator in ExtraBoundAnimators) CrossFade(animator);
		}
			...
			switch (Mode)
			{
				case Modes.Seconds:
					targetAnimator.CrossFadeInFixedTime(
						_stateHashName, TransitionDuration, Layer, TimeOffset, NormalizedTransitionTime);
					break;
				case Modes.Normalized:
					targetAnimator.CrossFade(
						_stateHashName, NormalizedTransitionDuration, Layer, NormalizedTimeOffset,
