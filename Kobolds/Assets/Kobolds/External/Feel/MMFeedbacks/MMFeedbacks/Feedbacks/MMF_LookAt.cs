...
namespace MoreMountains.Feedbacks
{
	...
	///     You can also use it to broadcast a MMLookAtShake event, that MMLookAtShakers on the right channel will be able to
	///     listen for and act upon
	/// </summary>
	[AddComponentMenu("")]
	[FeedbackHelp(
	...
	public class MMF_LookAt : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (Active || Owner.AutoPlayOnEnable) InitiateLookAt(position);
		}
		...
		protected virtual void InitiateLookAt(Vector3 position)
		{
			...
			switch (Mode)
			{
				case Modes.Direct:
					...
					break;
				case Modes.Event:
					MMLookAtShaker.MMLookAtShakeEvent.Trigger(
						Duration, LockXAxis, LockYAxis, LockZAxis, UpwardVector,
		...
		{
			if (TransformToRotate != null) _initialDirectTargetTransformRotation = TransformToRotate.transform.rotation;

			var duration = FeedbackDuration;
		...
		protected virtual void ApplyRotation(float percent)
		{
			...
			TransformToRotate.transform.rotation = Quaternion.SlerpUnclamped(
				_initialDirectTargetTransformRotation, _newRotation, percent);
		}
		...
		{
			if (Mode == Modes.Direct) return TransformToRotate == null;
		...
		public override string RequiredTargetText
		{
			get
			{
				if (Mode == Modes.Direct && TransformToRotate != null) return TransformToRotate.name;
