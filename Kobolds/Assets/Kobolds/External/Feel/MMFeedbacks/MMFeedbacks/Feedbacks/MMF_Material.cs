...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Material : MMF_Feedback
	{
		...
		[FormerlySerializedAs("MaterialIndexes")] [Tooltip(
			"the list of material indexes we want to change on the target renderer. If left empty, will only target the material at index 0")]
		...
		protected virtual void InitializeMaterials()
		{
			...
			for (var i = 0; i < _initialMaterials.Length; i++)
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			{
				Debug.LogWarning(
					"[Material Feedback] The material feedback on " + Owner.name + " has an empty Materials array.");
				return;
			}
			...
			{
				Debug.LogWarning(
					"[Material Feedback] The material feedback on " + Owner.name +
					" is attempting to switch to a null material.");
				return;
			}
		...
		/// <returns></returns>
		protected virtual IEnumerator TransitionMaterial(
			Material originalMaterial, Material newMaterial, int materialIndex)
		{
			IsPlaying = true;
			_startedAt = GetTime();
		...
		protected virtual int DetermineNextIndex()
		{
			switch (Method)
			{
				...
				case Methods.Sequential:
					_currentIndex++;
					if (_currentIndex >= Materials.Count) _currentIndex = Loop ? 0 : _currentIndex - 1;
					return _currentIndex;
			}
