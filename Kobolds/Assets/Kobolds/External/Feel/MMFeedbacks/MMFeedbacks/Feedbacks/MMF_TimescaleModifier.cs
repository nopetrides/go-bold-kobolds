...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_TimescaleModifier : MMF_Feedback
	{
		...
		/// the selected mode
		[Tooltip(
			"the selected mode : shake : changes the timescale for a certain duration" +
			"- change : sets the timescale to a new value, forever (until you change it again)" +
			"- reset : resets the timescale to its previous value")]
		...
		public MMTweenType TimescaleLerpCurve = new(
		...
		/// in Duration mode, the curve to use to lerp the timescale
		[Tooltip("in Duration mode, the curve to use to lerp the timescale")]
		public MMTweenType TimescaleLerpCurveOnReset = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(1, 1)),
			enumConditionPropertyName: "TimescaleLerpMode", enumConditionValues: (int) MMTimeScaleLerpModes.Duration);
			...
			switch (Mode)
			{
				case Modes.Shake:
					MMTimeScaleEvent.Trigger(
						MMTimeScaleMethods.For, TimeScale, FeedbackDuration, TimeScaleLerp, TimeScaleLerpSpeed, false,
						TimescaleLerpMode, TimescaleLerpCurve, TimescaleLerpDuration, TimeScaleLerpOnReset,
						TimescaleLerpCurveOnReset, TimescaleLerpDurationOnReset);
					break;
				case Modes.Change:
					MMTimeScaleEvent.Trigger(
						MMTimeScaleMethods.For, TimeScale, 0f, TimeScaleLerp, TimeScaleLerpSpeed, true,
						TimescaleLerpMode, TimescaleLerpCurve, TimescaleLerpDuration, TimeScaleLerpOnReset,
						TimescaleLerpCurveOnReset, TimescaleLerpDurationOnReset);
					break;
				case Modes.Reset:
		...
		{
			var (timeManager, createdNew) =
				Owner.gameObject.MMFindOrCreateObjectOfType<MMTimeManager>("MMTimeManager", null);
			if (createdNew) MMDebug.DebugLogInfo("Added a MMTimeManager to the scene. You're all set.");
		}
