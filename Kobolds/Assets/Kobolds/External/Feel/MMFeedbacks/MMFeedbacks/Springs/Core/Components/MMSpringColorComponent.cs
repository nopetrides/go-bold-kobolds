...
namespace MoreMountains.Feedbacks
{
	...
	public struct MMSpringColorEvent
	{
		...
		public static void Trigger(
			SpringCommands command, MMSpringComponentBase targetSpring, MMChannelData channelData,
		...
		where T : Component
	{
		...
		[MMInspectorButtonBar(
			new[] {"MoveTo", "MoveToAdditive", "MoveToSubtractive", "MoveToRandom", "MoveToInstant"},
			new[]
			{
		...
		[MMInspectorButtonBar(
			new[] {"Bump", "BumpRandom"},
			new[] {"TestBump", "TestBumpRandom"},
			new[] {true, true},
			new[] {"main-call-to-action", ""})]
		public bool BumpToToolbar;

		[MMInspectorButtonBar(
			new[] {"Stop", "Finish", "RestoreInitialValue", "ResetInitialValue"},
			new[] {"Stop", "Finish", "RestoreInitialValue", "ResetInitialValue"},
		...
		public override bool LowVelocity =>
			Mathf.Abs(ColorSpring.Velocity.r) + Mathf.Abs(ColorSpring.Velocity.g) + Mathf.Abs(ColorSpring.Velocity.b) +
			Mathf.Abs(ColorSpring.Velocity.a) + Mathf.Abs(ColorSpring.ColorSpring.Velocity) < _velocityLowThreshold;
...
#region PUBLIC_API
		...
		protected override void UpdateSpringValue()
		{
			if (_bumping)
			{
				var t = ColorSpring.ColorSpring.CurrentValue * BumpMultiplier;
...
#region EVENTS
		...
		{
			var eventMatch = springEvent.ChannelData != null && MMChannel.Match(
				springEvent.ChannelData, ChannelMode, Channel, MMChannelDefinition);
			var targetMatch = springEvent.TargetSpring != null && springEvent.TargetSpring.Equals(this);
			if (!eventMatch && !targetMatch) return;

			if (springEvent.OverrideDamping) ColorSpring.SetDamping(springEvent.NewDamping);
			if (springEvent.OverrideFrequency) ColorSpring.SetFrequency(springEvent.NewFrequency);
