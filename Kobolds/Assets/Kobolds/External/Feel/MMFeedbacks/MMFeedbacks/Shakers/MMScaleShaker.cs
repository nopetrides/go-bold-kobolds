...
{
	/// <summary>
	///     This shaker will let you move the scale of a transform, either once or permanently, shaking its scale for the
	///     specified duration and within the specified range.
	...
	public class MMScaleShaker : MMShaker
	{
		...
		{
			GrabInitialScale();
			if (RandomizeSeedOnShake) RandomnessSeed = Random.insideUnitSphere;
		...
		protected override void ShakeComplete()
		{
			...
			_attenuation = 0f;
			_newScale = ComputeNewScale();
			if (TargetTransform != null) ApplyNewScale(_newScale);
		}
		...
		{
			if (!AddDirectionalNoise) return _noNoise;
		...
		public virtual void OnMMScaleShakeEvent(
			float duration, float shakeSpeed, float shakeRange, Vector3 shakeMainDirection, bool randomizeDirection,
			Vector3 shakeAltDirection, bool randomizeDirectionOnPlay, bool addDirectionalNoise,
			...
			AnimationCurve rangeFalloff = null, Vector2 remapRangeFalloff = default, Vector3 rangePosition = default,
			...
			TimescaleModes timescaleMode = TimescaleModes.Scaled, bool stop = false, bool restore = false)
		{
			...
			if (!OnlyUseShakerValues)
			{
				...
				ShakeRange = shakeRange * feedbacksIntensity * ComputeRangeIntensity(
					useRange, rangeDistance,
	...
	public struct MMScaleShakeEvent
	{
		...
		{
			OnEvent?.Invoke(
				duration, shakeSpeed, shakeRange, shakeMainDirection, randomizeDirection, shakeAltDirection,
