...
#region Events
	...
	public struct MMFloatingTextSpawnEvent
	{
		...
		public delegate void Delegate(
			...
			Gradient animateColorGradient = null, bool useUnscaledTime = false, Transform attachmentTransform = null);
		...
		{
			OnEvent?.Invoke(
				channelData, spawnPosition, value, direction, intensity, forceLifetime, lifetime, forceColor,
				animateColorGradient, useUnscaledTime, attachmentTransform);
		}
	}
	...
	public class MMFloatingTextSpawner : MMMonoBehaviour
	{
		...
		[MMCondition("AnimateScale", true)]
		public AnimationCurve AnimateScaleCurve = new(
			new Keyframe(0f, 0f), new Keyframe(0.15f, 1f), new Keyframe(0.85f, 1f), new Keyframe(1f, 0f));
		...
		[MMCondition("AnimateOpacity", true)]
		public AnimationCurve AnimateOpacityCurve = new(
		...
		public virtual void Spawn(
			string value, Vector3 position, Vector3 direction, float intensity = 1f,
			bool forceLifetime = false, float lifetime = 1f, bool forceColor = false,
			...
			Transform attachmentTransform = null)
		{
			...
			_floatingText.SetProperties(
				value, _lifetime, _direction, AnimateMovement,
		...
		public virtual void OnMMFloatingTextSpawnEvent(
			...
			Gradient animateColorGradient = null, bool useUnscaledTime = false, Transform attachmentTransform = null)
		{
			if (!MMChannel.Match(channelData, ChannelMode, Channel, MMChannelDefinition)) return;

			UseUnscaledTime = useUnscaledTime;
			Spawn(
				value, spawnPosition, direction, intensity, forceLifetime, lifetime, forceColor, animateColorGradient,
				attachmentTransform);
		}
