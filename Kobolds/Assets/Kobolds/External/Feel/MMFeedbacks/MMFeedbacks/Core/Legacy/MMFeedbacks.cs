...
namespace MoreMountains.Feedbacks
{
	...
	public class MMFeedbacks : MonoBehaviour
	{
		...
		public bool ForcingUnscaledTimescaleMode =>
			ForceTimescaleMode && ForcedTimescaleMode == TimescaleModes.Unscaled;
...
#region PLAY
		...
		public virtual async Task PlayFeedbacksTask(
			Vector3 position, float feedbacksIntensity = 1.0f, bool forceChangeDirection = false)
		{
		...
		public virtual void PlayFeedbacksOnlyIfNormalDirection(
			Vector3 position, float feedbacksIntensity = 1.0f, bool forceChangeDirection = false)
		{
			if (Direction == Directions.TopToBottom) PlayFeedbacks(position, feedbacksIntensity, forceChangeDirection);
		}
		...
		/// <returns></returns>
		public virtual IEnumerator PlayFeedbacksCoroutine(
			Vector3 position, float feedbacksIntensity = 1.0f, bool forceChangeDirection = false)
		{
			PlayFeedbacks(position, feedbacksIntensity, forceChangeDirection);
...
#region SEQUENCE
		...
		/// <param name="position"></param>
		/// <param name="feedbacksIntensity"></param>
		protected virtual void PlayFeedbacksInternal(
			Vector3 position, float feedbacksIntensity, bool forceChangeDirection = false)
		{
			if (!CanPlay) return;
		...
		protected virtual void PlayAllFeedbacks(
			Vector3 position, float feedbacksIntensity, bool forceChangeDirection = false)
		{
		...
		protected virtual IEnumerator HandleInitialDelayCo(
			Vector3 position, float feedbacksIntensity, bool forceChangeDirection = false)
		{
			IsPlaying = true;
...
#region STOP
		...
		/// <param name="feedbacksIntensity"></param>
		public virtual void StopFeedbacks(
			Vector3 position, float feedbacksIntensity = 1.0f, bool stopAllFeedbacks = true)
		{
			if (stopAllFeedbacks)
				for (var i = 0; i < Feedbacks.Count; i++)
					if (Feedbacks[i] != null)
...
#region HELPERS
		...
		protected bool FeedbackCanPlay(MMFeedback feedback)
		{
			...
			if (feedback.Timing == null) return false;

			if (feedback.Timing.MMFeedbacksDirectionCondition ==
				MMFeedbackTiming.MMFeedbacksDirectionConditions.Always) return true;
		...
		{
			if (AutoChangeDirectionOnEnd) ShouldChangeDirectionOnNextPlay = true;
		}
		...
		{
			return duration * Mathf.Clamp(DurationMultiplier, _smallValue, float.MaxValue);
		}
...
#region EVENTS
		...
		{
			DurationMultiplier = Mathf.Clamp(DurationMultiplier, _smallValue, float.MaxValue);
		}
