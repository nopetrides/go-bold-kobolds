...
namespace MoreMountains.Feedbacks
{
	...
	public class MMFeedbacksEvents
	{
		...
		{
			if (!OnChangeDirectionIsNull && TriggerUnityEvents) OnChangeDirection.Invoke();

			if (TriggerMMFeedbacksEvents) MMFeedbacksEvent.Trigger(source, MMFeedbacksEvent.EventTypes.ChangeDirection);
		}
		...
		{
			if (!OnSkipToTheEndIsNull && TriggerUnityEvents) OnSkipToTheEnd.Invoke();

			if (TriggerMMFeedbacksEvents) MMFeedbacksEvent.Trigger(source, MMFeedbacksEvent.EventTypes.SkipToTheEnd);
		}

		public virtual void TriggerOnInitializationComplete(MMFeedbacks source)
		{
			if (!OnInitializationCompleteIsNull && TriggerUnityEvents) OnInitializationComplete.Invoke();
		...
		{
			if (!OnRestoreInitialValuesIsNull && TriggerUnityEvents) OnRestoreInitialValues.Invoke();
		...
		{
			if (!OnEnableIsNull && TriggerUnityEvents) OnEnable.Invoke();

			if (TriggerMMFeedbacksEvents) MMFeedbacksEvent.Trigger(source, MMFeedbacksEvent.EventTypes.Enable);
		}
		...
		{
			if (!OnDisableIsNull && TriggerUnityEvents) OnDisable.Invoke();

			if (TriggerMMFeedbacksEvents) MMFeedbacksEvent.Trigger(source, MMFeedbacksEvent.EventTypes.Disable);
		}
		...
		{
			if (!OnDisableIsNull && TriggerUnityEvents) OnStop.Invoke();

			if (TriggerMMFeedbacksEvents) MMFeedbacksEvent.Trigger(source, MMFeedbacksEvent.EventTypes.Stop);
		}
	}
}
