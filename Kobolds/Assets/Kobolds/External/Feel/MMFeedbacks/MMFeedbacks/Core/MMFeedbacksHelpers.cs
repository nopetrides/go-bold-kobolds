...
namespace MoreMountains.Feedbacks
{
...
#if UNITY_EDITOR
				Undo.RecordObject(owner, "Ports animation curve to tween system");
#endif
			}
		}
	}
	...
	public class MMFInspectorButtonPropertyDrawer : PropertyDrawer
	{
		...
		public override void OnGUI(Rect position, SerializedProperty prop, GUIContent label)
		{
			...
			var buttonRect = new Rect(
				position.x + (position.width - buttonLength) * 0.5f, position.y, buttonLength, position.height);
			...
			if (GUI.Button(buttonRect, inspectorButtonAttribute.MethodName))
			{
				...
				if (_eventMethodInfo != null)
					...
					Debug.LogWarning(
						string.Format("InspectorButton: Unable to find method {0} in {1}", eventName, eventOwnerType));
			}
		}
	}
#endif
	...
	public class MMVectorLabelsAttributeDrawer : PropertyDrawer
	{
		...
		{
			var vector = (MMFVectorAttribute) attribute;

			if (property.propertyType == SerializedPropertyType.Vector2)
			{
				var fieldArray = new[] {property.vector2Value.x, property.vector2Value.y};
				fieldArray = DrawFields(
					rect, fieldArray, ObjectNames.NicifyVariableName(property.name), EditorGUI.FloatField, vector,
					guiContent);
			...
			else if (property.propertyType == SerializedPropertyType.Vector2Int)
			{
				...
				fieldArray = DrawFields(
					rect, fieldArray, ObjectNames.NicifyVariableName(property.name), EditorGUI.IntField, vector,
					guiContent);
				property.vector2IntValue = new Vector2Int(fieldArray[0], fieldArray[1]);
			}
			else if (property.propertyType == SerializedPropertyType.Vector3Int)
			{
				var array = new[] {property.vector3IntValue.x, property.vector3IntValue.y, property.vector3IntValue.z};
				array = DrawFields(
					rect, array, ObjectNames.NicifyVariableName(property.name), EditorGUI.IntField, vector, guiContent);
				property.vector3IntValue = new Vector3Int(array[0], array[1], array[2]);
			}
		}

		protected T[] DrawFields<T>(
			Rect rect, T[] vector, string mainLabel, Func<Rect, GUIContent, T, T> fieldDrawer,
			MMFVectorAttribute vectors, GUIContent originalGuiContent)
		{
			var result = vector;
	...
	public class MMFInspectorGroupAttribute : PropertyAttribute
	{
		...
		public MMFInspectorGroupAttribute(
			string groupName, bool groupAllFieldsUntilNextGroupAttribute = false, int groupColorIndex = 24,
			bool requiresSetup = false, bool closedByDefault = false)
		{
	...
	public static class MMFeedbackStaticMethods
	{
		...
		public static object MMFGetObjectValue(this SerializedProperty property)
		{
			...
			foreach (var element in elements)
				...
				else
				{
					...
					var elementIndex = Convert.ToInt32(
		...
		private static object MMFGetPropertyValue(object source, string propertyName)
		{
			...
			{
				var fieldInfo = propertyType.GetField(
	...
	public static class MMF_FieldInfo
	{
		...
		public static int GetFieldInfo(MMF_Feedback target, out List<FieldInfo> fieldInfoList)
		{
			...
			if (!FieldInfoList.TryGetValue(targetTypeHashCode, out fieldInfoList))
			{
				...
				fieldInfoList = target.GetType().GetFields(
						BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic |
