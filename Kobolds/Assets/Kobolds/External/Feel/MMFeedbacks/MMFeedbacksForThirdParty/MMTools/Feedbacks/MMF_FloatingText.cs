	...
	public class MMF_FloatingText : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			MMFloatingTextSpawnEvent.Trigger(
				ChannelData, _playPosition, _value, Direction, Intensity * intensityMultiplier, ForceLifetime, Lifetime,
				ForceColor, AnimateColorGradient,
				ComputedTimescaleMode == TimescaleModes.Unscaled, AttachmentTransform);
		}

		protected virtual float ApplyRounding(float value)
		{
			if (RoundingMethod == RoundingMethods.NoRounding) return value;
