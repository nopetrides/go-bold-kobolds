...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Fog : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			{
				case Modes.Instant:
					if (ModifyColor) RenderSettings.fogColor = NormalPlayDirection ? InstantColor : _initialColor;

					if (ModifyStartDistance)
						RenderSettings.fogStartDistance = NormalPlayDirection ?
							StartDistanceInstantChange :
							_initialInstantStartDistance;

					if (ModifyEndDistance)
						RenderSettings.fogEndDistance =
							NormalPlayDirection ? EndDistanceInstantChange : _initialInstantEndDistance;

					if (ModifyFogDensity)
						RenderSettings.fogDensity = NormalPlayDirection ?
							DensityInstantChange * intensityMultiplier :
		...
		protected virtual void SetFogValues(float time, float intensityMultiplier)
		{
			...
			if (ModifyStartDistance)
				RenderSettings.fogStartDistance = MMTween.Tween(
					time, 0f, 1f, StartDistanceRemapZero, StartDistanceRemapOne, StartDistanceCurve);

			if (ModifyEndDistance)
				RenderSettings.fogEndDistance = MMTween.Tween(
					time, 0f, 1f, EndDistanceRemapZero, EndDistanceRemapOne, EndDistanceCurve);
		}
