...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Property : MMF_Feedback
	{
		...
		public MMTweenType LevelCurve = new(
			new AnimationCurve(new Keyframe(0, 0), new Keyframe(0.3f, 1f), new Keyframe(1, 0)), "", "Mode",
			(int) Modes.OverTime, (int) Modes.ToDestination);
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			{
				Debug.LogWarning(
					"[Property Feedback] The property feedback on " + Owner.name +
					" doesn't have a Target, it won't work. You need to specify one in its inspector.");
				return;
			}
		...
		{
			if (!Active || !FeedbackTypeAuthorized || Target == null) return;

			if (DetermineInitialValueOnPlay) GetInitialIntensity();
			...
			{
				case Modes.Instant:
					var newLevel = NormalPlayDirection ? InstantLevel : _initialIntensity;
		...
		protected virtual IEnumerator ToDestinationSequence(float intensityMultiplier)
		{
			...
			var initialValue = Target.GetLevel();
			var destinationValue = ToDestinationLevel;

			if (RelativeValues) destinationValue += _initialIntensity;
		...
		protected virtual void SetValues(
			float time, float intensityMultiplier, float remapZero, float remapOne, bool applyRelative)
		{
			var intensity = MMTween.Tween(time, 0f, 1f, remapZero, remapOne, LevelCurve);

			intensity *= intensityMultiplier;

			if (applyRelative && RelativeValues) intensity += _initialIntensity;
