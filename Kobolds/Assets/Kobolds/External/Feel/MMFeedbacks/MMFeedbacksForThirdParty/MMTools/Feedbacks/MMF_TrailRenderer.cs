	...
	public class MMF_TrailRenderer : MMF_Feedback
	{
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			if (Active)
			{
				...
				{
					Debug.LogWarning(
						"[Trail Renderer Feedback] The trail renderer feedback on " + Owner.name +
						" doesn't have a TargetTrailRenderer, it won't work. You need to specify one in its inspector.");
					return;
				}
		...
		{
			if (!Active || !FeedbackTypeAuthorized || TargetTrailRenderer == null) return;
			...
			{
				case Modes.Instant:
					if (ModifyColor) TargetTrailRenderer.colorGradient = NormalPlayDirection ? NewColor : _firstColor;
					if (ModifyWidth) TargetTrailRenderer.widthCurve = NormalPlayDirection ? NewWidth : _firstWidth;
					if (ModifyTime) TargetTrailRenderer.time = NormalPlayDirection ? NewTime : _firstTime;
					break;
				case Modes.OverTime:
		...
		protected virtual void SetTrailRendererValues(float time, float intensityMultiplier)
		{
			...
			{
				if (NormalPlayDirection)
					TargetTrailRenderer.widthCurve =
		...
		{
			return TargetTrailRenderer == null;
		}
		...
		public override string RequiresSetupText =>
			"This feedback requires that a TargetTrailRenderer be set to be able to work properly. You can set one below.";
#endif
	}
}
