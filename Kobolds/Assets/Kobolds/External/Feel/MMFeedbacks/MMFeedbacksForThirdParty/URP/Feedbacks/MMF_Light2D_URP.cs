...
namespace MoreMountains.FeedbacksForThirdParty
{
#if MM_URP
	/// <summary>
	///     This feedback will let you control a 2D light's intensity, color, falloff, shadow strength and volumetric intensity
	...
	public class MMF_Light2D_URP : MMF_Feedback
	{
		...
		[MMFEnumCondition("Mode", (int) Modes.OverTime, (int) Modes.ShakerEvent, (int) Modes.ToDestination)]
		public AnimationCurve VolumetricIntensityCurve = new(
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			if (ColorOverTime == null) ColorOverTime = new Gradient();
		...
		{
			if (!Active || !FeedbackTypeAuthorized || BoundLight == null) return;
			...
			switch (Mode)
			{
				...
				case Modes.ShakerEvent:
					if (EventOriginTransform == null) EventOriginTransform = Owner.transform;
					MMLight2DShakeEvent.Trigger(
						FeedbackDuration, RelativeValues, ModifyColor, ColorOverTime, IntensityCurve,
		...
		protected virtual void SetLightValues(float time, float intensityMultiplier)
		{
			...
			{
				case Modes.OverTime:
					intensity = MMFeedbacksHelpers.Remap(
						IntensityCurve.Evaluate(time), 0f, 1f, RemapIntensityZero, RemapIntensityOne);
					volumeIntensity = MMFeedbacksHelpers.Remap(
						VolumetricIntensityCurve.Evaluate(time), 0f, 1f, RemapVolumetricIntensityZero,
						RemapVolumetricIntensityOne);
					falloff = MMFeedbacksHelpers.Remap(
						FalloffCurve.Evaluate(time), 0f, 1f, RemapFalloffZero, RemapFalloffOne);
					shadowStrength = MMFeedbacksHelpers.Remap(
						ShadowStrengthCurve.Evaluate(time), 0f, 1f, RemapShadowStrengthZero, RemapShadowStrengthOne);
				...
				case Modes.ToDestination:
					...
					volumeIntensity = Mathf.Lerp(
						_initialVolumetricIntensity, ToDestinationVolumetricIntensity,
					...
					shadowStrength = Mathf.Lerp(
						_initialShadowStrength, ToDestinationShadowStrength, ShadowStrengthCurve.Evaluate(time));
			...
			if (ModifyIntensity) BoundLight.intensity = intensity * intensityMultiplier;
			if (ModifyVolumetricIntensity) BoundLight.volumeIntensity = volumeIntensity * intensityMultiplier;
			if (ModifyFalloff) BoundLight.shapeLightFalloffSize = falloff;
			if (ModifyShadowStrength) BoundLight.shadowIntensity = Mathf.Clamp01(shadowStrength);
			if (ModifyColor) BoundLight.color = _targetColor;
		}
