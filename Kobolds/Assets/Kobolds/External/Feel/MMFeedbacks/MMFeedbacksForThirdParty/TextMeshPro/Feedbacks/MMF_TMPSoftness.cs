...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_TMPSoftness : MMF_Feedback
	{
		...
		public MMTweenType SoftnessCurve = new(
			new AnimationCurve(new Keyframe(0, 0f), new Keyframe(0.3f, 1f), new Keyframe(1, 0f)), "", "Mode",
			(int) MMFeedbackBase.Modes.OverTime);
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			{
				Debug.LogWarning(
					"[TMP Softness Feedback] The TMP Softness feedback on " + Owner.name +
					" doesn't have a TargetTMPText, it won't work. You need to specify one in its inspector.");
				return;
			}
			...
			if (Active && FeedbackTypeAuthorized)
				switch (Mode)
				{
					case MMFeedbackBase.Modes.Instant:
						var newSoftness = NormalPlayDirection ? InstantSoftness : _initialSoftness;
