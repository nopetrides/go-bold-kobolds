...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_TMPTextReveal : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1.0f)
		{
			...
			if (DurationMode == DurationModes.TotalDuration) FeedbackDuration = RevealDuration;
		...
		public override float FeedbackDuration
		{
			get
			{
				...
				if (TargetTMPText == null) return 0f;

				if (TargetTMPText.textInfo == null)
				{
					var initiallyActive = TargetTMPText.gameObject.activeSelf;
				...
				{
					var disabledParents = TargetTMPText.transform.MMEnumerateAllParents(true)
				...
				if (TargetTMPText.textInfo == null) return 0f;
				...
				if (ReplaceText) TargetTMPText.text = _originalText;
		...
		protected virtual IEnumerator RevealCharacters()
		{
			...
			while (visibleCharacters <= _totalCharacters && !Owner.SkippingToTheEnd)
			{
				...
				var timeSinceLastChar = time - lastCharAt;
				var numberOfIntervals = (int) Mathf.Round(timeSinceLastChar / IntervalBetweenReveals);
				for (var i = 0; i < numberOfIntervals; i++) visibleCharacters++;
				lastCharAt = time;
				...
				{
					var remainingCharacters = _totalCharacters - visibleCharacters;
