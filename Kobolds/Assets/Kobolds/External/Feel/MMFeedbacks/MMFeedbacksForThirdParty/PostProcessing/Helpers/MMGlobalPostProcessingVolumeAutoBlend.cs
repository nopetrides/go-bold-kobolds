...
namespace MoreMountains.FeedbacksForThirdParty
{
	...
	public class MMGlobalPostProcessingVolumeAutoBlend : MonoBehaviour,
		MMEventListener<MMPostProcessingVolumeAutoBlendShakeEvent>
	{
		...
		public void OnMMEvent(MMPostProcessingVolumeAutoBlendShakeEvent shakeEvent)
		{
			...
			{
				if (!shakeEvent.TargetAutoBlend.Equals(this)) return;
			}
			else
			{
				var eventMatch = shakeEvent.ChannelData != null && MMChannel.Match(
					shakeEvent.ChannelData, ChannelMode, Channel, MMChannelDefinition);
			...
			if (shakeEvent.Mode == MMF_GlobalPPVolumeAutoBlend.Modes.Default)
			{
				if (!shakeEvent.NormalPlayDirection)
				{
					...
					if (shakeEvent.BlendAction == MMF_GlobalPPVolumeAutoBlend.Actions.BlendBack) Blend();
				}
				else
				{
					if (shakeEvent.BlendAction == MMF_GlobalPPVolumeAutoBlend.Actions.Blend)
					...
					if (shakeEvent.BlendAction == MMF_GlobalPPVolumeAutoBlend.Actions.BlendBack) BlendBack();
				}
			}
			else
			{
