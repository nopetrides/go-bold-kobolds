...
namespace MoreMountains.FeedbacksForThirdParty
{
	public class MMPostProcessingHelpers : MonoBehaviour
	{
			...
			where T : PostProcessEffectSettings where U : MMShaker
		{
			...
			var postProcessLayer = FindAnyObjectByType<PostProcessLayer>();
			...
			var volume = (PostProcessVolume) FindAnyObjectByType(typeof(PostProcessVolume));
			if (volume == null)
			{
				var postProcessingObject = Instantiate(Resources.Load<GameObject>("MMDefaultPostProcessingVolume"));
			...
			var shaker = volume.GetComponent<U>();
			if (shaker == null)
			{
