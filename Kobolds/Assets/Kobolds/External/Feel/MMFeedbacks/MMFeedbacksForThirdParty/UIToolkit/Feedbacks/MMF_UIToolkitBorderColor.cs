...
namespace MoreMountains.FeedbacksForThirdParty
{
	...
	public class MMF_UIToolkitBorderColor : MMF_UIToolkitColorBase
	{
		...
		protected override void ApplyColor(Color newColor)
		{
			...
			{
				if (BorderLeft) element.style.borderLeftColor = newColor;
				if (BorderRight) element.style.borderRightColor = newColor;
				if (BorderBottom) element.style.borderBottomColor = newColor;
				if (BorderTop) element.style.borderTopColor = newColor;
		...
		{
			if (BorderLeft) return _visualElements[0].resolvedStyle.borderLeftColor;
			if (BorderRight) return _visualElements[0].resolvedStyle.borderRightColor;
			if (BorderBottom) return _visualElements[0].resolvedStyle.borderBottomColor;
			if (BorderTop) return _visualElements[0].resolvedStyle.borderTopColor;
			return Color.black;
		}
	}
}
