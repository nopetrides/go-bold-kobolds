...
namespace MoreMountains.FeedbacksForThirdParty
{
	...
	public class MMCinemachineZoom : MonoBehaviour
	{
		...
		protected virtual void Update()
		{
			...
			{
				var t = MMMaths.Remap(_elapsedTime, 0f, _transitionDuration, 0f, 1f);
#if MM_CINEMACHINE
				_virtualCamera.m_Lens.FieldOfView =
 Mathf.LerpUnclamped(_startFieldOfView, _targetFieldOfView, ZoomTween.Evaluate(t));
#elif MM_CINEMACHINE3
				_virtualCamera.Lens.FieldOfView = Mathf.LerpUnclamped(
					_startFieldOfView, _targetFieldOfView, ZoomTween.Evaluate(t));
#endif
			}
			else
			{
		...
		public virtual void Zoom(
			MMCameraZoomModes mode, float newFieldOfView, float transitionDuration, float duration,
			bool useUnscaledTime, bool relative = false, MMTweenType tweenType = null)
		{
			if (enabled && !Interruptable) return;
			...
			if (tweenType != null) ZoomTween = tweenType;
		...
		public virtual void OnCameraZoomEvent(
			MMCameraZoomModes mode, float newFieldOfView, float transitionDuration, float duration,
