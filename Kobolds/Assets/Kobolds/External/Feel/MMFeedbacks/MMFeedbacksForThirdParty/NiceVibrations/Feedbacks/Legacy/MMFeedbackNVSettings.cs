...
namespace MoreMountains.FeedbacksForThirdParty
{
	...
	public class MMFeedbackNVSettings
	{
		...
		{
			if (ForceGamepadID) GamepadRumbler.SetCurrentGamepad(GamepadID);
		}
		...
		public virtual bool CanPlay()
		{
			...
			if (OnlyPlayIfAdvancedRequirementsMet && !DeviceCapabilities.meetsAdvancedRequirements) return false;

			if (OnlyPlayIfAmplitudeModulationSupported && !DeviceCapabilities.hasAmplitudeModulation) return false;

			if (OnlyPlayIfFrequencyModulationSupported && !DeviceCapabilities.hasFrequencyModulation) return false;
