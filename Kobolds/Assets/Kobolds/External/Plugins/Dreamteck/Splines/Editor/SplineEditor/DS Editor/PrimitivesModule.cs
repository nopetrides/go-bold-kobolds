	...
	public class PrimitivesModule : PointTransformModule
	{
		...
		private void ApplyDialog()
		{
			...
			if (EditorUtility.DisplayDialog(
					"Unapplied Primitives", "There is an unapplied primitive. Do you want to apply the changes?",
					"Apply", "Revert"))
				Apply();
			else
				Revert();
		...
		private void PresetsGUI()
		{
			...
			if (selectedPreset >= 0 && selectedPreset < presets.Length)
			{
				...
				if (GUILayout.Button("Delete", GUILayout.MaxWidth(80)))
					if (EditorUtility.DisplayDialog(
							"Delete Preset", "This will permanently delete the preset file. Continue?", "Yes", "No"))
					{
