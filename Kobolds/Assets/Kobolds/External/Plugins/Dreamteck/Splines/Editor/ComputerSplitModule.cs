...
namespace Dreamteck.Splines.Editor
{
	public class ComputerSplitModule : ComputerEditorModule
	{
		...
		protected override void OnDrawScene()
		{
			...
			for (var i = 0; i < spline.pointCount; i++)
			{
				...
				if (SplineEditorHandles.CircleButton(
						pos, Quaternion.LookRotation(editorCamera.transform.position - pos),
						HandleUtility.GetHandleSize(pos) * 0.12f, 1f, spline.editorPathColor))
				{
					SplitAtPoint(i);
					change = true;
					break;
			...
			if (!change)
			{
				...
				if (pointValue - pointIndex > spline.moveStep)
					if (SplineEditorHandles.CircleButton(
							...
							HandleUtility.GetHandleSize(projected.position) * 0.12f, 1f, spline.editorPathColor))
					{
		...
		private void SplitAtPercent(double percent)
		{
			...
			var splitPoint = SplinePoint.Lerp(
				spline.GetPoint(lastPointIndex), spline.GetPoint(nextPointIndex), lerpPercent);
