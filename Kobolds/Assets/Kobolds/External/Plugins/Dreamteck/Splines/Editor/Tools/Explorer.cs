...
namespace Dreamteck.Splines.Editor
{
	public class Explorer : SplineTool
	{
		...
		private List<SplineComputer> sceneSplines = new();
		...
		public override void Draw(Rect rect)
		{
			...
			scroll = EditorGUILayout.BeginScrollView(
				scroll, GUILayout.Width(rect.width), GUILayout.Height(rect.height));
			...
			for (var i = 0; i < sceneSplines.Count; i++)
			{
				...
				EditorGUILayout.LabelField(
					sceneSplines[i].name, isSelected ? selectedRow : normalRow, GUILayout.Width(rect.width - 200));
				...
				if (thickness != sceneSplines[i].editorDrawThickness)
					foreach (var index in selected)
						sceneSplines[index].editorDrawThickness = thickness;
				EditorGUILayout.EndHorizontal();
