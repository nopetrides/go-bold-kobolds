...
namespace Dreamteck.Splines.Editor
{
	...
	public class NodeEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
					...
					if (addPoint >= 0 && availablePoints.Length > addPoint)
					{
						if (node.HasConnection(addComp, availablePoints[addPoint]))
							EditorGUILayout.HelpBox(
								"Connection already exists (" + addComp.name + "," + availablePoints[addPoint],
								MessageType.Error);
			...
			{
				EditorGUILayout.HelpBox(
					"Connection UI not available when multiple Nodes are selected.", MessageType.Info);
			}
		...
		private void SettingsGUI()
		{
			...
			if (GUILayout.Button("Align Tangents X")) AlignTangents(node, 0);

			if (GUILayout.Button("Align Tangents Y")) AlignTangents(node, 1);

			if (GUILayout.Button("Align Tangents Z")) AlignTangents(node, 2);
		...
		{
			var axisVector = Vector3.forward;
			switch (axis)
			{
			...
			var point = node.GetPoint(0, false);
			var tan1 = point.tangent - point.position;
			var tan2 = point.tangent2 - point.position;
			var tan1Dir = Mathf.Sign(Vector3.Dot(tan1, axisVector));
			var tan2Dir = Mathf.Sign(Vector3.Dot(tan2, axisVector));
			...
			if (connections.Length > 0)
				for (var i = 0; i < connections.Length; i++)
				{
					...
					EditorGUILayout.LabelField(
						connections[i].spline.name + " at point " + (connections[i].pointIndex + 1));
					...
					if (SplineEditorGUI.EditorLayoutSelectableButton(
							new GUIContent("Swap Tangents"), connections[i].spline.type == Spline.Type.Bezier,
		...
		private void AddConnection(SplineComputer computer, int pointIndex)
		{
			...
			if (EditorUtility.DisplayDialog(
					"Link point?", "Add point " + (pointIndex + 1) + " to connections?", "Yes", "No"))
			{
		...
		protected virtual void DuringSceneGUI(SceneView current)
		{
			...
			for (var i = 0; i < availablePoints.Length; i++)
			{
				...
				if (SplineEditorHandles.CircleButton(
						points[availablePoints[i]].position,
						Quaternion.LookRotation(-camTransform.forward, camTransform.up),
						HandleUtility.GetHandleSize(points[availablePoints[i]].position) * 0.1f, 2f,
						addComp.editorPathColor))
				{
		...
		public static void SetDirty(Node node)
		{
			...
			var connections = node.GetConnections();
			for (var i = 0; i < connections.Length; i++) EditorUtility.SetDirty(connections[i].spline);
		}
	}
}
