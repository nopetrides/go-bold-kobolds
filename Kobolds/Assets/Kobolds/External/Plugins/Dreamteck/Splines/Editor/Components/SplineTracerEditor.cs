	...
	public class SplineTracerEditor : SplineUserEditor
	{
		...
		private void CreateRT()
		{
			...
			rt = new RenderTexture(
				GetRTWidth(), GetRTHeight(), 16, RenderTextureFormat.Default, RenderTextureReadWrite.Default);
		...
		protected override void BodyGUI()
		{
			...
			var dontLerpDirection = serializedObject.FindProperty("_dontLerpDirection");
			...
			{
				var mightBe2d = false;
				if (tracers[0].spline != null) mightBe2d = tracers[0].spline.is2D;
				if (!mightBe2d) mightBe2d = physicsMode.intValue == (int) SplineTracer.PhysicsMode.Rigidbody2D;
				if (!mightBe2d)
				...
				if (cameraFoldout)
				{
					...
					{
						EditorGUILayout.HelpBox(
							"There is no camera attached to the selected object or its children.", MessageType.Info);
					}
				}
			}
		...
		protected void DrawResult(SplineSample result)
		{
			...
			Handles.DrawLine(
				result.position, result.position + result.up * HandleUtility.GetHandleSize(result.position) * 0.5f);
			Handles.color = Color.red;
			Handles.DrawLine(
				result.position, result.position + result.right * HandleUtility.GetHandleSize(result.position) * 0.5f);
			Handles.color = Color.white;
		}
	}
}
