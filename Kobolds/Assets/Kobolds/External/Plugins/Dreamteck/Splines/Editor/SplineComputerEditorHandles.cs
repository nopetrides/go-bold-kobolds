...
namespace Dreamteck.Splines.Editor
{
	public static class SplineComputerEditorHandles
	{
		...
		private static SplineSample evalResult;

		public static bool Slider(
			SplineComputer spline, ref double percent, Color color, string text = "",
			SplineSliderGizmo gizmo = SplineSliderGizmo.Rectangle, float buttonSize = 1f)
		{
			...
			Handles.DrawSolidDisc(
				evalResult.position, cam.transform.position - evalResult.position, size * 0.2f * buttonSize);
			...
			if (text != "" && localPos.z > 0f)
			{
				...
				DreamteckEditorGUI.Label(
					new Rect(screenPosition.x - 120 + text.Length * 4, screenPosition.y, 120, 25), text);
				Handles.EndGUI();
			}
			...
			evalResult.position = SplineEditorHandles.FreeMoveHandle(
				evalResult.position, size * 0.2f * buttonSize, Vector3.zero, Handles.CircleHandleCap);
