...
namespace Dreamteck.Splines.Editor
{
	...
	{
		private static bool __holdInteraction;
		private Color _bgColor = Color.black;
		private bool _drag;
		private Rect _dragRect;
		private bool _finalizeDrag;
		private bool _pointsMoved;
		...
		public bool isDragging => _drag && _dragRect.width >= minimumRectSize && _dragRect.height >= minimumRectSize;
		...
		protected override void OnDrawScene()
		{
			...
			else
			{
				...
				else
				{
					...
					_dragRect = new Rect(
						Mathf.Min(_rectStart.x, _rectEnd.x), Mathf.Min(_rectStart.y, _rectEnd.y),
						Mathf.Abs(_rectEnd.x - _rectStart.x), Mathf.Abs(_rectEnd.y - _rectStart.y));
			...
			if (selectedPoints.Count > 1) tangentMode = false;
			...
			for (var i = 0; i < points.Length; i++)
			{
				...
				if (splineType == Spline.Type.Bezier && isSelected)
				{
					...
					if (tangentMode && selectedPoints.Count == 1) Handles.color = highlightColor;

					var lastTangentPos = points[i].tangent;

					var newPos = SplineEditorHandles.FreeMoveCircle(
						points[i].tangent, HandleUtility.GetHandleSize(points[i].tangent) * 0.22f);
					...
					newPos = SplineEditorHandles.FreeMoveCircle(
						points[i].tangent2, HandleUtility.GetHandleSize(points[i].tangent2) * 0.22f);
				...
				if (!eventModule.alt && !__holdInteraction)
				{
					if (excludeSelected && isSelected)
						SplineEditorHandles.FreeMoveRectangle(
							points[i].position, HandleUtility.GetHandleSize(points[i].position) * 0.1f);
					else
				...
				if (!__holdInteraction && lastPos != points[i].position)
				{
					...
					if (isSelected)
						for (var n = 0; n < selectedPoints.Count; n++)
						{
							...
							points[selectedPoints[n]].SetPosition(
								points[selectedPoints[n]].position + (points[i].position - lastPos));
						}
					else
						SelectPoint(i);
				...
				if (!excludeSelected || !isSelected)
					if (Event.current.type == EventType.Repaint)
						SplineEditorHandles.DrawPoint(
							points[i].position, isSelected && (!tangentMode || selectedPoints.Count != 1));
			}
			...
			if (isDragging && Event.current.type == EventType.MouseDrag)
			{
				...
				var viewportSize = new Vector2(_currentSceneView.position.width, _currentSceneView.position.height);
...
#endif
				if (eventModule.alt || mouseIsOutside || !eventModule.mouseLeft) FinishDrag();
			}

			if (eventModule.mouseLeftUp) _pointsMoved = false;
