...
namespace Dreamteck.Splines.Editor
{
	...
	public class SplineComputerEditor : UnityEditor.Editor
	{
		...
		private readonly List<int> _selectedPoints = new();
		...
		private void OnEnable()
		{
			...
			for (var i = 0; i < splines.Length; i++)
			{
				...
				if (splines[i].editorAlwaysDraw) DSSplineDrawer.RegisterComputer(splines[i]);
			}
		...
		public override void OnInspectorGUI()
		{
			...
			{
				if (Event.current.type == EventType.MouseUp && Event.current.button == 0) _rebuildSpline = true;
			...
			if (splines.Length == 1)
			{
				...
				if (_pathEditor.open)
				{
					var lastSpace = _pathEditor.editSpace;
			...
			if (_computerEditor.open) _computerEditor.DrawInspector();
			SplineEditorGUI.EndContainerBox();
