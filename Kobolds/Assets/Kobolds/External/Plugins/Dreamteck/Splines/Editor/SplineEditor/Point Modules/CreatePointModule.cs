...
namespace Dreamteck.Splines.Editor
{
	public class CreatePointModule : PointModule
	{
		...
		protected override void OnDrawScene()
		{
			...
			{
				GetCreatePointOnPlane(
					-editorCamera.transform.forward,
					editorCamera.transform.position + editorCamera.transform.forward * offset, out createPoint);
			...
			if (placementMode == PlacementMode.Surface)
			{
				...
				if (Physics.Raycast(ray, out hit, Mathf.Infinity, surfaceLayerMask))
				{
					...
					SplineEditorHandles.DrawRectangle(
						createPoint,
						Quaternion.LookRotation(-editorCamera.transform.forward, editorCamera.transform.up),
		...
		protected void AddPoint()
		{
			...
			var newPoints = editor.GetPointsArray();
		...
		private void UpdateVisualizer()
		{
			...
			if (appendMode == AppendMode.End)
			{
				for (var i = 0; i < points.Length; i++) visualizer.points[i] = points[i].CreateSplinePoint();
				visualizer.points[visualizer.points.Length - 1] = newPoint;
			}
			else
			{
				for (var i = 1; i < visualizer.points.Length; i++)
