...
namespace Dreamteck.Splines.Editor
{
	...
	public class ObjectControllerEditor : SplineUserEditor
	{
		protected override void BodyGUI()
		{
			...
			var spawnMethod = serializedObject.FindProperty("_spawnMethod");
			...
			var useCustomObjectDistance = serializedObject.FindProperty("_useCustomObjectDistance");
			var minObjectDistance = serializedObject.FindProperty("_minObjectDistance");
			var maxObjectDistance = serializedObject.FindProperty("_maxObjectDistance");
			var customOffsetRule = serializedObject.FindProperty("_customOffsetRule");
			var customRotationRule = serializedObject.FindProperty("_customRotationRule");
			var customScaleRule = serializedObject.FindProperty("_customScaleRule");
			...
			if (objectMethod.intValue == (int) ObjectController.ObjectMethod.Instantiate)
			{
				...
				else
				{
					...
					for (var i = 0; i < user.objects.Length; i++)
					{
						...
						user.objects[i] = (GameObject) EditorGUILayout.ObjectField(
							user.objects[i], typeof(GameObject), true);
				...
				var lastSpawnMethod = spawnMethod.intValue;
				EditorGUILayout.PropertyField(spawnMethod, new GUIContent("Spawn Method"));
				if (lastSpawnMethod != spawnMethod.intValue) objectsChanged = true;
		...
		private void CustomRuleUI(ObjectControllerCustomRuleBase customRule)
		{
			...
			var property = serializedRule.GetIterator();
			...
			if (EditorGUI.EndChangeCheck()) serializedRule.ApplyModifiedProperties();
		}
	}
}
