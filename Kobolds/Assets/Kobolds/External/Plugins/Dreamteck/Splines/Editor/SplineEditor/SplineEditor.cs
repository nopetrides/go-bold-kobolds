...
namespace Dreamteck.Splines.Editor
{
	public class SplineEditor : SplineEditorBase
	{
		...
		private readonly string _customSplinePropertyName = "";
		private float _editLabelAlpha;
		...
		private int _selectedPointOperation;
		...
		public SplineEditor(
			Matrix4x4 transformMatrix, SerializedObject splineHolder, string customSplinePropertyName) : base(
			splineHolder)
		{
		...
		private void Initialize(Matrix4x4 transformMatrix, SerializedObject splineHolder)
		{
			...
			pointOperations.Add(
				new PointOperation {name = "Distribute Evenly", action = delegate { DistributeEvenly(); }});
			pointOperations.Add(
				new PointOperation {name = "Auto Bezier Tangents", action = delegate { AutoTangents(); }});
			pointOperations.Add(
				new PointOperation {name = "Swap Bezier Tangents", action = delegate { SwapTangents(); }});
			pointOperations.Add(
				new PointOperation {name = "Flip Bezier Tangents", action = delegate { FlipTangents(); }});
			pointOperations.Add(
				new PointOperation {name = "Flip First Bezier Tangent", action = delegate { FlipFirstTangent(); }});
			pointOperations.Add(
				new PointOperation {name = "Flip Seconds Bezier Tangent", action = delegate { FlipSecondTangent(); }});

			_pointOperationStrings = new string[pointOperations.Count];
			for (var i = 0; i < pointOperations.Count; i++) _pointOperationStrings[i] = pointOperations[i].name;
		...
		public SplinePoint[] GetPointsArray()
		{
			...
			for (var i = 0; i < p.Length; i++) p[i] = points[i].CreateSplinePoint();
			return p;
		}
		...
		public void SetPointsArray(SplinePoint[] input)
		{
			...
			for (var i = 0; i < points.Length; i++) points[i].SetPoint(input[i]);
		}
		...
		public override void DrawInspector()
		{
			...
			if (editMode)
			{
				...
				if (currentModule != null)
				{
					...
					if (currentModule.hasChanged) ApplyModifiedProperties();
				}
		...
		private void DrawToolMenu()
		{
			...
			if (EditorGUI.EndChangeCheck()) ToggleModule(_selectModule);
		...
		protected virtual void PointPanel()
		{
			...
			if (mainModule.hasChanged) ApplyModifiedProperties();
			if (selectedPoints.Count > 0 && points.Length > 0) PointMenu();
		}

		public virtual void BeforeSceneGUI(SceneView current)
		{
			mainModule.BeforeSceneDraw(current);
			if (_module >= 0 && _module < _modules.Length) _modules[_module].BeforeSceneDraw(current);
		}
		...
		public void ToggleModule(int index)
		{
			...
			else
			{
				...
				if (currentModule.hasChanged) ApplyModifiedProperties(true);
			}
		...
		protected virtual void PointMenu()
		{
			...
			if (!is2D) avgPoint.normal = TransformedVectorField("Normal", avgPoint.normal);
			...
			if (isBezier) avgPoint.type = (SplinePoint.Type) EditorGUILayout.EnumPopup("Point Type", avgPoint.type);

			if (!EditorGUI.EndChangeCheck()) return;
			for (var i = 0; i < selectedPoints.Count; i++)
			{
				points[selectedPoints[i]].SetPosition(
					GetChangedVector(avgPos, avgPoint.position, points[selectedPoints[i]].position));
				points[selectedPoints[i]].normal = GetChangedVector(
					avgNormal, avgPoint.normal, points[selectedPoints[i]].normal);

				if (isBezier)
				{
					points[selectedPoints[i]].SetTangentPosition(
						GetChangedVector(avgTan, avgPoint.tangent, points[selectedPoints[i]].tangent));
					points[selectedPoints[i]].SetTangent2Position(
						GetChangedVector(avgTan2, avgPoint.tangent2, points[selectedPoints[i]].tangent2));
				}
		...
		public void FlatSelection(int axis)
		{
			...
			if (_typeProperty.enumValueIndex == (int) Spline.Type.Bezier)
				switch (EditorUtility.DisplayDialogComplex(
							"Flat Bezier", "How do you want to flat the selected Bezier points?", "Points Only",
							"Tangens Only", "Everything"))
				{
					case 0:
						flatTangent = false;
						flatPosition = true;
		...
		public void MirrorSelection(int axis)
		{
			...
			if (_typeProperty.enumValueIndex == (int) Spline.Type.Bezier)
				if (EditorUtility.DisplayDialog(
						"Mirror tangents", "Do you want to mirror the tangents too ?", "Yes", "No"))
					mirrorTangents = true;
			float min = 0f, max = 0f;
			switch (axis)
			{
