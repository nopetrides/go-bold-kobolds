...
namespace Dreamteck.Splines
{
	...
	public class Spline
	{
		...
		/// </summary>
		public bool isClosed => closed && points.Length >= 3;
		...
		public double Project(Vector3 position, int subdivide = 4, double from = 0.0, double to = 1.0)
		{
			...
			if (closed && from == 0.0 && to == 1.0) //Handle looped splines
			{
				var closest = GetClosestPoint(
					subdivide, position, from, to, Mathf.RoundToInt(Mathf.Max(iterations / points.Length, 10)) * 5);
				if (closest < moveStep)
				{
					var nextClosest = GetClosestPoint(
						subdivide, position, 0.5, to, Mathf.RoundToInt(Mathf.Max(iterations / points.Length, 10)) * 5);
		...
		/// <returns></returns>
		public bool Raycast(
			out RaycastHit hit, out double hitPercent, LayerMask layerMask, double resolution = 1.0, double from = 0.0,
		...
		public bool RaycastAll(
			out RaycastHit[] hits, out double[] hitPercents, LayerMask layerMask, double resolution = 1.0,
		...
		)
		{
			...
			while (true)
			{
				...
				percent = DMath.Move(percent, to, moveStep / resolution);
				var toPos = EvaluatePosition(percent);
				var h = Physics.RaycastAll(
					fromPos, toPos - fromPos, Vector3.Distance(fromPos, toPos), layerMask, hitTriggers);
		...
		{
			if (closed) return DMath.Clamp01((double) pointIndex / points.Length);
		...
		public void Evaluate(double percent, ref SplineSample sample)
		{
			...
			if (closed) doubleIndex = points.Length * percent;
			var fromIndex = DMath.FloorInt(doubleIndex);
			var toIndex = fromIndex + 1;
			if (closed)
			{
				if (fromIndex >= points.Length - 1) fromIndex = points.Length - 1;
				if (toIndex > points.Length - 1) toIndex = 0;
			}
			else
			{
				if (toIndex > points.Length - 1) toIndex = points.Length - 1;
			}

			var getPercent = doubleIndex - fromIndex;
		...
		public void EvaluateUniform(
			ref SplineSample[] samples, ref double[] originalSamplePercents, double from = 0.0, double to = 1.0)
		{
			if (points.Length == 0)
			{
				samples = new SplineSample[0];
		...
		public void EvaluatePosition(double percent, ref Vector3 position)
		{
			...
			if (type == Type.Bezier) pointIndex = Mathf.Clamp(pointIndex, 0, Mathf.Max(points.Length - 1, 0));
		...
		public void Break(int at)
		{
			...
			for (var i = at; i < previousPoints.Length; i++) points[i - at] = previousPoints[i];

			for (var i = 0; i < at; i++) points[points.Length - at + i] = previousPoints[i];
			...
			switch (type)
			{
				...
				case Type.CatmullRom:
					for (var i = 0; i < points.Length; i++)
					{
						...
						var percent = GetPointPercent(i);
						var tangent = Vector3.forward;
						EvaluateTangent(percent, ref tangent);
						if (_knotParametrization > 0f)
						{
							ComputeCatPoints(i);
							points[i].SetTangent2Position(
								points[i].position + tangent.normalized * Vector3.Distance(P[0], P[2]) / 6f);
						}
						else
						{
		...
		private void CalculatePositionAndTangent(
			double percent, int pointIndex, ref Vector3 position, ref Vector3 tangent)
		{
			switch (type)
			{
				case Type.CatmullRom:
		...
		private void CalculateCatmullRomComponents(double t)
		{
			...
			var tf = Mathf.LerpUnclamped(t1, t2, (float) t);
		...
		private void CalculateCatmullRomTangent(double t, ref Vector3 tangent)
		{
			var tf = Mathf.LerpUnclamped(t1, t2, (float) t);
			var A1p = (P[1] - P[0]) / t1;
			var A2p = (P[2] - P[1]) / (t2 - t1);
			var A3p = (P[3] - P[2]) / (t3 - t2);

			var B1p = (A2 - A1) / t2 + (t2 - tf) / t2 * A1p + tf / t2 * A2p;
			var B2p = (A3 - A2) / (t3 - t1) + (t3 - tf) / (t3 - t1) * A2p + (tf - t1) / (t3 - t1) * A3p;
		...
		private void ComputeCatPoints(int i)
		{
			...
			if (closed)
			{
				if (p1 < 0) p1 += points.Length;
				if (p2 >= points.Length) p2 -= points.Length;
				if (p3 >= points.Length) p3 -= points.Length;
				if (p4 >= points.Length) p4 -= points.Length;
