...
namespace Dreamteck.Splines.Primitives
{
	public class Ellipse : SplinePrimitive
	{
		...
		protected override void Generate()
		{
			...
			points[2].SetTangentPosition(
				points[2].position + Vector3.left * 2 * (Mathf.Sqrt(2f) - 1f) / 1.5f * xRadius);
			points[3].position = Vector3.right * xRadius;
			points[3].SetTangentPosition(
				points[3].position + Vector3.down * 2 * (Mathf.Sqrt(2f) - 1f) / 1.5f * yRadius);
		}
	}
}
