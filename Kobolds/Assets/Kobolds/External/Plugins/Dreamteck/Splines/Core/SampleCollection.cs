...
namespace Dreamteck.Splines
{
	...
	{
		private static bool __useModifier;
		private static ISampleModifier __modifier;
		...
		public int GetClippedSampleCount(double clipFrom, double clipTo, out int startIndex, out int endIndex)
		{
			...
			if (clipTo < clipFrom) //Handle looping segments
			{
				...
				var fromSamples = samples.Length - startIndex;
		...
		public void GetSamplingValues(double percent, out int sampleIndex, out double lerp)
		{
			...
			if (sampleMode == SplineComputer.SampleMode.Optimized)
			{
				...
				if (sampleIndex < samples.Length - 1)
					lerp = DMath.InverseLerp(
						samples[sampleIndex].percent, samples[sampleIndex + 1].percent, lerpPercent);
				return;
			}
		...
		public void Evaluate(ref SplineSample[] results, double from = 0.0, double to = 1.0)
		{
			...
			for (var i = 1; i < results.Length - 1; i++) results[i].FastCopy(ref samples[i + fromIndex]);
		}
		...
		public double Travel(
			double start, float distance, Spline.Direction direction, out float moved, double clipFrom = 0.0,
			double clipTo = 1.0)
		{
			moved = 0f;
			...
			{
				var transformedPos = samples[sampleIndex].position;
			...
			var lerpPercent = 0.0;
			if (lastDistance > 0.0) lerpPercent = moveExcess / lastDistance;
			var p = DMath.Lerp(lastPercent, samples[sampleIndex].percent, 1f - lerpPercent);
			moved -= moveExcess;
			return p;
		}
		...
		/// <returns></returns>
		public void Project(
			Vector3 position, int controlPointCount, ref SplineSample result, double from = 0.0, double to = 1.0,
			ISampleModifier modifier = null)
		{
			...
			var step = samples.Length / steps;
			if (step < 1) step = 1;

			var minDist = (position - GetModifiedPosition(ref samples[0])).sqrMagnitude;
			var fromIndex = 0;
			var toIndex = samples.Length - 1;
			double lerp;
			...
			for (var i = fromIndex; i < toIndex; i += step)
			{
				...
				var pos1 = GetModifiedPosition(ref samples[i]);
				var pos2 = GetModifiedPosition(ref samples[Mathf.Min(i + step, toIndex)]);
				var projected = LinearAlgebraUtility.ProjectOnLine(pos1, pos2, position);
				var dist = (position - projected).sqrMagnitude;
				if (dist < minDist)
				{
					minDist = dist;
			...
			{
				var dist = (position - GetModifiedPosition(ref samples[i])).sqrMagnitude;
				if (dist < minDist)
				{
					minDist = dist;
					index = i;
			...
			var backPos = GetModifiedPosition(ref samples[backIndex]);
			var currentPos = GetModifiedPosition(ref samples[index]);
			var frontPos = GetModifiedPosition(ref samples[frontIndex]);

			var back = LinearAlgebraUtility.ProjectOnLine(backPos, currentPos, position);
			var front = LinearAlgebraUtility.ProjectOnLine(currentPos, frontPos, position);
			var backLength = (currentPos - backPos).magnitude;
			var frontLength = (currentPos - frontPos).magnitude;
			var backProjectDist = (back - backPos).magnitude;
			var frontProjectDist = (front - frontPos).magnitude;
			if (backIndex < index && index < frontIndex)
			{
				if ((position - back).sqrMagnitude < (position - front).sqrMagnitude)
				{
					SplineSample.Lerp(
						ref samples[backIndex], ref samples[index], backProjectDist / backLength, ref result);
					if (sampleMode == SplineComputer.SampleMode.Uniform)
						result.percent = DMath.Lerp(
							GetSamplePercent(backIndex), GetSamplePercent(index), backProjectDist / backLength);
				}
				else
				{
					SplineSample.Lerp(
						ref samples[frontIndex], ref samples[index], frontProjectDist / frontLength, ref result);
					if (sampleMode == SplineComputer.SampleMode.Uniform)
						result.percent = DMath.Lerp(
							GetSamplePercent(frontIndex), GetSamplePercent(index), frontProjectDist / frontLength);
				}
			}
			...
			{
				var pos1 = GetModifiedPosition(ref samples[samples.Length - 1]);
				var pos2 = GetModifiedPosition(ref samples[samples.Length - 2]);
				...
				if ((position - projected).sqrMagnitude < (position - result.position).sqrMagnitude)
				{
					...
					if (sampleMode == SplineComputer.SampleMode.Uniform)
						result.percent = DMath.Lerp(
							GetSamplePercent(samples.Length - 1), GetSamplePercent(samples.Length - 2), l);
				}
			}

			if (__useModifier) modifier.ApplySampleModifiers(ref result);
		}
		...
		public float CalculateLength(double from = 0.0, double to = 1.0, bool preventInvert = true)
		{
			...
			var length = 0f;
			var lastPos = EvaluatePosition(from);
			int fromIndex, toIndex;
			double lerp;
			GetSamplingValues(from, out fromIndex, out lerp);
			GetSamplingValues(to, out toIndex, out lerp);

			if (lerp > 0.0 && toIndex < this.length - 1) toIndex++;

			for (var i = fromIndex + 1; i < toIndex; i++)
			{
				var currentPos = samples[i].position;
