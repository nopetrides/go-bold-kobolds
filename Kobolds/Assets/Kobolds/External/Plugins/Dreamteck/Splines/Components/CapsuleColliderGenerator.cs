...
namespace Dreamteck.Splines
{
	public class CapsuleColliderGenerator : SplineUser, ISerializationCallbackReceiver
	{
		...
		protected override void Build()
		{
			...
			if (objectCount != _colliders.Length) GenerateColliders(objectCount);

			var current = new SplineSample();
			var next = new SplineSample();
			Evaluate(0.0, ref current);

			var controlHeight = _direction == CapsuleColliderZDirection.Z;
			...
			for (var i = 0; i < newColliders.Length; i++)
				...
				else
				{
					...
					newColliders[i] = new ColliderObject(
						newObject.transform, newObject.AddComponent<CapsuleCollider>(), _direction, _height);
				}
		...
		public class ColliderObject
		{
			...
			public ColliderObject(
				Transform transform, CapsuleCollider collider, CapsuleColliderZDirection direction, float height)
			{
