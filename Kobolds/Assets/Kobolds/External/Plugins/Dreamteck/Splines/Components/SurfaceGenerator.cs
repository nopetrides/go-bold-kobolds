...
#region UVs
			...
			if (pathExtrude)
			{
				...
				for (var i = 0; i < extrudeResults.Length; i++)
				{
					...
					for (var n = 0; n < identityVertices.Length; n++)
					{
						...
						if (_uniformUvs)
							_tsMesh.uv[startIndex + n] = new Vector2(
								(float) n / (identityVertices.Length - 1) * _sideUvScale.x + _sideUvOffset.x,
						...
						if (clockwise) _tsMesh.uv[startIndex + n].x = 1f - _tsMesh.uv[startIndex + n].x;
					}
				}
				...
				MeshUtility.GeneratePlaneTriangles(
					ref wallTris, sampleCount - 1, extrudeResults.Length, flipSide, 0, 0, true);
			...
			else if (simpleExtrude)
			{
				...
				for (var i = 0; i < surfaceVertexCount + 1; i++)
				{
					...
					if (_uniformUvs)
						_tsMesh.uv[offsetIndex] = new Vector2(
							(float) i / surfaceVertexCount * _sideUvScale.x + _sideUvOffset.x,
					...
					if (clockwise) _tsMesh.uv[offsetIndex].x = 1f - _tsMesh.uv[offsetIndex].x;
				}
