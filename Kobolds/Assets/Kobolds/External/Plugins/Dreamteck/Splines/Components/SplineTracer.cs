...
namespace Dreamteck.Splines
{
	public class SplineTracer : SplineUser
	{
		...
		private readonly List<NodeConnection> nodeConnectionQueue = new();
		...
		protected void ApplyMotion()
		{
			...
			{
				var unclippedPercent = UnclipPercent(_result.percent);
				int index;
				double lerp;
			...
			switch (_physicsMode)
			{
					...
					if (targetRigidbody == null)
					{
						...
						if (targetRigidbody == null)
							throw new MissingComponentException(
								"There is no Rigidbody attached to " + name +
				...
				case PhysicsMode.Rigidbody2D:
					if (targetRigidbody2D == null)
					{
						...
						if (targetRigidbody2D == null)
							throw new MissingComponentException(
								"There is no Rigidbody2D attached to " + name +
								" but the Physics mode is set to use one.");
					}
		...
		protected void CheckNodes(double from, double to)
		{
			...
			{
				var pointCount = spline.pointCount - 1;
				if (spline.isClosed) pointCount = spline.pointCount;
				var junction = GetJunction(pointCount);
				if (junction != null) nodeConnectionQueue.Add(junction);
			}
		}
