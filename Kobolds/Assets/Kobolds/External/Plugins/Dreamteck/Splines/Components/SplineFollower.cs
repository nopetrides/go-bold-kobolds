...
namespace Dreamteck.Splines
{
	...
	public class SplineFollower : SplineTracer
	{
		...
		[SerializeField]
		[HideInInspector]
		private FollowerSpeedModifier _speedModifier = new();
		...
		public float followSpeed
		{
			...
			set
			{
				if (_followSpeed != value)
				{
					...
					if (_followSpeed < 0f) direction = Spline.Direction.Backward;
					if (_followSpeed > 0f) direction = Spline.Direction.Forward;
				}
			}
		}
		...
		public override Spline.Direction direction
		{
			...
			set
			{
				...
				{
					if (_followSpeed < 0f) _followSpeed = -_followSpeed;
				}
				else
				{
					if (_followSpeed > 0f) _followSpeed = -_followSpeed;
		...
		{
			base.Start();
			if (_follow && autoStartPosition) SetPercent(spline.Project(GetTransform().position).percent);
		}
		...
		private void Follow()
		{
			switch (followMode)
			{
				case FollowMode.Uniform:
					...
					if (!_speedModifier.useClippedPercent) UnclipPercent(ref percent);
					var speed = _speedModifier.GetSpeed(Mathf.Abs(_followSpeed), percent);
		...
		public void Move(double percent)
		{
			...
			if (callOnEndReached)
			{
				...
				if (_unityOnEndReached != null) _unityOnEndReached.Invoke((float) startPercent);
			}
			else if (callOnBeginningReached)
			{
				if (onBeginningReached != null) onBeginningReached(startPercent);
				if (_unityOnBeginningReached != null) _unityOnBeginningReached.Invoke((float) startPercent);
			}
		...
		public void Move(float distance)
		{
			...
			var travelPercent = DoTravel(_result.percent, distance, out moved);
