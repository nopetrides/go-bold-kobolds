...
namespace Dreamteck.Splines
{
	public class MeshGenerator : SplineUser
	{
		...
		[SerializeField]
		[HideInInspector]
		private IndexFormat _meshIndexFormat = IndexFormat.UInt16;
		...
		{
			base.Build();
			if (_tsMesh == null || _mesh == null) CreateMesh();
		...
		protected virtual void WriteMesh()
		{
			...
			else if (_flipFaces) MeshUtility.FlipFaces(_tsMesh);

			if (_calculateTangents) MeshUtility.CalculateTangents(_tsMesh);

			if (_meshIndexFormat == IndexFormat.UInt16 && _tsMesh.vertexCount > UNITY_16_VERTEX_LIMIT)
				Debug.LogError(
					"WARNING: The generated mesh for " + name +
					" exceeds the maximum vertex count for standard meshes in Unity (" + UNITY_16_VERTEX_LIMIT +
			...
			if (_markDynamic) _mesh.MarkDynamic();

			if (_normalMethod == 0) _mesh.RecalculateNormals();
		...
		protected virtual void AllocateMesh(int vertexCount, int trisCount)
		{
			...
			if (_tsMesh.triangles.Length != trisCount) _tsMesh.triangles = new int[trisCount];
		}

		protected void ResetUVDistance()
		{
			_vDist = 0f;
			if (uvMode == UVMode.UniformClip) _vDist = spline.CalculateLength(0.0, GetSamplePercent(0));
		}
		...
		public void Bake(bool makeStatic, bool lightmapUV)
		{
			...
			if (lightmapUV) Unwrapping.GenerateSecondaryUVSet(_bakedMesh);
