	...
	public class SplinePositioner : SplineTracer
	{
		...
		public SplineTracer followTarget
		{
			...
			set
			{
				...
				{
					if (_followTarget != null) _followTarget.onMotionApplied -= OnFollowTargetMotionApplied;
					if (value == this)
					{
		...
		{
			get => _followTargetDistance;
			set
			{
				if (value != _followTargetDistance)
				{
					_followTargetDistance = value;
					if (followTarget != null) OnFollowTargetMotionApplied();
				}
			}
		}
		...
		{
			float moved;
			var percent = Travel(followTarget.result.percent, _followTargetDistance, _followTargetDirection, out moved);
			if (_followLoop)
		...
		{
			base.PostBuild();
			if (mode == Mode.Distance) SetDistance(_position, true);
		...
		public override void SetDistance(float distance, bool checkTriggers = false, bool handleJunctions = false)
		{
			...
			var travel = Travel(0.0, distance);
