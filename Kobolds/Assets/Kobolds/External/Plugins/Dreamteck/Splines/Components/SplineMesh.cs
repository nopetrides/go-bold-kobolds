...
namespace Dreamteck.Splines
{
	...
	public partial class SplineMesh : MeshGenerator
	{
		//Mesh data
		[SerializeField]
		[HideInInspector] [FormerlySerializedAs("channels")]
		private List<Channel> _channels = new();

		private readonly List<TS_Mesh> _combineMeshes = new();
		...
		protected override void Awake()
		{
			...
			{
				for (var j = 0; j < _channels[i].GetMeshCount(); j++) _channels[i].GetMesh(j).Refresh();
			}
#endif
		}
		...
		private void Generate()
		{
			...
			for (var i = 0; i < _channels.Count; i++)
			{
				...
				{
					var avgBounds = 0f;
					for (var j = 0; j < _channels[i].GetMeshCount(); j++)
						avgBounds += _channels[i].GetMesh(j).bounds.size.z;

					if (_channels[i].GetMeshCount() > 1) avgBounds /= _channels[i].GetMeshCount();

					if (avgBounds > 0f)
					{
						var length = CalculateLength(_channels[i].clipFrom, _channels[i].clipTo, false);
			...
			if (_combineMeshes.Count < meshCount)
				...
				_combineMeshes.RemoveRange(
					_combineMeshes.Count - 1 - (_combineMeshes.Count - meshCount), _combineMeshes.Count - meshCount);
			...
			for (var i = 0; i < _channels.Count; i++)
			{
				...
				var step = 1.0 / _channels[i].count;
				var space = step * _channels[i].spacing * 0.5;

				switch (_channels[i].type)
				{
					case Channel.Type.Extrude:
						for (var j = 0; j < _channels[i].count; j++)
						{
							var from = DMath.Lerp(_channels[i].clipFrom, _channels[i].clipTo, j * step + space);
							var to = DMath.Lerp(_channels[i].clipFrom, _channels[i].clipTo, j * step + step - space);
					...
					case Channel.Type.Place:
						for (var j = 0; j < _channels[i].count; j++)
						{
							if (_combineMeshes[combineMeshIndex] == null)
								_combineMeshes[combineMeshIndex] = new TS_Mesh();
							Place(
								_channels[i], _combineMeshes[combineMeshIndex],
								DMath.Lerp(
									_channels[i].clipFrom, _channels[i].clipTo,
		...
		private void Place(Channel channel, TS_Mesh target, double percent)
		{
			...
			var channelOffset = channel.NextRandomOffset();
			var channelRotation = channel.NextRandomQuaternion();

			var customValues = channel.GetCustomPlaceValues(percent);

			var finalOffset = channelOffset + customValues.Item1 + new Vector2(offset.x, offset.y);
			var finalRotation = channelRotation * Quaternion.AngleAxis(rotation, Vector3.forward) * customValues.Item2;
			var finalScale = channel.NextPlaceScale();
			...
			if (!channel.scaleModifier.useClippedPercent) UnclipPercent(ref evalResult.percent);
			var scaleMod = channel.scaleModifier.GetScale(evalResult);
			...
			if (!channel.scaleModifier.useClippedPercent) ClipPercent(ref evalResult.percent);
		...
		private void Extrude(Channel channel, TS_Mesh target, double from, double to)
		{
			...
			var channelScale = channel.NextRandomScale();
			var channelRotation = channel.NextRandomAngle();
			...
			for (var i = 0; i < definition.vertexGroups.Count; i++)
			{
				...
				var finalOffset = offset + channelOffset + (Vector3) customValues.Item1;
				var finalRotation = rotation + channelRotation + customValues.Item2;
				var finalScale = channelScale;
				if (!channel.scaleModifier.useClippedPercent) UnclipPercent(ref evalResult.percent);
				...
				finalScale.y *= customValues.Item3.y * scaleMod.y;
				finalScale.z = 1f;
				var resultSize = evalResult.size;
				_vertexMatrix.SetTRS(
					evalResult.position + originalRight * (finalOffset.x * resultSize) +
					originalNormal * (finalOffset.y * resultSize) + originalDirection * offset.z, //Position
				...
				for (var n = 0; n < definition.vertexGroups[i].ids.Length; n++)
				{
					...
					if (target.uv.Length > index)
					{
						...
						target.uv[index] = new Vector2(
							uv.x * uvScale.x * channel.uvScale.x, uv.y * uvScale.y * channel.uvScale.y);
