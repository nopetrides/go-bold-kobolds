	...
	public class TS_Mesh
	{
		...
		public void Combine(List<TS_Mesh> combineMeshes)
		{
			...
			if (_submeshTrisCount.Length != addedSubmeshCount)
				...
				for (var i = 0; i < _submeshTrisCount.Length; i++)
			...
			if (_submeshOffsets.Length != addedSubmeshCount)
				...
				for (var i = 0; i < _submeshOffsets.Length; i++)
					_submeshOffsets[i] = 0;
			...
			for (var i = 0; i < combineMeshes.Count; i++)
			{
				...
				for (var j = 0; j < combineMeshes[i].subMeshes.Count; j++)
				{
					...
					else if (subMeshes[j].Length != _submeshTrisCount[j]) subMeshes[j] = new int[_submeshTrisCount[j]];
					var submesh = combineMeshes[i].subMeshes[j];

					for (var x = 0; x < submesh.Length; x++)
					{
						var index = _submeshOffsets[j] + x;
		...
		public void AddMeshes(List<TS_Mesh> addedMeshes)
		{
			...
			for (var i = 0; i < addedMeshes.Count; i++)
			{
				...
				if (addedMeshes[i].subMeshes.Count > submeshCount) submeshCount = addedMeshes[i].subMeshes.Count;
			}
			...
			for (var i = 0; i < addedMeshes.Count; i++)
			{
				...
				for (var n = newTris; n < newTris + addedMeshes[i].triangles.Length; n++)
					newTriangles[n] = addedMeshes[i].triangles[n - newTris] + newVerts;


				for (var n = 0; n < addedMeshes[i].subMeshes.Count; n++)
				{
					for (var x = submeshTrisCount[n]; x < submeshTrisCount[n] + addedMeshes[i].subMeshes[n].Length; x++)
