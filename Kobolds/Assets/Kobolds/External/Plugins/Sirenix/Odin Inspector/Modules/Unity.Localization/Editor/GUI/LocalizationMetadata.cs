...
namespace Sirenix.OdinInspector.Modules.Localization.Editor
{
	public class LocalizationMetadata
	{
		...
		private readonly LocalizationTableCollection localizationTableCollection;
		private Vector2 scrollPosition;
		private readonly SearchField searchField = new();
		private string searchTerm;
		private readonly StringBuilder stringBuilder = new();

		private readonly List<Type> tags = new()
		{
		...
		private readonly OdinLocalizationEditorWindow.WindowState windowState;
		...
		private GUIStyle ContentPadding =>
			_contentPadding = _contentPadding ?? new GUIStyle
			{
				padding = new RectOffset(10, 10, 10, 10)
			};

		private GUIStyle PlatformOverridePadding =>
			_platformOverridePadding = _platformOverridePadding ?? new GUIStyle
			{
		...
		{
			localizationTableCollection.SharedData.Metadata.AddMetadata(metadata);
		}
		...
		{
			localizationTableCollection.SharedData.Metadata.RemoveMetadata(metadata);
		}
		...
		{
			var minWidth = 0f;
			if (Target == null) return minWidth;
			...
			if (Event.current.type != EventType.Repaint)
			{
				if (toBeAdded != null)
				{
					switch (Target)
					{
						case LocalizationTable localizationTable:
							Undo.RecordObject(localizationTableCollection, "Localization Tables Metadata Added");
							Add(localizationTable, toBeAdded);
							break;
						case SharedTableData.SharedTableEntry sharedTableEntry:
							Undo.RecordObject(
								localizationTableCollection.SharedData, "Localization Tables Metadata Added");
							Add(sharedTableEntry, toBeAdded);
							break;
						case TableEntry tableEntry:
							Undo.RecordObject(tableEntry.Table, "Localization Tables Metadata Added");
							Add(tableEntry, toBeAdded);
							break;
						default:
							Undo.RecordObject(
								localizationTableCollection.SharedData, "Localization Tables Metadata Added");
				...
				if (toBeRemoved != null)
				{
					...
					{
						case LocalizationTable localizationTable:
							Undo.RecordObject(localizationTableCollection, "Localization Tables Metadata Removed");
							Remove(localizationTable, toBeRemoved);
							break;
						case SharedTableData.SharedTableEntry sharedTableEntry:
							Undo.RecordObject(
								localizationTableCollection.SharedData, "Localization Tables Metadata Removed");
							Remove(sharedTableEntry, toBeRemoved);
							break;
						case TableEntry tableEntry:
							Undo.RecordObject(tableEntry.Table, "Localization Tables Metadata Removed");
							Remove(tableEntry, toBeRemoved);
							break;
						default:
							Undo.RecordObject(
								localizationTableCollection.SharedData, "Localization Tables Metadata Removed");
			...
			if (windowState.MetadataTree == null || lastTarget != Target)
			{
				...
				{
					case LocalizationTable localizationTable:
						windowState.MetadataTree = PropertyTree.Create(localizationTable);
						break;
					case SharedTableData.SharedTableEntry sharedTableEntry:
						windowState.MetadataTree = PropertyTree.Create(sharedTableEntry);
						break;
					case TableEntry tableEntry:
					{
						var bindingFlags = BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic;
						var entryData = typeof(TableEntry).GetProperty("Data", bindingFlags).GetValue(tableEntry);
						windowState.MetadataTree = PropertyTree.Create(entryData);
						break;
					}
					case LocalizationTableCollection localizationTableCollection:
						windowState.MetadataTree = PropertyTree.Create(localizationTableCollection.SharedData);
						break;
				}
			...
			var metadataCollection = windowState.MetadataTree.RootProperty.Children["m_Metadata"];
			...
			metadataEntries = metadataEntries
				...
				.OrderBy(entry => !tags.Contains(entry.ValueEntry.TypeOfValue))
				.ThenBy(entry => entry.Name)
				.ToList();

			windowState.MetadataTree.BeginDraw(false);
			switch (Target)
			{
				case LocalizationTable localizationTable:
					minWidth = Draw(
						rect, localizationTable, "Table", metadataEntries, m => Add(localizationTable, m),
						m => Remove(localizationTable, m), () => OpenMetadataSelector(localizationTable));
					break;
				case SharedTableData.SharedTableEntry sharedTableEntry:
					minWidth = Draw(
						rect, this.localizationTableCollection.SharedData, "Shared Entry", metadataEntries,
						m => Add(sharedTableEntry, m), m => Remove(sharedTableEntry, m),
						() => OpenMetadataSelector(sharedTableEntry));
					break;
				case TableEntry tableEntry:
					minWidth = Draw(
						rect, tableEntry.Table, "Entry", metadataEntries, m => Add(tableEntry, m),
						m => Remove(tableEntry, m), () => OpenMetadataSelector(tableEntry));
					break;
				default:
					minWidth = Draw(
						rect, this.localizationTableCollection.SharedData, "Shared Table", metadataEntries, m => Add(m),
						m => Remove(m), () => OpenMetadataSelector(this.localizationTableCollection));
					break;
			}

			windowState.MetadataTree.EndDraw();

			return minWidth;
		}

		private float Draw(
			Rect rect, Object target, string title, List<InspectorProperty> metadataEntries, Action<IMetadata> add,
			Action<IMetadata> remove, Action openMetadataSelector)
		{
			...
			if (Event.current.OnKeyUp(KeyCode.LeftAlt)) lastMetadataTypeUnderCursor = null;
			...
			if (searchAreaRect.width > 125)
			{
				...
				searchTerm = searchField.Draw(
					searchAreaRect.SubXMax(titleWidth).Padding(6), searchTerm,
					"Search for metadata...");
			...
			var filteredMetadataEntries = string.IsNullOrWhiteSpace(searchTerm) ?
				metadataEntries.ToList() :
				metadataEntries.Where(metadataEntry =>
				{
					if (FuzzySearch.Contains(searchTerm, metadataEntry.Info.TypeOfOwner.Name)) return true;

					if (FuzzySearch.Contains(
							searchTerm, metadataEntry.ValueEntry?.WeakSmartValue?.ToString())) return true;

					foreach (var child in metadataEntry.Children.Recurse())
					{
						if (FuzzySearch.Contains(searchTerm, child.Name)) return true;

						if (FuzzySearch.Contains(searchTerm, child.ValueEntry?.WeakSmartValue?.ToString())) return true;
					}
			...
			GUIHelper.BeginLayoutMeasuring();
			{
				for (var i = 0; i < filteredMetadataEntries.Count; i++)
				{
					...
					var metadataHeaderRect = GUILayoutUtility.GetRect(
						0, headerHeight, GUILayoutOptions.ExpandWidth().ExpandHeight(false));
					...
					genericMenu.AddItem(
						new GUIContent("Delete"), false, () =>
						{
							...
							toBeRemoved = metadata;
						});

					var isTag = tags.Contains(metadataEntry.ValueEntry.TypeOfValue);

					if (!isTag)
					{
						genericMenu.AddItem(
							new GUIContent("Copy"), false, () =>
							{
								var metadata = (IMetadata) metadataEntry.ValueEntry.WeakSmartValue;
						...
						genericMenu.AddItem(
							new GUIContent(
								$"{(metadataEntry.State.Expanded ? "Collapse" : "Expand")} All   (Ctrl-Click)"), false,
							() =>
							{
						...
						genericMenu.AddItem(
							...
							{
								var newState = !metadataEntry.State.Expanded;
								foreach (var entry in filteredMetadataEntries.Where(m =>
											m.Info.TypeOfOwner == metadataEntry.Info.TypeOfOwner))
									entry.State.Expanded = newState;
							});
					}

					var metadataHeaderBackground = new Color(0.243f, 0.243f, 0.243f);
					...
					else if (!isTag && holdingAlt)
					{
						...
						{
							lastMetadataTypeUnderCursor = metadataEntry.ValueEntry.TypeOfValue;
							GUIHelper.RequestRepaint();
						}

						metadataHeaderBackground = lastMetadataTypeUnderCursor == metadataEntry.ValueEntry.TypeOfValue ?
					...
					{
						var headerLabelWidth = EditorStyles.label.CalcWidth(niceName) + 1;
						var preloadAssetLabel =
							ObjectNames.NicifyVariableName(metadataEntry.ValueEntry.WeakSmartValue.ToString());
						var hintRect = metadataHeaderRect.AlignRight(metadataHeaderRect.width - headerLabelWidth - 10);
					...
					else if (metadataEntry.Info.TypeOfOwner == typeof(PlatformOverride))
					{
						...
						for (var j = 0; j < platformOverrides.Count; j++)
						{
							...
							var platform = (RuntimePlatform) platformOverrideInspectorProperty.Children[0].ValueEntry
								.WeakSmartValue;
							stringBuilder.Append(platform);

							if (j != platformOverrides.Count - 1) stringBuilder.Append(", ");
						}

						var platforms = stringBuilder.ToString();
						...
						if (SirenixGUIStyles.RightAlignedGreyMiniLabel.CalcWidth(platforms) > hintRect.width)
						{
							...
							{
								GUI.Label(
									hintRect, $"[{platformOverrides.Count}]",
					...
					if (evt.OnMouseDown(unchangedMetadataHeaderRect, 0, false))
					{
						...
						else if (holdingAlt)
						{
							var newState = !metadataEntry.State.Expanded;
							foreach (var entry in filteredMetadataEntries.Where(m =>
										m.ValueEntry.TypeOfValue == lastMetadataTypeUnderCursor))
								entry.State.Expanded = newState;
						}
						else
						{
					...
					if (SirenixEditorGUI.BeginFadeGroup(metadataEntry, metadataEntry.State.Expanded))
					{
						...
						{
							EditorGUILayout.BeginVertical(ContentPadding);
							foreach (var child in metadataEntry.Children) child.Draw(null);
						...
						else if (metadataEntry.ValueEntry.TypeOfValue == typeof(PreloadAssetTableMetadata))
						{
							...
							var preloadAssetTableMetadata =
								(PreloadAssetTableMetadata) metadataEntry.ValueEntry.WeakSmartValue;
							...
							var leftActive = mouseOverLeft && Event.current.type == EventType.MouseDown &&
											Event.current.button == 0;
							var leftOn = preloadAssetTableMetadata.Behaviour ==
							...
							var rightActive = mouseOverRight && Event.current.type == EventType.MouseDown &&
											Event.current.button == 0;
							var rightOn = preloadAssetTableMetadata.Behaviour ==
										PreloadAssetTableMetadata.PreloadBehaviour.PreloadAll;

							if (Event.current.type == EventType.Repaint)
							{
								SirenixGUIStyles.ButtonLeft.Draw(
									leftRect, "No Preload", mouseOverLeft, leftActive, leftOn, false);
								SirenixGUIStyles.ButtonRight.Draw(
									rightRect, "Preload All", mouseOverRight, rightActive, rightOn, false);
							}
						...
						{
							DrawPlatformOverride(metadataEntry);
						}
						else
						{
							EditorGUILayout.BeginVertical(ContentPadding);
							foreach (var child in metadataEntry.Children) child.Draw();
							EditorGUILayout.EndVertical();
						}
			...
			if (metadataEntries.Count > 1) EditorGUI.DrawRect(rect.TakeFromTop(1), SirenixGUIStyles.BorderColor);

			var addMetadataRect = rect.AlignTop(addButtonAreaHeight).AlignCenter(Mathf.Min(200, rect.width - 16))
				.VerticalPadding(8);

			if (GUI.Button(addMetadataRect, "Add Metadata")) openMetadataSelector();
			...
			GUI.DrawTexture(
				...
				true,
				1.0f,
				new Color(0, 0, 0, Mathf.InverseLerp(0f, 30f, scrollPosition.y) * 0.6f),
				0,
				0);

			if (measureRect.height > metadataViewHeight)
			{
				var pos = scrollPosition.y + metadataViewHeight;
		...
		private void DrawPlatformOverride(InspectorProperty metadataEntry)
		{
			...
			var platformOverrideDatas =
				(IList) metadataEntry.Children["m_PlatformOverrides"]?.ValueEntry.WeakSmartValue;
			var platformOverrideDataInspectorProperties = metadataEntry.Children["m_PlatformOverrides"]?.Children;

			if (platformOverrideDatas == null) return;
			...
			for (var i = 0; i < platformOverrideDataInspectorProperties.Count; i++)
			{
				...
				var backgroundRect = EditorGUILayout.BeginVertical(PlatformOverridePadding);
				var backgroundColor =
				...
				var entryOverrideRect = GUILayoutUtility.GetRect(
					0, EditorGUIUtility.singleLineHeight, GUILayoutOptions.ExpandWidth().ExpandHeight(false));

				switch (entryOverrideType)
				{
					case EntryOverrideType.Table:
					{
						var tableReference = (TableReference) platformOverrideDataInspectorProperty.Children
							.FirstOrDefault(c => c.Info.TypeOfValue == typeof(TableReference))?.ValueEntry
							.WeakSmartValue;
						DoTableGUI(entryOverrideRect, tableReference, platformOverrideDataInspectorProperty);
						break;
					}
					case EntryOverrideType.Entry:
					{
						var tableReference = (TableReference) platformOverrideDataInspectorProperty.Children
							.FirstOrDefault(c => c.Info.TypeOfValue == typeof(TableReference))
							?.ValueEntry.WeakSmartValue;
						var tableEntryReference = (TableEntryReference) platformOverrideDataInspectorProperty.Children
							.FirstOrDefault(c => c.Info.TypeOfValue == typeof(TableEntryReference))?.ValueEntry
							.WeakSmartValue;
						DoTableAndEntryGUI(
							entryOverrideRect, tableReference, tableEntryReference,
							platformOverrideDataInspectorProperty, false);
						break;
					}
					...
					case EntryOverrideType.TableAndEntry:
					{
						...
						DoTableAndEntryGUI(
							entryOverrideRect, tableReference, tableEntryReference,
							platformOverrideDataInspectorProperty, true);
						break;
					}
				}
				...
				SdfIcons.DrawIcon(
					xIconRect, SdfIconType.X, mouseOverX ? Color.white : EditorStyles.label.normal.textColor);

				if (Event.current.OnMouseDown(xIconRect, 0, false))
				{
					var platform = (RuntimePlatform) platformOverrideDataInspectorProperty.Children[0].ValueEntry
						.WeakSmartValue;
		...
		private void OpenMetadataSelector(LocalizationTableCollection localizationTableCollection)
		{
			OpenMetadataSelector(
				localizationTableCollection.SharedData.Metadata.MetadataEntries, MetadataType.SharedTableData);
		}
		...
		private void OpenMetadataSelector(LocalizationTable localizationTable)
		{
			...
			{
				case StringTable _:
					OpenMetadataSelector(localizationTable.MetadataEntries, MetadataType.StringTable);
					break;
				case AssetTable _:
					OpenMetadataSelector(localizationTable.MetadataEntries, MetadataType.AssetTable);
					break;
			}
		}
		...
		{
			switch (localizationTableCollection)
			{
				case StringTableCollection _:
					OpenMetadataSelector(
						sharedTableEntry.Metadata.MetadataEntries, MetadataType.StringTableEntry,
						MetadataType.SharedStringTableEntry);
					break;
				case AssetTableCollection _:
					OpenMetadataSelector(
						sharedTableEntry.Metadata.MetadataEntries, MetadataType.AssetTableEntry,
		...
		private void OpenMetadataSelector(TableEntry tableEntry)
		{
			switch (tableEntry)
			{
				case StringTableEntry _:
					OpenMetadataSelector(tableEntry.MetadataEntries, MetadataType.StringTableEntry);
					break;
				case AssetTableEntry _:
					OpenMetadataSelector(tableEntry.MetadataEntries, MetadataType.AssetTableEntry);
					break;
			}
		}
		...
		private void OpenMetadataSelector(IList<IMetadata> existingMetadata, params MetadataType[] allowedTypes)
		{
			...
			foreach (var metadataType in metadataTypes)
			{
				...
				if (metadataAttribute != null &&
					allowedTypes.Any(allowedType => metadataAttribute.AllowedTypes.HasFlag(allowedType)))
				{
					if (existingMetadataTypes.Contains(metadataType) &&
						!OdinLocalizationMetadataRegistry.MetadataAllowsMultiple.ContainsKey(metadataType)) continue;
			...
			var selector = new GenericSelector<Type>(
				"", validMetadataTypes, false, type => ObjectNames.NicifyVariableName(type.Name));

			foreach (var item in selector.SelectionTree.MenuItems) item.SdfIcon = SdfIconType.Braces;
		...
		private void DoTableGUI(Rect rect, TableReference tableReference, InspectorProperty platformOverrideData)
		{
			...
			if (EditorGUI.DropdownButton(dropDownPosition, new GUIContent(label), FocusType.Passive))
			{
				...
				tableReferenceSelector = localizationTableCollection.GetType() == typeof(AssetTableCollection) ?
				...
				tableReferenceSelector.SelectionConfirmed += selections =>
				{
					...
					var platformOverrideDataType = platformOverrideType.GetNestedType(
						"PlatformOverrideData", BindingFlags.NonPublic);

					platformOverrideDataType
						.GetField("tableReference")
						.SetValue(
							platformOverrideData.ValueEntry.WeakSmartValue, selection.TableCollectionNameReference);
			...
			bool displayTableName)
		{
			...
			var entryLabel = tableEntryReference.ReferenceType != TableEntryReference.Type.Empty ?
				tableEntryReference.ResolveKeyName(localizationTableCollection.SharedData) :
				null;
			...
			if (displayTableName)
			{
				...
				label = string.IsNullOrEmpty(tableLabel) || string.IsNullOrEmpty(entryLabel) ?
					"None" :
			...
			if (EditorGUI.DropdownButton(dropDownPosition, new GUIContent(label), FocusType.Passive))
			{
				var tableType = localizationTableCollection.GetType() == typeof(AssetTableCollection) ?
					typeof(AssetTable) :
					typeof(StringTable);

				var tableEntryReferenceSelector = new TableEntryReferenceSelector(tableType);
		...
		public class TableReferenceSelector : OdinSelector<LocalizationTableCollection>
		{
			...
			protected override void BuildSelectionTree(OdinMenuTree tree)
			{
				...
				foreach (var guid in collectionGUIDs)
				{
					...
					if (collection.Tables[0].asset.GetType() != tableType) continue;
