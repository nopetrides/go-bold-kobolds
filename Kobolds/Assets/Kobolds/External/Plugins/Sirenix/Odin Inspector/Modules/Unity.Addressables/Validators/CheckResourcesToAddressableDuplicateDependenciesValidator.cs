...
[assembly: RegisterValidationRule(
	typeof(CheckResourcesToAddressableDuplicateDependenciesValidator),
	Description =
...
namespace Sirenix.OdinInspector.Modules.Addressables.Editor
{
	public class CheckResourcesToAddressableDuplicateDependenciesValidator : GlobalValidator
	{
		...
		public List<string> IgnoredGUIDs = new();
			...
			foreach (var addressableAssetGroup in addressableAssetSettings.groups)
			{
				...
				foreach (var addressableAssetEntry in addressableAssetGroup.entries)
				{
					...
					foreach (var dependencyAssetPath in dependencyAssetPaths)
					{
						...
						if (IgnoredGUIDs.Contains(dependencyGUID.ToString())) continue;
						...
						result.Add(
								ValidatorSeverity,
								$"{dependencyAssetPath} is duplicated in addressable data and resource folders.")
							.WithFix<FixArgs>(
								args =>
								{
									if (args.FixChoice == FixChoice.Ignore)
									{
										var sourceType = args.IgnoreForEveryone ?
											ConfigSourceType.Project :
											ConfigSourceType.Local;
										var data = RuleConfig.Instance
											.GetRuleData<CheckResourcesToAddressableDuplicateDependenciesValidator>(
												sourceType);
							...
							.WithModifyRuleDataContextClick<CheckResourcesToAddressableDuplicateDependenciesValidator>(
								"Ignore", data => { data.IgnoredGUIDs.Add(dependencyGUID.ToString()); })
							.SetSelectionObject(
								AssetDatabase.LoadAssetAtPath<Object>(
									AssetDatabase.GUIDToAssetPath(dependencyGUID.ToString())));
		...
		private class FixArgs
		{
			...
			public readonly bool IgnoreForEveryone = true;

			[FolderPath]
			[PropertySpace(10)]
			[ValidateInput(nameof(ValidateFolderPath))]
			[ShowIf("FixChoice", FixChoice.MoveAsset, Animate = false)]
			public readonly string NewFolder = "Assets/Resources_moved";
