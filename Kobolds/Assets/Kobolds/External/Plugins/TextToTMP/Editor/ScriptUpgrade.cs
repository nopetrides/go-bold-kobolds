...
namespace TextToTMPNamespace
{
	...
	{
#if UNITY_2017_3_OR_NEWER
		private readonly ObjectsToUpgradeList assemblyDefinitionFilesToUpgrade = new();
#endif

		private readonly ObjectsToUpgradeList scriptsToUpgrade = new();

		private void UpgradeScripts()
		{
#if UNITY_2017_3_OR_NEWER
			textMeshProAssemblyDefinitionFilePath =
				CompilationPipeline.GetAssemblyDefinitionFilePathFromAssemblyName("Unity.TextMeshPro");
			...
			{
				var tmpScriptGuid = AssetDatabase.FindAssets("TextMeshProUGUI t:MonoScript");
...
#if UNITY_2017_3_OR_NEWER
			var progressTotal = scriptsToUpgrade.EnabledCount + assemblyDefinitionFilesToUpgrade.EnabledCount;
#else
			...
			try
			{
				foreach (var script in scriptsToUpgrade)
				{
					EditorUtility.DisplayProgressBar(
						"Upgrading scripts...", script, (float) progressCurrent / progressTotal);
					progressCurrent++;
...
#if UNITY_2017_3_OR_NEWER
				foreach (var assemblyDefinitionFile in assemblyDefinitionFilesToUpgrade)
				{
					EditorUtility.DisplayProgressBar(
						"Upgrading Assembly Definition Files...", assemblyDefinitionFile,
		...
		private void UpgradeScript(string scriptPath)
		{
			...
			for (var i = 0; i < tokenReplacementsVariableDeclarations.Length; i += 2)
			{
				// Variable declarations
				scriptModified = scriptModified.Replace(
					"public " + tokenReplacementsVariableDeclarations[i] + " ",
					"public " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"private " + tokenReplacementsVariableDeclarations[i] + " ",
					"private " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"protected " + tokenReplacementsVariableDeclarations[i] + " ",
					"protected " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"internal " + tokenReplacementsVariableDeclarations[i] + " ",
					"internal " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"static " + tokenReplacementsVariableDeclarations[i] + " ",
					"static " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"readonly " + tokenReplacementsVariableDeclarations[i] + " ",
					"readonly " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"virtual " + tokenReplacementsVariableDeclarations[i] + " ",
					"virtual " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"volatile " + tokenReplacementsVariableDeclarations[i] + " ",
					"volatile " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"sealed " + tokenReplacementsVariableDeclarations[i] + " ",
					"sealed " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"partial " + tokenReplacementsVariableDeclarations[i] + " ",
					"partial " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					"extern " + tokenReplacementsVariableDeclarations[i] + " ",
				...
				scriptModified = scriptModified.Replace(
					"\t" + tokenReplacementsVariableDeclarations[i] + " ",
				...
				scriptModified = scriptModified.Replace(
					...
					" " + tokenReplacementsVariableDeclarations[i + 1] + "[]");
				scriptModified = scriptModified.Replace(
					" new " + tokenReplacementsVariableDeclarations[i] + "[",
				...
				scriptModified = scriptModified.Replace(
					...
					"( " + tokenReplacementsVariableDeclarations[i + 1] + " )");
				scriptModified = scriptModified.Replace(
					" is " + tokenReplacementsVariableDeclarations[i] + " ",
					" is " + tokenReplacementsVariableDeclarations[i + 1] + " ");
				scriptModified = scriptModified.Replace(
					" as " + tokenReplacementsVariableDeclarations[i] + " ",
				...
				scriptModified = scriptModified.Replace(
					"this " + tokenReplacementsVariableDeclarations[i] + " ",
			...
			for (var i = 0; i < tokenReplacementsWithTMPChecks.Length; i += 2)
			{
				...
				while ((index = scriptModified.IndexOf(tokenReplacementsWithTMPChecks[i], index + 1)) >= 0)
					if (index == 0 || scriptModified[index - 1] != '_')
						scriptModified = string.Concat(
							scriptModified.Substring(0, index), tokenReplacementsWithTMPChecks[i + 1],
							scriptModified.Substring(index + tokenReplacementsWithTMPChecks[i].Length));
			}

			var shouldAddExtensionFunctionsToScript = false;
			for (var i = 0; i < tokenReplacementsExtensionFunctions.Length; i += 3)
			{
				...
				{
					var endIndex = scriptModified.IndexOf(';', index);
					if (endIndex >= 0)
					{
						var functionParametersStartIndex = index + tokenReplacementsExtensionFunctions[i].Length;
						var functionParametersLength = endIndex - functionParametersStartIndex;

						scriptModified = string.Concat(
							scriptModified.Substring(0, index), tokenReplacementsExtensionFunctions[i + 1],
			...
			for (var i = 0; i < tokenReplacementsAsIs.Length; i += 2)
		...
		private void AddExtensionFunctionsToScript(string scriptPath, ref string scriptContents)
		{
			...
			var extensionFunctionsInsertIndex = scriptContents.LastIndexOf("using ");
			...
			var namespaceGuid = Guid.NewGuid().ToString().Replace("-", ""); // Each namespace should be unique
			scriptContents = scriptContents.Insert(
				extensionFunctionsInsertIndex,
		...
		private void UpgradeAssemblyDefinitionFile(string path)
		{
			...
			if (AssetDatabase.GetMainAssetTypeAtPath(path) != typeof(AssemblyDefinitionAsset))
				return;

			var asmFile = JsonUtility.FromJson<AssemblyDefinitionFileContents>(File.ReadAllText(path));
			if (asmFile.references != null)
				for (var i = 0; i < asmFile.references.Length; i++)
				{
...
";
		...
		private readonly string[] tokenReplacementsVariableDeclarations =
		{
		...
		private readonly string[] tokenReplacementsWithTMPChecks =
		{
		...
		private readonly string[] tokenReplacementsAsIs =
		{
		...
		private readonly string[] tokenReplacementsExtensionFunctions =
		{
