...
#region Constants
		...
		private readonly List<PrefabInstancesRemovedComponent> upgradedComponentsToRemoveInPrefabInstances = new();

		private readonly HashSet<string> upgradedPrefabs = new();

		private bool alwaysUseOverflowForNonWrappingTexts;
		...
		private void UpgradeComponents()
		{
			...
			var prefabsToUpgrade = new List<GameObject>(assetsToUpgrade.Length);
			var prefabsToUpgradePaths = new HashSet<string>();
			...
			try
			{
				foreach (var asset in assetsToUpgrade)
				{
					EditorUtility.DisplayProgressBar(
						"Upgrading components in assets...", asset, (float) progressCurrent / progressTotal);
					progressCurrent++;
				...
				for (var i = 0; i < prefabsToUpgrade.Count - 1; i++)
				{
					var swapIndex = i;
					for (var j = i + 1; j < prefabsToUpgrade.Count; j++)
						// If the prefab at index j is a nested/base prefab of the one at swapIndex, upgrade it first
						if ((bool) prefabCyclicReferenceCheckerMethod.Invoke(
								null, new object[] {prefabsToUpgrade[j], prefabsToUpgrade[swapIndex]}))
							swapIndex = j;

					var temp = prefabsToUpgrade[swapIndex];
				...
				{
					EditorUtility.DisplayProgressBar(
						"Upgrading components in prefabs...", AssetDatabase.GetAssetPath(prefab),
				...
				{
					EditorUtility.DisplayProgressBar(
						"Upgrading components in scenes...", scene, (float) progressCurrent / progressTotal);
					progressCurrent++;

					UpgradeComponentsInScene(SceneManager.GetSceneByPath(scene));
				}

				foreach (var modifiedPrefabInstance in modifiedTextPrefabInstances)
					ApplyPrefabInstanceModifications(
						modifiedPrefabInstance, tmp => PasteTextProperties(tmp, modifiedPrefabInstance),
						ref progressCurrent, progressTotal);
				foreach (var modifiedPrefabInstance in modifiedTextMeshPrefabInstances)
					ApplyPrefabInstanceModifications(
						modifiedPrefabInstance, tmp => PasteTextMeshProperties(tmp, modifiedPrefabInstance),
						ref progressCurrent, progressTotal);
				foreach (var modifiedPrefabInstance in modifiedInputFieldPrefabInstances)
					ApplyPrefabInstanceModifications(
						modifiedPrefabInstance, tmp => PasteInputFieldProperties(tmp, modifiedPrefabInstance),
						ref progressCurrent, progressTotal);
				foreach (var modifiedPrefabInstance in modifiedDropdownPrefabInstances)
					ApplyPrefabInstanceModifications(
						modifiedPrefabInstance, tmp => PasteDropdownProperties(tmp, modifiedPrefabInstance),
						ref progressCurrent, progressTotal);
			}
			finally
			{
		...
		private bool ShouldUpgradeComponentsInPrefab(GameObject prefab)
		{
			...
			foreach (var graphicComponent in prefab.GetComponentsInChildren<UIBehaviour>(true))
		...
		private void UpgradeComponentsInPrefab(GameObject prefab)
		{
			...
			var prefabInstanceRoot = PrefabUtility.LoadPrefabContents(prefabPath);
#else
 ...
 PrefabUtility.ReplacePrefab( prefabInstanceRoot, prefab, ReplacePrefabOptions.ConnectToPrefab );
			...
			foreach (var removedComponentHolder in upgradedComponentsToRemoveInPrefabInstances)
			{
				if (!removedComponentHolder.componentOwner)
					continue;

				foreach (var prefabInstance in removedComponentHolder.removedPrefabInstances)
				{
					if (!prefabInstance)
						continue;

					foreach (var component in
							prefabInstance.GetComponents(removedComponentHolder.UpgradedComponentType))
					{
						if (!component)
							continue;
						...
						if (prefabComponent && prefabComponent.gameObject == removedComponentHolder.componentOwner)
						{
							stringBuilder.Append("Removing ").Append(component.GetType().Name).Append(" from ")
								.Append(GetPathOfObject(component.transform)).AppendLine(
									" since its legacy version was also removed as prefab override");
		...
		/// must resist the temptation). That's because the
		/// <see cref="UpgradeTextMesh" />
		/// function converts legacy TextMesh's Transform component to RectTransform,
		...
		private void UpgradeGameObjectRecursively(GameObject go, GameObject prefab)
		{
			try
			{
				...
				var inputField = UpgradeInputField(
					go.GetComponent<InputField>(), prefab ? prefab.GetComponent<InputField>() : null, false);
				...
				for (var i = 0; i < go.transform.childCount; i++)
					UpgradeGameObjectRecursively(
						go.transform.GetChild(i).gameObject, prefab ? prefab.transform.GetChild(i).gameObject : null);
			...
			catch (Exception e)
			{
				if (!e.Data.Contains("LoggedHierarchy"))
				{
					...
					Debug.LogError(
						"Error while upgrading components of: " + GetPathOfObject(go.transform),
		...
		private TextMeshProUGUI UpgradeText(Text text, Text prefabText)
		{
			...
			var properties = CopyTextProperties(text);
		...
		private TextProperties CopyTextProperties(Text text)
		{
			...
			var font = GetCorrespondingTMPFontAsset(text.font, text, out fontMaterial);
		...
		private TextMeshPro UpgradeTextMesh(TextMesh text, TextMesh prefabText)
		{
			...
			// Copy fields
			var properties = CopyTextMeshProperties(text);
		...
		private TMP_InputField UpgradeInputField(
			InputField inputField, InputField prefabInputField, bool createViewportImmediately = true)
		{
			if (!inputField)
				return null;
			...
			// Copy fields
			var sizeDelta = ((RectTransform) inputField.transform).sizeDelta;
			var properties = CopyInputFieldProperties(inputField);

			// Upgrade child objects
			var textComponent = UpgradeText(
				inputField.textComponent, prefabInputField ? prefabInputField.textComponent : null);
			var placeholderComponent = inputField.placeholder as Text ?
				UpgradeText(
					(Text) inputField.placeholder, prefabInputField ? prefabInputField.placeholder as Text : null) :
		...
		private InputFieldProperties CopyInputFieldProperties(InputField inputField)
		{
			...
			if (inputFieldOnSubmitField == null)
				inputFieldOnSubmitField = typeof(InputField).GetField(
					"m_OnSubmit", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Instance);
		...
		private void PasteInputFieldProperties(TMP_InputField tmp, InputFieldProperties properties)
		{
			...
			if (properties.onSubmit != null && properties.onSubmit.persistentCalls != null)
			{
				...
				var bridgeComponent = tmp.GetComponent<TMP_InputFieldOnSubmitEvent>();
				if (bridgeComponent == null &&
					((IList) unityEventPersistentCallsListField.GetValue(properties.onSubmit.persistentCalls)).Count >
					0)
		...
		private TMP_Dropdown UpgradeDropdown(Dropdown dropdown, Dropdown prefabDropdown)
		{
			...
			// Copy fields
			var sizeDelta = ((RectTransform) dropdown.transform).sizeDelta;
			var properties = CopyDropdownProperties(dropdown);
		...
		private void CreateInputFieldViewport(TMP_InputField tmp)
		{
			...
			var textTransform = tmp.textComponent.rectTransform;
			var placeholderTransform = tmp.placeholder ? tmp.placeholder.rectTransform : null;
			...
			else
			{
				try
				{
					viewport = (RectTransform) new GameObject(
						TMP_INPUT_FIELD_TEXT_AREA_NAME, typeof(RectTransform), typeof(RectMask2D)).transform;
					...
					viewport.GetComponent<RectMask2D>().padding = new Vector4(
						2f - viewport.offsetMin.x, 1f - viewport.offsetMin.y, 2f + viewport.offsetMax.x,
						2f + viewport.offsetMax.y);
#endif
					...
					{
						var child = tmp.transform.GetChild(i) as RectTransform;
								...
								if ((child.anchorMax - child.anchorMin).sqrMagnitude > Mathf.Epsilon)
								{
									...
									if (rectTransformAnchorsSetterMethod == null)
										rectTransformAnchorsSetterMethod = typeof(Editor).Assembly
											.GetType("UnityEditor.RectTransformEditor").GetMethod(
												"SetAnchorSmart",
												BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static,
												null,
												new[]
									...
									rectTransformAnchorsSetterMethod.Invoke(
										null, new object[] {child, 0, 0, false, true, true});
									rectTransformAnchorsSetterMethod.Invoke(
										null, new object[] {child, 1, 0, true, true, true});
									rectTransformAnchorsSetterMethod.Invoke(
										null, new object[] {child, 0, 1, false, true, true});
									rectTransformAnchorsSetterMethod.Invoke(
										null, new object[] {child, 1, 1, true, true, true});
								}
		...
		{
			EditorUtility.DisplayProgressBar(
				"Applying prefab instance modifications...",
			...
			var tmp = properties.gameObject.GetComponent<UpgradedComponentType>();
			if (!tmp)
			{
		...
		private TextOverflowModes GetTMPVerticalOverflow(
		...
		private UnityEventProperties CopyUnityEvent(UnityEventBase target)
		{
			...
			var persistentCalls = Activator.CreateInstance(unityEventPersistentCallsField.FieldType);
			unityEventPersistentCallsListField.SetValue(
				persistentCalls,
				Activator.CreateInstance(
					unityEventPersistentCallsListField.FieldType,
			...
			var targetTypes = new UnityEventProperties.TargetType[target.GetPersistentEventCount()];
			var targetGameObjects = new GameObject[target.GetPersistentEventCount()];
			for (var i = 0; i < targetTypes.Length; i++)
			{
				var component = target.GetPersistentTarget(i) as Component;
				if (!component)
					continue;
		...
		private void PasteUnityEvent(UnityEventBase target, UnityEventProperties unityEventProperties)
		{
			...
			{
				var persistentCallsList =
					(IList) unityEventPersistentCallsListField.GetValue(unityEventProperties.persistentCalls);
				for (var i = 0; i < unityEventProperties.targetTypes.Length; i++)
				{
					var gameObject = unityEventProperties.targetGameObjects[i];
					if (!gameObject)
						continue;
					...
					switch (unityEventProperties.targetTypes[i])
					{
						case UnityEventProperties.TargetType.Text:
							component = gameObject.GetComponent<TextMeshProUGUI>(); break;
						case UnityEventProperties.TargetType.InputField:
							component = gameObject.GetComponent<TMP_InputField>(); break;
						case UnityEventProperties.TargetType.Dropdown:
							component = gameObject.GetComponent<TMP_Dropdown>(); break;
						case UnityEventProperties.TargetType.TextMesh:
							component = gameObject.GetComponent<TextMeshPro>(); break;
					}
