...
#region Updating to new versions
		...
		private static bool UpdateProject(
			TextAnimatorInstallationData installationData, Version oldVersion, bool performUpdate)
		{
...
#region Installation
		...
		private static TextAnimatorInstallationData GetOrCreateInstallationData()
		{
			...
			var data = _CreateScriptableAssetAtPath<TextAnimatorInstallationData>(
...
#region Databases and Tags
		...
		private static void CreateDefaultDatabases(
			string installationFolder, out AnimationsDatabase behaviors, out AnimationsDatabase appearances,
			out ActionDatabase actions, out StyleSheetScriptable styleSheet)
		{
			var progressTitle = "Text Animator";

			// --- DATABASES ---
			EditorUtility.DisplayProgressBar(progressTitle, "Creating Behaviors Database", 1 / 5f);
			behaviors = _CreateDatabase<AnimationsDatabase, AnimationScriptableBase>(
				installationFolder, "Behaviors", "Behaviors Database", EffectCategory.Behaviors);

			EditorUtility.DisplayProgressBar(progressTitle, "Creating Appearances Database", 2 / 5f);
			appearances = _CreateDatabase<AnimationsDatabase, AnimationScriptableBase>(
				installationFolder, "Appearances", "Appearances Database", EffectCategory.Appearances);
			...
			actions = _CreateDatabase<ActionDatabase, ActionScriptableBase>(
				installationFolder, "Actions", "Actions Database", EffectCategory.None);
		...
		public static DatabaseType _CreateDatabase<DatabaseType, ElementType>(
			string installationFolder, string folderName, string fileName, EffectCategory category)
		...
		private static bool TryCreatingDefaultTagScriptable(
			Type type, EffectCategory category, out ScriptableObject result)
		{
			var attribute =
...
#region Settings
		...
		private static TextAnimatorSettings GetOrCreateSettings(string installationFolder)
		{
			...
			return _CreateScriptableAssetAtPath<TextAnimatorSettings>(
				installationFolder + "/Resources", TextAnimatorSettings.expectedName);
		}
		...
		public static void FixSettingsFileNotFound()
		{
			...
			{
				var databaseGuid = AssetDatabase.FindAssets($"t:{nameof(StyleSheetScriptable)}").FirstOrDefault();
				...
				// tries creating new database
				if (TryGetInstallationFolder(out installationFolder)) return CreateStyleSheet(installationFolder);

				return null;
			}

			var settings = GetOrCreateSettings(installationFolder);
			AssignDatabasesToSettings(
				settings,
				GetOrCreateDatabase<AnimationsDatabase, AnimationScriptableBase>(
					"Behaviors", "Behaviors Database", EffectCategory.Behaviors),
				GetOrCreateDatabase<AnimationsDatabase, AnimationScriptableBase>(
					"Appearances",
		...
		private static StyleSheetScriptable CreateStyleSheet(string installationFolder)
		{
			var result = _CreateScriptableAssetAtPath<StyleSheetScriptable>(
