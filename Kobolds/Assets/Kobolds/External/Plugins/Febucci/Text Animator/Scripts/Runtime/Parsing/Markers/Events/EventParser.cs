	...
	public class EventParser : TagParserBase
	{
			...
			int internalOrder)
		{
			...
			if (indexOfEquals != -1)
			{
				...
				//TODO fast strip
				textEvent = new EventMarker(
					eventName, realTextIndex, internalOrder, parameters.Replace(" ", "").Split(','));
			}
			else
			{
				textEvent = new EventMarker(
					textInsideBrackets.Substring(1), realTextIndex, internalOrder, new string[0]);
			}
