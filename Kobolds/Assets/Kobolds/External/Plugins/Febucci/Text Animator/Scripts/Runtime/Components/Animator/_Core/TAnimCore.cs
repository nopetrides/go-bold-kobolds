...
#region Effects and Databases
		...
		{
			get =>
				useDefaultDatabases ? TextAnimatorSettings.Instance.appearances.defaultDatabase : databaseAppearances;
			set
			{
				useDefaultDatabases = false;
		...
		/// </summary>
		/// <remarks>
		///     <seealso cref="IsTypewriterEnabledAtIndex" />
...
#region Default Tags
		...
		private struct DefaultRegion
		{
			...
			public DefaultRegion(
				string tagID, VisibilityMode visibilityMode, AnimationScriptableBase scriptable, string[] tagWords)
			{
...
#region Local Methods
			...
			void AddFallbackEffectsFor<T>(
				ref AnimationRegion[] currentEffects, VisibilityMode visibilityMode, Database<T> database,
				string[] defaultEffectsTags) where T : AnimationScriptableBase
			{
				...
				foreach (var tag in defaultEffectsTags)
				{
					if (string.IsNullOrEmpty(tag))
					{
						if (Application.isPlaying)
							Debug.LogError(
								$"Empty tag as default effect in database {database.name}. Skipping.", gameObject);
						continue;
					}
			...
			var ruleBehavior = new AnimationParser<AnimationScriptableBase>(
				settings.behaviors.openingSymbol, '/', settings.behaviors.closingSymbol, VisibilityMode.Persistent,
				databaseBehaviors);
			var ruleAppearance = new AnimationParser<AnimationScriptableBase>(
				settings.appearances.openingSymbol, '/', settings.appearances.closingSymbol, VisibilityMode.OnVisible,
				databaseAppearances);
			var ruleDisappearance = new AnimationParser<AnimationScriptableBase>(
				settings.appearances.openingSymbol, '/', '#', settings.appearances.closingSymbol,
				VisibilityMode.OnHiding, databaseAppearances);
			var ruleActions = new ActionParser(
				settings.actions.openingSymbol, '/', settings.actions.closingSymbol, databaseActions);
			...
			textWithoutTextAnimTags =
				StyleSheet ? TextParser.ParseText(_text, new StylesParser('<', '/', '>', StyleSheet)) : _text;
			...
			AddFallbackEffectsFor(
				ref appearances, VisibilityMode.OnVisible, databaseAppearances, defaultAppearancesTags);
			AddFallbackEffectsFor(
				ref disappearances, VisibilityMode.OnHiding, databaseAppearances, defaultDisappearancesTags);
		...
		{
			var visible = maxVisibleCharacters;
		...
		public void AppendText(string appendedText, bool hideText = false)
		{
			...
			var previousMaximum = maxVisibleCharacters;
			...
			for (var i = firstVisibleCharacter; i < previousMaximum; i++)
			{
		...
		{
			for (var i = 0; i < CharactersCount; i++) SetVisibilityChar(i, isVisible, canPlayEffects);
		}
