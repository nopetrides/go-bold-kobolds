...
#region Special
		...
		public static Sequence DOJump(
			this Rigidbody2D target, Vector2 endValue, float jumpPower, int numJumps, float duration,
			bool snapping = false)
		{
			...
			Tween yTween = DOTween.To(
					() => target.position, x => target.position = x, new Vector2(0, jumpPower),
					duration / (numJumps * 2))
		...
		)
		{
			...
			var t = DOTween.To(
					PathPlugin.Get(), () => target.position, x => target.MovePosition(x),
