...
#region Material
			...
			this Material target, Vector2 endValue, int propertyID, float duration)
		{
			...
			var t = DOTween.To(
				() => target.GetTextureOffset(propertyID), x => target.SetTextureOffset(propertyID, x), endValue,
				duration);
			t.SetTarget(target);
			return t;
		}
			...
			this Material target, Vector2 endValue, int propertyID, float duration)
		{
			...
			var t = DOTween.To(
				() => target.GetTextureScale(propertyID), x => target.SetTextureScale(propertyID, x), endValue,
				duration);
			t.SetTarget(target);
			return t;
		}
