...
namespace ModelShark
{
	public static class TooltipExtensions
	{
		...
		public static void SetPosition(this Tooltip tooltip, TooltipTrigger trigger, Canvas canvas, Camera camera)
		{
			...
			var triggerRectTrans = trigger.gameObject.GetComponent<RectTransform>();

			// If the Tooltip Position is set to a position on the Canvas, use the Canvas as the trigger rect transform.
			if (trigger.tipPosition == TipPosition.CanvasTopMiddle ||
				trigger.tipPosition == TipPosition.CanvasBottomMiddle)
			...
			tooltip.SetPosition(
				trigger.tipPosition, trigger.tooltipStyle, triggerCorners, tooltip.BackgroundImage,
				tooltip.RectTransform, canvas, camera);
			...
			if (canvas.renderMode == RenderMode.ScreenSpaceCamera || canvas.renderMode == RenderMode.WorldSpace)
				for (var i = 0; i < tooltipCorners.Length; i++)
			...
			var overflow = new TooltipOverflow
			{
			...
			if (overflow.IsAny)
				tooltip.SetPosition(
					overflow.SuggestNewPosition(trigger.tipPosition), trigger.tooltipStyle,
		...
		private static void SetPosition(
			this Tooltip tooltip, TipPosition tipPosition, TooltipStyle style, Vector3[] triggerCorners, Image bkgImage,
			RectTransform tooltipRectTrans, Canvas canvas, Camera camera)
		{
			...
			if (tooltip.TooltipTrigger != null && tooltip.TooltipTrigger.shouldOverridePosition &&
				tooltip.TooltipTrigger.overridePositionType == PositionOverride.Transform)
			{
				var posRectTrans = tooltip.TooltipTrigger.overridePositionTransform.gameObject
					.GetComponent<RectTransform>();
				if (posRectTrans != null)
					posRectTrans.GetWorldCorners(triggerCorners);
				else // We're not using a trigger from a Canvas, so that means it's a world space game object and we need to get the corners from the collider or mesh bounds.
					triggerCorners = GetWorldObjectTriggerCorners(
						tooltip.TooltipTrigger.overridePositionTransform, canvas, camera);
			}

			if (tooltip.TooltipTrigger != null && tooltip.TooltipTrigger.shouldOverridePosition &&
				tooltip.TooltipTrigger.overridePositionType == PositionOverride.Vector)
				triggerCorners[0] = triggerCorners[1] =
					triggerCorners[2] = triggerCorners[3] = tooltip.TooltipTrigger.overridePositionVector;

			// Are we using the mouse position?
			useMousePos = tipPosition == TipPosition.MouseBottomLeftCorner ||
						tipPosition == TipPosition.MouseTopLeftCorner ||
						tipPosition == TipPosition.MouseBottomRightCorner ||
						tipPosition == TipPosition.MouseTopRightCorner
						|| tipPosition == TipPosition.MouseTopMiddle || tipPosition == TipPosition.MouseLeftMiddle ||
						tipPosition == TipPosition.MouseRightMiddle || tipPosition == TipPosition.MouseBottomMiddle;
			var mousePos = GetMousePosition(canvas, camera);
			...
			if (tooltip.TooltipTrigger != null && tooltip.TooltipTrigger.shouldOverridePosition &&
				tooltip.TooltipTrigger.overridePositionType == PositionOverride.MouseCursor)
		...
		{
			var coll = trans.GetComponentInChildren<Collider>();
			var coll2D = trans.GetComponentInChildren<Collider2D>();
		...
		{
			var rend = trans.GetComponentInChildren<Renderer>();
			var skinRend = trans.GetComponentInChildren<SkinnedMeshRenderer>();
			if (rend != null)
			{
			...
			if (skinRend != null)
			{
				var rootBone = skinRend.rootBone;
				if (rootBone != null)
				{
		...
		/// <summary>
		///     Gets the 4 corners of a world space trigger object. (For instance, if you're using a cube 3D object as the
		///     tooltip trigger.)
		/// </summary>
		private static Vector3[] GetWorldObjectTriggerCorners(this Transform trans, Canvas canvas, Camera camera)
		{
			...
			var frontBottomLeftCorner = trans.position;
			var frontTopLeftCorner = trans.position;
			var frontTopRightCorner = trans.position;
			var frontBottomRightCorner = trans.position;
			if (foundExtents)
			{
