...
namespace ModelShark
{
	public class Tooltip
	{
		...
		public void Initialize()
		{
			...
			var textsTMP = GameObject.GetComponentsInChildren<TextMeshProUGUI>(true);
			...
			for (var i = 0; i < textsTMP.Length; i++)
				// If the delimiter wasn't found anywhere in the Text field, don't bother adding it.
				if (textsTMP[i].text.Contains(Delimiter))
					TextFields.Add(new TextField {TextTMP = textsTMP[i], Original = textsTMP[i].text});

			// Get Image fields.
			var dynImgs = GameObject.GetComponentsInChildren<DynamicImage>(true).ToList();
			ImageFields = new List<ImageField>();
			for (var i = 0; i < dynImgs.Count; i++)
			{
				var img = dynImgs[i].GetComponent<Image>();
				ImageFields.Add(
					new ImageField
					{
						Image = img,
			...
			var dynSects = GameObject.GetComponentsInChildren<DynamicSection>(true).ToList();
			SectionFields = new List<SectionField>();
			for (var i = 0; i < dynSects.Count; i++)
			{
				var go = dynSects[i].gameObject;
				SectionFields.Add(
					new SectionField
					{
						GameObject = go,
		...
		/// <summary>
		///     Activates the tooltip and immediately hides all canvas renderers, so the tooltip doesn't "flash" in before
		///     it's positioned and ready.
		/// </summary>
		public void WarmUp()
		{
		...
		public void ResetParameterizedFields()
		{
			...
			for (var i = 0; i < ImageFields.Count; i++)
	...
	public class SectionField
	{
		...
		/// <summary>
		///     The original state of the section, as it began in the scene (did it start on or off?). Used for resetting the
		///     tooltip.
		/// </summary>
