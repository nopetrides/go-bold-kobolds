...
namespace ModelShark
{
		...
		IDeselectHandler, IPointerDownHandler, IPointerUpHandler
	{
		...
		[HideInInspector]
		public bool
			isRemotelyActivated; // Is this tooltip activated from another game object? (ie, NOT "hover" activated)

		[Tooltip("Controls the color and fade amount of the tooltip background.")]
		public Color backgroundTint = Color.white;

		[HideInInspector] [Tooltip(
			"Overrides how the tooltip is positioned. You can choose to have it positioned at a specific Vector point on the screen, have it follow another transform, or have it follow the mouse cursor.")]
		...
		[HideInInspector] [Tooltip(
			"Once open, this tooltip will stay open until the user hovers over another tooltip trigger or something (like a script) manually closes it.")]
		public bool staysOpen;

		[HideInInspector] [Tooltip(
			"If true, this tooltip will not be angled/rotated along with other tooltips (see MatchRotationTo on TooltipManager).")]
		public bool neverRotate;

		[HideInInspector] [Tooltip(
			"While open, this tooltip will prevent any other tooltips from triggering. Something (like a script) will need to manually close it.")]
		...
		private void Update()
		{
			...
			if (Tooltip.TooltipTrigger != null && Tooltip.GameObject.activeInHierarchy && popupTimer <= popupTime &&
				Tooltip != null && shouldOverridePosition)
		...
		private void Initialize()
		{
			...
			if (tooltipStyle != null)
			{
				...
				{
					var ttStyle = Instantiate(tooltipStyle);
					...
					var newTooltip = new Tooltip {GameObject = ttStyle.gameObject};
		...
		/// <summary>
		///     Forces the tooltip to be hidden and deactivated, and also resets all timers so the tooltip won't automatically
		///     re-enable (unless it's triggered again).
		/// </summary>
		...
		public string GetText(string parameterName)
		{
			...
			foreach (var txt in parameterizedTextFields)
			{
		...
		public void SetText(string parameterName, string text)
		{
			...
			var delimiter = TooltipManager.Instance.TextFieldDelimiter;
			parameterizedTextFields.Add(
				new ParameterizedTextField
					{name = parameterName, placeholder = $"{delimiter}{parameterName}{delimiter}", value = text});
		}
		...
		public void SetImage(string parameterName, Sprite sprite)
		{
			...
			var fieldExists = false;
			foreach (var img in dynamicImageFields)
			{
			...
			if (!fieldExists)
				dynamicImageFields.Add(
					new DynamicImageField {name = parameterName, placeholderSprite = null, replacementSprite = sprite});
		}
		...
		private void ToggleSection(string parameterName, bool isOn)
		{
			...
			// Check to see if we find a matching field. If so, set its sprite to what was passed in.
			var fieldExists = false;
			foreach (var section in dynamicSectionFields)
			{
