...
namespace ModelShark
{
	...
	public class TooltipManager : MonoBehaviour
	{
		...
		[Tooltip(
			"The RectTransform to match if you have an angled UI and you want tooltips to match the RectTransform's angle.")]
		...
		[Tooltip(
			"When enabled, tooltips will be triggered by pressing-and-holding, not hovering over. They will be dismissed by releasing the hold, instead of hover off.")]
		...
		[Tooltip(
			"Determines whether tooltips are repositioned when they would flow off the canvas. Disable for increased performance.")]
		public bool overflowProtection = true;

		[Tooltip(
			"For 3D objects, determines whether tooltips are positioned based on the object's collider bounds or mesh renderer bounds.")]
		...
		/// <summary>
		///     Sets the tooltip rotation. Call this anytime your UI rotation has changed, and you want the tooltips to match
		///     it again.
		/// </summary>
		public void ResetTooltipRotation()
		{
			// Rotate the TooltipContainer if there is a specified RectTransform to match the angle to.
			TooltipContainer.transform.rotation = matchRotationTo != null ?
				matchRotationTo.transform.rotation :
				GuiCanvas.transform.rotation;
		}

		/// <summary>Sets the parameterized text fields on the tooltip.</summary>
		/// <remarks>
		///     This is separate from the Show() method because we need to wait a frame or two so the text's RectTransform has
		///     time to update its preferredWidth.
		/// </remarks>
		public void SetTextAndSize(TooltipTrigger trigger)
		{
			...
			var mainTextContainer = tooltip.TooltipStyle.mainTextContainer;
			if (mainTextContainer == null)
				Debug.LogWarning(
					$"No main text container defined on tooltip style \"{trigger.Tooltip.GameObject.name}\". Note: This LayoutElement is needed in order to resize text appropriately.");
			else
				mainTextContainer.preferredWidth = trigger.minTextWidth;

			for (var i = 0; i < tooltip.TextFields.Count; i++)
			{
				var txt = tooltip.TextFields[i].Text;

				var txtTMP = tooltip.TextFields[i].TextTMP;
				if (txtTMP != null)
				{
		...
		private void SetTextAndSizeUI(Text txt, TooltipTrigger trigger, LayoutElement mainTextContainer)
		{
			...
			for (var j = 0; j < trigger.parameterizedTextFields.Count; j++)
				if (!string.IsNullOrEmpty(trigger.parameterizedTextFields[j].value))
					txt.text = txt.text.Replace(
						trigger.parameterizedTextFields[j].placeholder, trigger.parameterizedTextFields[j].value);
			...
			if (mainTextContainer != null)
			{
				...
				else if (txt.preferredWidth > trigger.minTextWidth &&
						txt.preferredWidth > mainTextContainer.preferredWidth)
		...
		public IEnumerator Show(TooltipTrigger trigger)
		{
			...
			{
				Debug.LogWarning(
					"TooltipTrigger \"" + trigger.name + "\" has no associated TooltipStyle. Cannot show tooltip.");
				yield break;
			}

			var tooltip = trigger.Tooltip;
			var tooltipBkgImg = tooltip.BackgroundImage;
			...
			if (trigger.dynamicImageFields != null)
				for (var i = 0; i < trigger.dynamicImageFields.Count; i++)
				{
					for (var j = 0; j < tooltip.ImageFields.Count; j++)
			...
			if (trigger.dynamicSectionFields != null)
				for (var i = 0; i < trigger.dynamicSectionFields.Count; i++)
				{
					for (var j = 0; j < tooltip.SectionFields.Count; j++)
			...
			if (trigger.tipPosition != TipPosition.CanvasBottomMiddle &&
				trigger.tipPosition !=
				TipPosition
					.CanvasTopMiddle) // unless the tooltip will be positioned in a static location, then there is no need.
		...
		public void HideAll()
		{
			...
			var tooltipTriggers = FindObjectsOfType<TooltipTrigger>();
			foreach (var trigger in tooltipTriggers)
				trigger.ForceHideTooltip();
		}

		public List<TooltipStyle> VisibleTooltips()
		{
			var visibleTooltips = new List<TooltipStyle>();
			...
			for (var i = 0; i < tooltipStyles.Length; i++)
				visibleTooltips.Add(tooltipStyles[i]);
			for (var i = 0; i < tooltipStylesNoAngle.Length; i++)
