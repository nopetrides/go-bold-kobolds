...
namespace Lolopupka
{
	...
	{
		[Tooltip(
			"Step distance is used to calculate step height. When character makes a short step there is no need to rase foot all the way up so if the current step distance is less then this step distance value step height will be lover then usual.")]
		...
		[Tooltip(
			"If you want only one leg to move at a time then set Timings offset as one divided by the number of legs. For example: if your character has four legs you need to set this as ¼ = 0.25. The script will offset the cycle of every leg by 0.25 seconds. ")]
		...
		[SerializeField] private AnimationCurve legArcPathY = new(
			new Keyframe(0, 0, 0, 2.5f), new Keyframe(0.5f, 1), new Keyframe(1, 0, -2.5f, 0));
		...
		[SerializeField] private float legRayoffset = 3;
		[SerializeField] private float legRayLength = 6;
		...
		private void Update()
		{
			...
			for (var i = 0; i < nbLegs; ++i)
			{
				...
				legIktargets[i].position = TargetPoint.FitToTheGround(
					lastLegPositions[i], layerMask, legRayoffset, legRayLength, sphereCastRadius);
			}

			//to move legs more frequently when speed is close to max speed
			var cycleSpeedMultiplyer = Remap(velocity.magnitude, 0f, velocityClamp, 1f, 2f);
		...
		{
			if (!showGizmoz || !Application.IsPlaying(this)) return;

			for (var i = 0; i < nbLegs; ++i)
			{
				//target points
				var v = transform.TransformPoint(
						defaultLegPositions[i]) + velocity.normalized *
					Mathf.Clamp(velocity.magnitude, 0, velocityClamp * clampDevider) * velocityMultiplier;
				var v2 = TargetPoint.FitToTheGround(v, layerMask, legRayoffset, legRayLength, sphereCastRadius);
		...
		public void SetUp(int index)
		{
			...
			var v = transform.TransformPoint(
					...
					velocity.normalized * Mathf.Clamp(velocity.magnitude, 0, velocityClamp * clampDevider) *
					velocityMultiplier;
			targetStepPosition[index] = TargetPoint.FitToTheGround(
				v, layerMask, legRayoffset, legRayLength, sphereCastRadius);

			totalDistance[index] = GetDistanceToTarget(index);

			var distance = Vector3.Distance(legIktargets[index].position, targetStepPosition[index]);
			arcHeitMultiply[index] = distance / stepDistance;

			if (targetStepPosition[index] != Vector3.zero && TargetPoint.IsValidStepPoint(
					targetStepPosition[index], layerMask, legRayoffset, legRayLength, sphereCastRadius))
		...
		private IEnumerator MakeStep(Vector3 targetPosition, int index)
		{
			...
			while (current < 1)
			{
				...
				var positionY = legArcPathY.Evaluate(current) * stepHeight * Mathf.Clamp(arcHeitMultiply[index], 0, 1f);

				var desiredStepPosition = new Vector3(
					targetPosition.x,
					positionY + targetPosition.y,
					targetPosition.z);

				legIktargets[index].position = Vector3.Lerp(
					lastLegPositions[index], desiredStepPosition, easingFunction.Evaluate(current));
		...
		private void LegReachedTargetPosition(Vector3 targetPosition, int index)
		{
			...
			if (totalDistance[index] > .3f) OnStepFinished?.Invoke(this, targetPosition);
		...
		{
			var leg = legIktargets[index].position;
			var ray = new Ray(leg + Vector3.up * .1f, -Vector3.up);
			RaycastHit hit;

			if (Physics.Raycast(ray, out hit, layerMask)) return Vector3.Distance(leg, hit.point);
		...
		public float GetAverageLegHeight()
		{
			...
			for (var i = 0; i < nbLegs; ++i) averageHeight += GetDistanceToGround(i);
