...
namespace TMPro.Examples
{
	public class SkewTextExample : MonoBehaviour
	{
		public AnimationCurve VertexCurve = new(
			new Keyframe(0, 0), new Keyframe(0.25f, 2.0f), new Keyframe(0.5f, 0), new Keyframe(0.75f, 2.0f),
			new Keyframe(1, 0f));
			...
			while (true)
			{
				...
				for (var i = 0; i < characterCount; i++)
				{
					...
					var topShear = new Vector3(
						shear_value * (textInfo.characterInfo[i].topRight.y - textInfo.characterInfo[i].baseLine), 0,
						0);
					var bottomShear = new Vector3(
						shear_value * (textInfo.characterInfo[i].baseLine - textInfo.characterInfo[i].bottomRight.y), 0,
						0);
