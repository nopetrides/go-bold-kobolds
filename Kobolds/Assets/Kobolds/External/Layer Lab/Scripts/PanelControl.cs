	...
	public class PanelControl : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (Keyboard.current.leftArrowKey.wasPressedThisFrame)
				Click_Prev();
			else if (Keyboard.current.rightArrowKey.wasPressedThisFrame) Click_Next();
#else
		...
		private void Click_Prev()
		{
			...
			{
				if (otherPanels.Count > 0) _textTitle.text = otherPanels[_page].name;
			}
		...
		private void CheckControl()
		{
			...
			{
				if (otherPanels.Count > 0) _textTitle.text = otherPanels[_page].name.Replace("_", " ");
			}
