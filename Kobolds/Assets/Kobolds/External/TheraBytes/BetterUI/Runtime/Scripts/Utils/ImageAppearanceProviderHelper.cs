...
namespace TheraBytes.BetterUi
{
	...
	{
		public static void SetMaterialType(
			string value,
		...
		public static void SetMaterialEffect(
			MaterialEffect value,
			Graphic graphic, VertexMaterialData materialProperties, ref MaterialEffect effect, ref string materialType)
		{
		...
		public static float GetMaterialPropertyValue(
			int propertyIndex,
			ref float materialProperty1, ref float materialProperty2, ref float materialProperty3)
		{
			if (propertyIndex < 0 || propertyIndex >= 3)
		...
		{
			var cnt = vertexHelper.currentVertCount;
			...
			var uv1 = new Vector2(uvX, uvY);
			var normal = Vector3.back;
			var tangent = new Vector4(1, 0, 0, tangentW);

			vertexHelper.AddVert(
				new Vector3(posMin.x, posMin.y, 0f), colors[0], new Vector2(uvMin.x, uvMin.y),
				uv1, normal, tangent);
			vertexHelper.AddVert(
				new Vector3(posMin.x, posMax.y, 0f), colors[1], new Vector2(uvMin.x, uvMax.y),
				uv1, normal, tangent);
			vertexHelper.AddVert(
				new Vector3(posMax.x, posMax.y, 0f), colors[2], new Vector2(uvMax.x, uvMax.y),
				uv1, normal, tangent);
			vertexHelper.AddVert(
				new Vector3(posMax.x, posMin.y, 0f), colors[3], new Vector2(uvMax.x, uvMin.y),
				uv1, normal, tangent);
		...
		private static Color32 GetColor(ColorMode mode, Color a, Color b, Rect bounds, float x, float y)
		{
			switch (mode)
			{
				...
				{
					var amount = (x - bounds.xMin) / bounds.size.x;
					return Color.Lerp(a, b, amount);
				}

				case ColorMode.VerticalGradient:
				{
					var amount = 1 - (y - bounds.yMin) / bounds.size.y;
					return Color.Lerp(a, b, amount);
				}
