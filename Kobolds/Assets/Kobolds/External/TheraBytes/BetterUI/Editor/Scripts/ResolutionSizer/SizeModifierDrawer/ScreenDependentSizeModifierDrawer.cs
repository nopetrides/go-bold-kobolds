...
namespace TheraBytes.BetterUi.Editor
{
	public abstract class ScreenDependentSizeDrawer<T> : ScreenDependentSizeModifierDrawer
	{
		...
		private static readonly GUIContent removeSizeModsContent = new(
			"Remove Size Modifications",
			"Removes all size modifications of this sizer, so that the 'Current Value' will always be the same as the 'Optimized Size'.");

		private static readonly GUIContent fixCanvasScalerContent = new(
			"Fix Canvas Scaler",
			"Changes the Canvas Scaler mode to 'Constant Pixel Size'. This will affect all elements beneath the root-canvas.");
		...
		public override void OnGUI(Rect rect, SerializedProperty property, GUIContent label)
		{
			...
			EditorGUILayout.LabelField(
				"CurrentValue", GetValueString(sizer.LastCalculatedSize), EditorStyles.boldLabel);
			...
			var isConstantPixelSize = scaler == null || scaler.uiScaleMode == CanvasScaler.ScaleMode.ConstantPixelSize;
			var isProblematic = !isConstantPixelSize && sizer.GetModifiers().Any(o => o.SizeModifiers.Any());
			...
			if (foldout)
			{
				if (isProblematic)
				{
					...
					if (GUILayout.Button(removeSizeModsContent, "minibutton")) RemoveSizeModifiers(obj, sizer);

					if (GUILayout.Button(fixCanvasScalerContent, "minibutton")) FixCanvasScaler(scaler);
...
#region find optimized resolution
				...
				if (obj != null)
				{
					...
					{
						var settings =
							overrideProps.SettingsList.Items.FirstOrDefault(o =>
								o.ScreenConfigName == sizer.ScreenConfigName)
							?? overrideProps.FallbackSettings;

						var parent =
							settings.PropertyIterator()
								.Any(o => o.Mode == OverrideScreenProperties.OverrideMode.Inherit) ?
								overrideProps.GetComponentInParent<OverrideScreenProperties>() :
								null;

						var optWidth = overrideProps.CalculateOptimizedValue(
							settings, OverrideScreenProperties.ScreenProperty.Width, parent);
						var optHeight = overrideProps.CalculateOptimizedValue(
							settings, OverrideScreenProperties.ScreenProperty.Height, parent);
						var optDpi = overrideProps.CalculateOptimizedValue(
							settings, OverrideScreenProperties.ScreenProperty.Dpi, parent);

						screenInfo = new ScreenInfo(new Vector2(optWidth, optHeight), optDpi);
					}
				}

				if (screenInfo == null) screenInfo = ResolutionMonitor.GetOpimizedScreenInfo(sizer.ScreenConfigName);

#endregion

				var opt = string.Format(
					"{0} x {1} @ {2} DPI", screenInfo.Resolution.x, screenInfo.Resolution.y, screenInfo.Dpi);

				EditorGUILayout.LabelField(opt, EditorStyles.boldLabel);

				if (GUILayout.Button("Change"))
					Selection.activeObject = (Object) overrideProps ?? ResolutionMonitor.Instance;
				EditorGUILayout.EndHorizontal();
		...
		private void RemoveSizeModifiers(Component container, ScreenDependentSize<T> sizer)
		{
			...
			foreach (var mod in sizer.GetModifiers()) mod.SizeModifiers.Clear();
		}
		...
		private void ShowOptionalField(
			SerializedProperty parentProp, string propName, string boolPropName, string displayName, ref T value)
		{
		...
		protected virtual void ShowField(
			SerializedProperty parentProp, string propName, string displayName, ref T value)
		{
		...
		public class SizeModifierCollectionDrawer : PropertyDrawer
		{
			...
			private ReorderableList GetList(SerializedProperty property)
			{
				if (list == null)
				{
					...
					list.elementHeight = EditorGUIUtility.singleLineHeight + 4;
					list.drawHeaderCallback = rect => { EditorGUI.LabelField(rect, "Size Modifiers"); };
					list.drawElementCallback = (rect, index, isActive, isFocused) =>
					{
						var element = list.serializedProperty.GetArrayElementAtIndex(index);
