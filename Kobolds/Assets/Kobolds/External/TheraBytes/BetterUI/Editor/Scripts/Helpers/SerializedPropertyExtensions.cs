...
namespace TheraBytes.BetterUi.Editor
{
	...
	public static class SerializedPropertyExtensions
	{
		...
		public static T GetNestedObject<T>(string path, object obj, bool includeAllBases = true)
		{
			...
			var regex = new Regex(@"([a-zA-Z\.]+.)?Array.data\[(\d+)\]");
			path = regex.Replace(path, "$1$2");

			foreach (var part in path.Split('.')) obj = GetFieldOrPropertyValue<object>(part, obj, includeAllBases);
			return (T) obj;
		}

		public static T GetFieldOrPropertyValue<T>(
			string fieldName, object obj, bool includeAllBases = true,
