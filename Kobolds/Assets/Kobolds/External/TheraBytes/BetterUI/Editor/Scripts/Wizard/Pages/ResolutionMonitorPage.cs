...
namespace TheraBytes.BetterUi.Editor
{
	public class ResolutionMonitorPage : WizardPage
	{
		...
		protected override void OnInitialize()
		{
			...
			optimizedResolutionElement = new ValueWizardPageElement<Vector2>(
				"optimizedResolution",
				(o, v) =>
				{
					if (v == Vector2.zero) v = new Vector2(1920, 1080);
			...
			isFallbackLandscapeElement = new ValueWizardPageElement<bool>(
				"isFallbackLandscape",
				(o, v) =>
				{
					EditorGUILayout.BeginHorizontal();
			...
			supportBothOrientationsElement = new ValueWizardPageElement<bool>(
			...
			responsiveDesignElement = new ValueWizardPageElement<int>(
				"responsiveDesign",
				(o, v) =>
				{
					var containsSmall = (v & SMALL_SCREEN) == SMALL_SCREEN;
					var containsBig = (v & BIG_SCREEN) == BIG_SCREEN;
			...
			{
				var suffix = ResolutionMonitor.ScriptableObjectFileExists ? " (overwrite existing)" : "";
			...
			Add(new SeparatorWizardPageElement());
			Add(
				new CustomWizardPageElement(o =>
				{
					EditorGUILayout.HelpBox(
						"You have configured your Resolution Monitor. " +
		...
		private void GenerateResolutionMonitor()
		{
			...
			var lRes = optimizedResolutionElement.Value;
			if (lRes.x < lRes.y) lRes = new Vector2(lRes.y, lRes.x);

			var pRes = new Vector2(lRes.y, lRes.x);

			// fallback
			var isLandscape = isFallbackLandscapeElement.Value;
			if (isLandscape)
			{
			...
			var supportBoth = supportBothOrientationsElement.Value;
			if (supportBoth) alt = CreateScreenTypeCondition(!isLandscape, "", lRes, pRes);
			// added later
			// Responsive Design
			var containsSmall = (responsiveDesignElement.Value & SMALL_SCREEN) == SMALL_SCREEN;
			var containsBig = (responsiveDesignElement.Value & BIG_SCREEN) == BIG_SCREEN;
			...
			if (containsSmall)
			{
				...
				if (smallAlt != null) smallAlt.Fallbacks.Add(smallMain.Name);
			}
			...
			if (containsBig)
			{
				...
				if (bigAlt != null) bigAlt.Fallbacks.Add(bigMain.Name);
			}

			Debug.Log("Resolution Monitor has been generated successfully.");
		}

		private ScreenTypeConditions CreateScreenTypeCondition(
			bool isLandscape, string nameSuffix, Vector2 lRes, Vector2 pRes,
			IsScreenOfCertainSize sizeInfo = null)
		{
			var name = isLandscape ? "Landscape" : "Portrait";

			name += nameSuffix;

			var sct = new ScreenTypeConditions(name, typeof(IsCertainScreenOrientation));

			sct.OptimizedScreenInfo.Resolution = isLandscape ? lRes : pRes;
		...
		{
			if (o.State == WizardElementState.WaitForInput && GUILayout.Button("Continue...")) o.MarkComplete();
		}
	}
}
