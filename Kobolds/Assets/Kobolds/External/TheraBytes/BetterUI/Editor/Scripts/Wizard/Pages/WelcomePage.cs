		...
		protected override void OnInitialize()
		{
			...
			Add(
				new InfoWizardPageElement(
					"Welcome to the setup wizard of Better UI!\n" +
					"It will guide you configuring Better UI properly for your project."));

			var skippingIsRecommended = false;
			if (data.FileExists() && data.SavedDataCount > 1)
			{
				Add(
					new InfoWizardPageElement(
						"It seems like you did the setup wizard for this project already.\n" +
			...
			else if (ResolutionMonitor.ScriptableObjectFileExists)
			{
				Add(
					new InfoWizardPageElement(
						"It seems like you already have Better UI prepared for your project.\n" +
			...
			{
				Add(
					new UnserializedButtonPageElement(
						"Jump to Third-Party-Support Page",
						() => wizard.JumpToPage<ThirdPartySupportPage>()));

#if UNITY_2017_3_OR_NEWER
				Add(
					new UnserializedButtonPageElement(
						"Jump to Assembly-Definitions Page",
						() => wizard.JumpToPage<AssemblyDefinitionsPage>()));
#endif

				Add(
					new UnserializedButtonPageElement(
						"Jump to Example-Scenes Page",
			...
			Add(
				new ValueWizardPageElement<bool>(
						"SkipWizard",
						(o, v) =>
						{
							if (GUILayout.Button("Skip Wizard"))
								if (skippingIsRecommended
									|| EditorUtility.DisplayDialog(
										"Skip Wizard", "Do you really want to skip the wizard?", "Yes", "No"))
								{
									EditorUtility.DisplayDialog(
										"Wizard Skipped",
