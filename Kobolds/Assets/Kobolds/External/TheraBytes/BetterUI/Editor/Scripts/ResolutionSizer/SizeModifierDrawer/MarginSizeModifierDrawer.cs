...
namespace TheraBytes.BetterUi.Editor
{
	...
	public class MarginSizeModifierDrawer : ScreenDependentSizeDrawer<Margin>
	{
		...
		protected override void ShowField(
			SerializedProperty parentProp, string propName, string displayName, ref Margin value)
		{
			if (!foldouts.ContainsKey(displayName)) foldouts.Add(displayName, false);

			var title = string.Format("{0} {1}", displayName, GetValueString(value));
			...
			if (foldouts[displayName])
			{
				...
				var right = prop.FindPropertyRelative("right");
				EditorGUILayout.PropertyField(right);
				var top = prop.FindPropertyRelative("top");
				EditorGUILayout.PropertyField(top);
				var bottom = prop.FindPropertyRelative("bottom");
