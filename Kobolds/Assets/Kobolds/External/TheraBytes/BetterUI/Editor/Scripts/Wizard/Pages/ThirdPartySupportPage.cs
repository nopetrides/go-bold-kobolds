...
namespace TheraBytes.BetterUi.Editor
{
	public class ThirdPartySupportPage : WizardPage
	{
		...
		protected override void OnInitialize()
		{
			...
			{
				int v1, v2, v3;
				var foundTmpVersion = TryGetTextMethProPackageVersion(out v1, out v2, out v3);
				if (foundTmpVersion)
				{
					var versionString = string.Format("{0}.{1}.{2}", v1, v2, v3);
					...
					if (v1 > 3 || (v1 == 3 && v2 > 0) || (v1 == 3 && v2 == 0 && v3 > 6))
						Add(
							new InfoWizardPageElement(
								"The latest tested TextMesh Pro version is 3.0.6.\n" +
					...
					else
					{
						...
						var packageName = isNewBaseClass ? PACKAGE_PATH_TMP_NEW : PACKAGE_PATH_TMP_OLD;
										...
										wizard.DoReloadOperation(
											this, () =>
											{
												AssetDatabase.ImportPackage(
													Path.Combine(Application.dataPath, packageName), false);
												v = packageName;
											});
									return v;
								}).MarkComplete());
				...
				else
				{
					...
					Add(
						new InfoWizardPageElement(
							"You may install the right Better UI add-on now " +
					...
					Add(
						new InfoWizardPageElement(
							"Please select the add on for the text mesh pro version you have installed."));

#if UNITY_2020_1_OR_NEWER
					var textNewVersion = "v3.0 or above";
					var textOldVersion = "below v3.0";
#elif UNITY_2019_4_OR_NEWER
							...
							(o, v) =>
							{
								if (GUILayout.Button("Import Add-On (TextMesh Pro " + textNewVersion + ")"))
									wizard.DoReloadOperation(
										...
										{
											AssetDatabase.ImportPackage(
												Path.Combine(Application.dataPath, PACKAGE_PATH_TMP_NEW), false);
								...
								if (GUILayout.Button("Import Add-On (TextMesh Pro " + textOldVersion + ")"))
									wizard.DoReloadOperation(
										...
										{
											AssetDatabase.ImportPackage(
												Path.Combine(Application.dataPath, PACKAGE_PATH_TMP_OLD), false);
		...
		private bool TryGetTextMethProPackageVersion(out int v1, out int v2, out int v3)
		{
			...
			var folder = Path.Combine(Path.GetDirectoryName(Application.dataPath), "Packages");
