...
namespace TheraBytes.BetterUi.Editor.AlignDistribute
{
	public class AlignDistributeWindow : EditorWindow
	{
		...
		{
			var selectionStatus = Utility.IsSelectionValid();
			...
			else
			{
				...
				var label = selection.Length == 1 ?
					selection[0].name :
					string.Format("{0} UI Elements", selection.Length);
		...
		private void DrawAlignButtons()
		{
			...
			if (GUILayout.Button(
					new GUIContent(alignLeft, "Align to the left"), GUILayout.Width(60f), GUILayout.Height(60f)))
				Align.AlignSelection(AlignMode.Left, alignTo);
			if (GUILayout.Button(
					new GUIContent(alignCenter, "Align to the center"), GUILayout.Width(60f), GUILayout.Height(60f)))
				Align.AlignSelection(AlignMode.Vertical, alignTo);
			if (GUILayout.Button(
					new GUIContent(alignRight, "Align to the right"), GUILayout.Width(60f), GUILayout.Height(60f)))
			...
			if (GUILayout.Button(
					new GUIContent(alignTop, "Align to the top"), GUILayout.Width(60f), GUILayout.Height(60f)))
				Align.AlignSelection(AlignMode.Top, alignTo);
			if (GUILayout.Button(
					new GUIContent(alignMiddle, "Align to the middle"), GUILayout.Width(60f), GUILayout.Height(60f)))
				Align.AlignSelection(AlignMode.Horizontal, alignTo);
			if (GUILayout.Button(
					new GUIContent(alignBottom, "Align to the bottom"), GUILayout.Width(60f), GUILayout.Height(60f)))
		...
		private void DrawDistributeButtons()
		{
			...
			if (GUILayout.Button(
					new GUIContent(distributeHorizontal, "Distribute horizontally"), GUILayout.Width(60f),
					GUILayout.Height(60f)))
			{
				if (Selection.GetTransforms(SelectionMode.Unfiltered).Length > 1)
					Distribute.DistributeSelection(
						AlignMode.Horizontal, distanceOption, sortOrder,
			...
			if (GUILayout.Button(
					new GUIContent(distributeVertical, "Distribute vertically"), GUILayout.Width(60f),
					GUILayout.Height(60f)))
			{
				if (Selection.GetTransforms(SelectionMode.Unfiltered).Length > 1)
					Distribute.DistributeSelection(
						AlignMode.Vertical, distanceOption, sortOrder,
						paddingLeftBottomPixels, paddingRightTopPixels);
				else
