...
namespace FIMSpace.Jiggling
{
	...
	/// </summary>
	public abstract class FJiggling_Base : MonoBehaviour, IDropHandler, IFHierarchyIcon
	{
		...
		public string EditorIconPath => "Jiggling/Jiggling Icon";

		/// <summary>
		///     Method to initialize component, to have more controll than waiting for Start() method, init can be executed before
		///     or after start, as programmer need it.
		/// </summary>
			...
			else
			{
				...
				for (var i = 0; i < RandomLevel; i++)
				{
					for (var t = 0; t < 2; t++)
					{
						var newParams = new TrigonoParams();
						newParams.Randomize();
						trigParams[i + t].Multiplier = Mathf.Lerp(
							trigParams[i + t].Multiplier, newParams.Multiplier, transition);
						trigParams[i + t].RandomTimeMul = Mathf.Lerp(
							trigParams[i + t].RandomTimeMul, newParams.RandomTimeMul, transition);
						trigParams[i + t].TimeOffset = Mathf.Lerp(
							trigParams[i + t].TimeOffset, newParams.TimeOffset, transition);
		...
		{
			var finishingSafeRange = currentJigglePower - 0.01f;
			if (finishingSafeRange <= 0f) finishingSafeRange = 0f;
			...
			for (var i = 0; i < RandomLevel; i++)
			{
				for (var j = 0; j < 2; j++)
				{
					var parameters = trigParams[i + j];

					var timeValue = time * parameters.RandomTimeMul + parameters.TimeOffset;
					var multiplyValue = (JiggleTiltValue * powerMultiplier + addTilt) * parameters.Multiplier *
						finishingSafeRange / AdditionalSpeedValue;
