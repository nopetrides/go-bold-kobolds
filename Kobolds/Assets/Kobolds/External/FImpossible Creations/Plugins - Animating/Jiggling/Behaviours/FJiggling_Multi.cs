...
namespace FIMSpace.Jiggling
{
	...
	public class FJiggling_Multi : FJiggling_Base
	{
		...
		[Tooltip(
			"Calculating individual randomization variables for each element of chain, less optimal but can provide more interesting effects (it's cheap in cpu anyway)")]
		...
		{
			/* Don't do anything in Update() we will use LateUpdate() to support also animated models jiggling animation */
		}
		...
		private void LateUpdate()
		{
			...
			if (reJiggled)
			{
				if (reJiggleProgress > 0f)
				{
					...
					var targetValue =
						Mathf.Sin(
							time * 1.45f * trigParams[1].RandomTimeMul + reJiggleRandomOffset +
							trigParams[0].TimeOffset + trigParams[1].TimeOffset) * JiggleTiltValue *
						trigParams[0].Multiplier * reJiggleProgress * ReJigglePower;
			...
			if (BonesNotAnimatedByAnimator)
			{
				if (!SeparatedCalculations)
					for (var i = 0; i < ToJiggle.Count; i++)
					{
		...
		protected override void Init()
		{
			...
			for (var i = 0; i < ToJiggle.Count; i++)
			{
				if (ToJiggle[i].Transform == null) continue;
		...
		{
			var jigglE = ToJiggle[index];
			var t = jigglE.Transform;

			var val1 = 0f;
			var val2 = 0f;

			for (var i = 0; i < RandomLevel * 2; i++)
				if (i % 2 == 0) val1 += jigglE.trigParams[i].Value;
				else val2 += jigglE.trigParams[i].Value;
			...
			t.transform.localRotation *= Quaternion.Euler(
				(val1 + reJiggleValue + add1) * jigglE.RotationAxesMul.x * RotationAxesMultiplier.x,
				(-val2 + reJiggleValue - add1) * jigglE.RotationAxesMul.y * RotationAxesMultiplier.y,
				(val2 + reJiggleValue + add2) * jigglE.RotationAxesMul.z * RotationAxesMultiplier.z);
			t.transform.localScale += new Vector3(
				trigParams[0].Value * jigglE.ScaleAxesMul.x * ScaleAxesMultiplier.x,
				(trigParams[0].Value + trigParams[1].Value) / 2f * jigglE.ScaleAxesMul.y * ScaleAxesMultiplier.y,
				trigParams[0].Value * jigglE.ScaleAxesMul.z * ScaleAxesMultiplier.z) * 0.01f;
		}


		protected void CalculateTrigonoFor(int i)
		{
			for (var r = 0; r < RandomLevel; r++)
			{
				for (var j = 0; j < 2; j++)
				{
					var parameters = ToJiggle[i].trigParams[r + j];

					var timeValue = time * parameters.RandomTimeMul + parameters.TimeOffset;
					var multiplyValue = JiggleTiltValue * parameters.Multiplier * currentJigglePower /
										AdditionalSpeedValue;

					if (j % 2 == 0) parameters.Value = Mathf.Sin(timeValue) * multiplyValue;
					else parameters.Value = Mathf.Cos(timeValue) * multiplyValue;
				}
			}
		}
		...
		protected override void RandomizeVariables(float transition = 1f)
		{
			if (!SeparatedCalculations)
			{
				...
				for (var j = 0; j < ToJiggle.Count; j++) ToJiggle[j].trigParams = trigParams;
			}
			else
			{
				base.RandomizeVariables(transition);
				...
				if (transition >= 1f)
					for (var j = 0; j < ToJiggle.Count; j++)
					{
						...
						for (var i = 0; i < RandomLevel; i++)
						{
							for (var t = 0; t < 2; t++)
							{
								var newParams = new TrigonoParams();
								newParams.Randomize();
					...
					for (var j = 0; j < ToJiggle.Count; j++)
					{
						for (var i = 0; i < RandomLevel; i++)
						{
							for (var t = 0; t < 2; t++)
							{
								var newParams = new TrigonoParams();
								newParams.Randomize();
								ToJiggle[j].trigParams[i + t].Multiplier = Mathf.Lerp(
									ToJiggle[j].trigParams[i + t].Multiplier, newParams.Multiplier, transition);
								ToJiggle[j].trigParams[i + t].RandomTimeMul = Mathf.Lerp(
									ToJiggle[j].trigParams[i + t].RandomTimeMul, newParams.RandomTimeMul, transition);
								ToJiggle[j].trigParams[i + t].TimeOffset = Mathf.Lerp(
									ToJiggle[j].trigParams[i + t].TimeOffset, newParams.TimeOffset, transition);
								ToJiggle[j].trigParams[i + t].Value = Mathf.Lerp(
									ToJiggle[j].trigParams[i + t].Value, newParams.Value, transition);
							}
						}
					}
			}
...
#region List Support Methods
		...
		public void AddNewElement(FJiggling_Element el)
		{
			...
			if (el.Transform == null)
				canAdd = true;
			else if (!ToJiggle.Contains(el))
				canAdd = true;

			if (canAdd) ToJiggle.Add(el);
		}
