...
namespace FIMSpace.GroundFitter
{
	...
	public class FGroundFitter_Movement : MonoBehaviour
	{
		...
		private static PhysicsMaterial pm_Sliding;
		...
		[Tooltip(
			"You need collider and rigidbody on object to make it work right - ALSO CHANGE YOUR CAMERA UPDATE CLOCK TO FIXED UPDATE AND USE TIME.fixedDeltaTime - ! For now it can cause errors when jumping, character can go through floor sometimes ! - Will be upgraded in future versions")]
		[Header("(experimental)")]
		public bool UsePhysics;

		[Tooltip(
			"Disabling translating object from code and running animation without need to hold minimum movement speed")]
		...
		protected virtual void Start()
		{
			...
			if (string.IsNullOrEmpty(SetIsGroundedParam) == false)
			...
			if (UseRootMotionTranslation)
			{
				...
				if (animator.gameObject != gameObject)
					if (!animator.applyRootMotion)
						if (!animator.GetComponent<FGroundFitter_RootMotionHelper>())
							animator.gameObject.AddComponent<FGroundFitter_RootMotionHelper>().MovementController =
								this;
		...
		protected virtual void HandleGravity()
		{
			...
			if (inAir)
			{
				...
				if (NotFallingThrough)
					if (inAir)
					{
						var dir = fitter.transform.forward;
						var scale = fitter.RaycastCheckRange;
						//if( itsCollider) scale = itsCollider.bounds.extents.magnitude;

						if (Physics.Raycast(
								fitter.GetRaycastOrigin() - dir * scale * 0.1f, dir, scale * 1.11f,
								fitter.GroundLayerMask, QueryTriggerInteraction.Ignore))
						{
							var pre = fitter.RaycastCheckRange;
		...
		protected virtual void HandleTransforming()
		{
			...
			if (fitter.enabled)
			{
				if (fitter.LastRaycast.transform)
				{
					var nPos = fitter.LastRaycast.point + fitter.UpOffset * Vector3.up;
			...
			if (MoveVector != Vector3.zero)
			{
				if (!UseRootMotionRotation)
				{
					if (!fitter.enabled)
					{
						fitter.UpAxisRotation = Mathf.LerpAngle(
							fitter.UpAxisRotation, Camera.main.transform.eulerAngles.y + RotationOffset,
	...
	[CustomEditor(typeof(FGroundFitter_Movement))]
	public class FGroundFitter_MovementEditor : Editor
	{
		private FGroundFitter_Movement _get;
		private bool displayParams;

		private SerializedProperty sp_SetIsGroundedParam;

		public FGroundFitter_Movement Get
		{
			get
			{
				if (_get == null) _get = (FGroundFitter_Movement) target;
				return _get;
			}
		}
		...
		public override void OnInspectorGUI()
		{
			...
			displayParams = EditorGUILayout.Foldout(displayParams, " Extra Animator Params", true);

			if (displayParams)
			{
				GUILayout.Space(4);

				var sp = sp_SetIsGroundedParam.Copy();
				...
				if (anim)
					if (!AnimatorHasParam(anim, FGroundFitter_Movement._hash_animSp))
						...
						{
							var aContr = (AnimatorController) anim.runtimeAnimatorController;
							aContr.AddParameter(
								new AnimatorControllerParameter
								{
									defaultFloat = 1f, name = "AnimationSpeed",
		...
		{
			var aContr = (AnimatorController) anim.runtimeAnimatorController;
			foreach (var p in aContr.parameters)
				if (p.nameHash == hash)
					return true;
