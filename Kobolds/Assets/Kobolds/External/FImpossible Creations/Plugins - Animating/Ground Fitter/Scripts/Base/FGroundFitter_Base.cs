...
{
	/// <summary>
	///     FM: Base class for ground fitter components, it contains variables needed for all ground fitter component in shared
	///     way
	/// </summary>
...
#region < Main Variables >
		...
		[Space(3f)]
		[Tooltip(
			"If you want this script only to change your object's rotation and do nothing with position, untoggle this")]
...
#region < Tweaking Variables >
		...
		[Tooltip(
			"Maximum rotation angle in rotation of x/pitch axis, so rotating forward - degrees value of maximum rotation")]
		[Range(0f, 90f)]
		...
		[Tooltip(
			"Max roll rotation. If rotation should work on also on x axis - good for spiders, can look wrong on quadropeds etc.")]
		[Range(0f, 90f)]
...
#region < Zone cast Variables >
		...
		[Space(8f)]
		[Tooltip(
			"Casting more raycsts under object to detect ground more precisely, then we use average from all casts to set new rotation")]
		public bool ZoneCast;

		public Vector2 ZoneCastDimensions = new(0.3f, 0.5f);
...
#region < Rest of the variables >
		...
		/// <summary>
		///     With this variable you define in which direction model should look - use it fro other scripts, check demo
		///     script for patrolling")]
		/// </summary>
		[NonSerialized] public float UpAxisRotation;
...
#region Rotation & Physical Calculations
		...
		protected virtual void FitToGround()
		{
			...
			if (LookAheadRaycast != 0f)
				Physics.Raycast(
					TransformToRotate.position + GetUpVector(RaycastHeightOffset) +
					TransformToRotate.forward * LookAheadRaycast, -GetUpVector(), out aheadHit, RaycastCheckRange,
		...
		internal virtual void RotationCalculations()
		{
			...
			var targetRotation = helperRotation;

			targetRotation = Quaternion.Euler(
				Mathf.Clamp(
					FLogicMethods.WrapAngle(targetRotation.eulerAngles.x), -MaxForwardRotation, MaxForwardRotation) *
				(1 - MildForwardValue), targetRotation.eulerAngles.y,
				Mathf.Clamp(
					FLogicMethods.WrapAngle(targetRotation.eulerAngles.z), -MaxHorizontalRotation,
					MaxHorizontalRotation) * (1 - MildHorizontalValue));

			if (TotalSmoother == 0f)
			{
				targetRotationToApply = targetRotation * Quaternion.AngleAxis(UpAxisRotation, Vector3.up);
			}
			else
			{
				var yRotation = Quaternion.AngleAxis(UpAxisRotation, Vector3.up);
				targetRotationToApply *= Quaternion.Inverse(yRotation);
				targetRotationToApply = Quaternion.Slerp(
					targetRotationToApply, targetRotation, deltaTime * Mathf.Lerp(50f, 1f, TotalSmoother));
				targetRotationToApply *= yRotation;
			}

			if (ApplyRotation) TransformToRotate.rotation = targetRotationToApply;
		...
		internal virtual RaycastHit CastRay()
		{
			...
			Physics.Raycast(
				LastRaycastOrigin, -GetUpVector(), out outHit, RaycastCheckRange + Mathf.Abs(UpOffset), GroundLayerMask,
				QueryTriggerInteraction.Ignore);

			// Making zone cast calculations to create average ray hit
			if (ZoneCast)
			{
				var p = TransformToRotate.position + GetRotation() * ZoneCastOffset + GetUpVector(RaycastHeightOffset);
				var x = TransformToRotate.right * ZoneCastDimensions.x;
				var z = TransformToRotate.forward * ZoneCastDimensions.y;
				...
				var c = 0;
				var part = 1f;
				for (var i = 0; i < Mathf.Lerp(4, 24, ZoneCastPrecision); i++)
				{
					...
					Physics.Raycast(
						p + sum / part, -GetUpVector() + sum * ZoneCastBias + castOffset, out newHit,
						RaycastCheckRange + Mathf.Abs(UpOffset), GroundLayerMask, QueryTriggerInteraction.Ignore);
				...
				if (!outHit.transform) outHit.point = new Vector3(avPos.x, TransformToRotate.position.y, avPos.z);
			}
...
#region Utilities
		...
		protected virtual Vector3 GetUpVector(float mulRange = 1f)
		{
			if (RelativeLookUp)
				return Vector3.Lerp(
							WorldUp, TransformToRotate.TransformDirection(Vector3.up).normalized, RelativeLookUpBias) *
						mulRange;
...
#region Gizmos Stuff
		...
		protected virtual void OnDrawGizmos()
		{
			...
			var scale = RaycastCheckRange;
			Gizmos.DrawRay(
				TransformToRotate.position + GetUpVector(RaycastHeightOffset), GetUpVector() * -RaycastCheckRange);
			Gizmos.DrawRay(
				TransformToRotate.position + TransformToRotate.TransformVector(Vector3.one * 0.001f * scale) +
				GetUpVector(RaycastHeightOffset), GetUpVector() * -RaycastCheckRange);
			Gizmos.DrawRay(
				TransformToRotate.position + TransformToRotate.TransformVector(-Vector3.one * 0.001f * scale) +
				GetUpVector(RaycastHeightOffset), GetUpVector() * -RaycastCheckRange);
			Gizmos.DrawRay(
				TransformToRotate.position + TransformToRotate.TransformVector(Vector3.one * 0.002f * scale) +
				GetUpVector(RaycastHeightOffset), GetUpVector() * -RaycastCheckRange);
			Gizmos.DrawRay(
				TransformToRotate.position + TransformToRotate.TransformVector(-Vector3.one * 0.002f * scale) +
				GetUpVector(RaycastHeightOffset), GetUpVector() * -RaycastCheckRange);
			Gizmos.DrawRay(
				TransformToRotate.position + TransformToRotate.TransformVector(-Vector3.one * 0.005f * scale) +
				GetUpVector(RaycastHeightOffset), GetUpVector() * -RaycastCheckRange);
			Gizmos.DrawRay(
				TransformToRotate.position + TransformToRotate.TransformVector(-Vector3.one * 0.005f * scale) +
				GetUpVector(RaycastHeightOffset), GetUpVector() * -RaycastCheckRange);
			...
			if (LookAheadRaycast != 0f)
			{
				...
				Gizmos.DrawRay(
					TransformToRotate.position + GetUpVector(RaycastHeightOffset) +
					TransformToRotate.forward * LookAheadRaycast, -GetUpVector() * RaycastCheckRange /*+ YOffset*/);
			}
			...
			if (ZoneCast)
			{
				...
				var downDir = -GetUpVector() * RaycastCheckRange;
...
#region Custom Inspector Window Stuff
	...
	/// </summary>
	[CustomEditor(typeof(FGroundFitter_Base))]
	[CanEditMultipleObjects]
	public class FGroundFitter_BaseEditor : Editor
	{
		public override void OnInspectorGUI()
		{
			...
			if (!targetScript.RelativeLookUp) exclude.Add("RelativeLookUpBias");
