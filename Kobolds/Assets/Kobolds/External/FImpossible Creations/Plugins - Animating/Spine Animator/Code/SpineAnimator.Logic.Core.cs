...
namespace FIMSpace.FSpine
{
	public partial class FSpineAnimator
	{
		...
		[Tooltip(
			"You can use this variable to blend intensity of spinal animator motion over skeleton animation\n\nValue = 1: Animation with spinal Animator motion\nValue = 0: Only skeleton animation")]
		public float SpineAnimatorAmount = 1f;

		private readonly Quaternion Rotate180 = Quaternion.Euler(0f, 180f, 0f);
		...
		{
			var otherBone = SpineBones[index - chainIndexDirection];
			var currentBone = SpineBones[index];

			// Target position calculation
			var targetPosition = otherBone.ProceduralPosition - currentBone.ProceduralRotation *
				ModelForwardAxisScaled * (currentBone.BoneLength * DistancesMultiplier);
...
#region Springiness Stuff
			...
			if (Springiness > 0f)
				if (!LastBoneLeading)
				{
					var backPosDiff = currentBone.ProceduralPosition - currentBone.PreviousPosition;
					var newPos = currentBone.ProceduralPosition;
					currentBone.PreviousPosition = currentBone.ProceduralPosition;
					newPos += backPosDiff * (1 - Mathf.Lerp(.05f, .25f, Springiness));

					var restDistance = (otherBone.ProceduralPosition - newPos).magnitude;

					var otherLocalToWorld = otherBone.transform.localToWorldMatrix;
					otherLocalToWorld.SetColumn(3, otherBone.ProceduralPosition);
					var restPos = otherLocalToWorld.MultiplyPoint3x4(currentBone.transform.localPosition);

					var diffPosVector = restPos - newPos;
					newPos += diffPosVector * Mathf.Lerp(0.05f, 0.2f, Springiness);

					diffPosVector = restPos - newPos;
					var distance = diffPosVector.magnitude;
					var maxDistance = restDistance * (1 - Mathf.Lerp(0.0f, 0.2f, Springiness)) * 2;
					if (distance > maxDistance) newPos += diffPosVector * ((distance - maxDistance) / distance);

					if (MaxStretching < 1f)
					{
						var dist = Vector3.Distance(currentBone.ProceduralPosition, newPos);
						if (dist > 0f)
						{
							var maxDist = currentBone.BoneLength * 4 * MaxStretching;
							if (dist > maxDist)
			...
			if (PosSmoother > 0f)
				if (MaxStretching < 1f)
				{
					var dist = Vector3.Distance(currentBone.ProceduralPosition, targetPosition);
					if (dist > 0f)
					{
						var maxDist = currentBone.BoneLength * 4 * MaxStretching;
						if (dist > maxDist)
							currentBone.ProceduralPosition = Vector3.Lerp(
								currentBone.ProceduralPosition, targetPosition, Mathf.InverseLerp(dist, 0f, maxDist));
					}
				}
			...
			if (PosSmoother == 0f)
				...
				currentBone.ProceduralPosition = Vector3.LerpUnclamped(
					currentBone.ProceduralPosition, targetPosition, Mathf.LerpUnclamped(1f, unifiedDelta, PosSmoother));
		}
		...
		private void CalculateTargetBoneRotation(int index)
		{
			...
			if (Slithery >= 1f)
				...
				backRotationRef = Quaternion.LerpUnclamped(
					currentBone.ReferenceRotation, otherBone.ProceduralRotation, Slithery);
			else
				backRotationRef = currentBone.ReferenceRotation;


			// Initial target rotation calculation
			//Vector3 towards = (otherBone.ProceduralPosition+BaseTransform.TransformVector(MainPivotOffset) ) - currentBone.ProceduralPosition;
			var towards = otherBone.ProceduralPosition - currentBone.ProceduralPosition;
...
#region Calculations to limit rotations in order to adjust animation behaviour for project needs
			...
			if (AngleLimit < 91)
			{
				var lookDiff = Quaternion.Angle(targetLookRotation, backRotationRef);
				...
				if (lookDiff > AngleLimit)
				{
					...
					var limitRange = Quaternion.LerpUnclamped(targetLookRotation, backRotationRef, limiting);

					var elasticPush = Mathf.Min(1f, lookDiff / (AngleLimit / 0.75f));
					elasticPush =
						Mathf.Sqrt(Mathf.Pow(elasticPush, 4)) *
						elasticPush; // sqrt and power will make this value increase slower but reaching 1f at the end
					...
					if (LimitSmoother == 0f)
						...
						targetLookRotation = Quaternion.LerpUnclamped(
							targetLookRotation, limitRange, unifiedDelta * (1f - LimitSmoother) * 50f * elasticPush);
				}
			}
			...
			if (GoBackSpeed <= 0f)
			{
				...
				if (StraightenSpeed > 0f)
				{
					...
					var diff = (currentBone.ReferencePosition - currentBone.PreviousReferencePosition).magnitude /
								currentBone.GetUnscalledBoneLength();
					if (diff > 0.5f) diff = 0.5f;
					var target = diff * (1f + StraightenSpeed / 5f);

					currentBone.StraightenFactor = Mathf.Lerp(
						currentBone.StraightenFactor, target, unifiedDelta * (7f + StraightenSpeed));

					if (diff > 0.0001f)
						targetLookRotation = Quaternion.Lerp(
							targetLookRotation, backRotationRef,
							unifiedDelta * currentBone.StraightenFactor * (StraightenSpeed + 5f) *
							(TurboStraighten ? 6f : 1f));
				}
			}
			else // When we set GoBackSpeed variable spine is going back to straight pose continously so diff would be detected all the time and we don't want this
			{
				...
				if (StraightenSpeed > 0f)
				{
					...
					currentBone.StraightenFactor = Mathf.Lerp(
						currentBone.StraightenFactor, currentBone.TargetStraightenFactor,
						unifiedDelta * (1f + StraightenSpeed));
				...
				targetLookRotation = Quaternion.Lerp(
					targetLookRotation, backRotationRef,
					unifiedDelta * (Mathf.Lerp(0f, 55f, GoBackSpeed) + straightenVal));
			}
			...
			if (RotSmoother == 0f) currentBone.ProceduralRotation = targetLookRotation;
			else
				currentBone.ProceduralRotation = Quaternion.LerpUnclamped(
		...
		private void RefreshReferencePose()
		{
			if (HeadAnchor)
				if (!AnimateAnchor)
					SpineBones[leadingBoneIndex].transform.localRotation =
						SpineBones[leadingBoneIndex].InitialLocalRotation;
			...
			if (LastBoneLeading) // Head bone is last bone in chain
			{
				...
				if (LeadBoneRotationOffset.sqrMagnitude != 0f)
					if (LeadBoneOffsetReference)
				...
				for (var i = SpineBones.Count - 2; i >= 0; i--)
				{
					...
					SpineBones[i].UpdateReferencePosition(
						SpineBones[i + 1].ReferencePosition - SpineBones[i].ReferenceRotation * ModelForwardAxis *
						(SpineBones[i].BoneLength * DistancesMultiplier * BaseTransform.lossyScale.x));
				}
			}
