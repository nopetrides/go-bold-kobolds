...
namespace FIMSpace.FSpine
{
	public partial class FSpineAnimator
	{
		...
		{
			return new Vector3(
				(float) Math.Round(pos.x, digits), (float) Math.Round(pos.y, digits),
				(float) Math.Round(pos.z, digits));
		}


		/// <summary>
		///     Rounding fix correction angles to nearest values, we calculate axes directions in precise way, but in most cases
		///     rounded are doing job much better
		/// </summary>
		private Vector3 RoundToBiggestValue(Vector3 vec)
		{
			...
			else if (biggest == 1) vec = new Vector3(0f, Mathf.Round(vec.y), 0f);
			else
