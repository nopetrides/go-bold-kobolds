...
namespace FIMSpace.FSpine
{
	public partial class FSpineAnimator
	{
		...
		public partial class SpineBone
		{
			...
			/// <summary>
			///     Bone length - distance to child bone calculated at game start. SetDistanceForFrame...() used for directional
			///     definition
			/// </summary>
...
#region Preparations
			...
			public void PrepareBone(Transform baseTransform, List<SpineBone> bones, int index)
			{
				...
				if (index == 0)
					nextPos = bones[index + 1].transform
						.position; // else nextPos = bones[index - 1].transform.position;
				...
				var dist = Vector3.Distance(
					baseTransform.InverseTransformPoint(nextPos),
					baseTransform.InverseTransformPoint(bones[index].transform.position));
				...
				boneLengthB = (baseTransform.InverseTransformPoint(transform.position) -
								baseTransform.InverseTransformPoint(nextPos)).magnitude;
				boneLocalOffsetB = baseTransform.InverseTransformPoint(nextPos);

				boneLengthF = (baseTransform.InverseTransformPoint(transform.position) -
								baseTransform.InverseTransformPoint(nextPos)).magnitude;
			...
			{
				if (boneLengthF > boneLengthB) return boneLengthF;
				return boneLengthB;
			}
...
#region Detecting parent and child at the end of the array
				...
				var forwardInBoneOrientation = parentTr.InverseTransformDirection(childPos) -
												parentTr.InverseTransformDirection(parentPos);
				var projectedUp = Vector3.ProjectOnPlane(
					baseTransform.up, transform.TransformDirection(forwardInBoneOrientation).normalized).normalized;
				var upInBoneOrientation = parentTr.InverseTransformDirection(parentPos + projectedUp) -
										parentTr.InverseTransformDirection(parentPos);
				var crossRight = Vector3.Cross(
					transform.TransformDirection(forwardInBoneOrientation),
					transform.TransformDirection(upInBoneOrientation));
				var rightInBoneOrientation = parentTr.InverseTransformDirection(parentPos + crossRight) -
											parentTr.InverseTransformDirection(parentPos);
...
#region Playmode Methods
			...
			internal void CalculateDifferencePose(Vector3 upAxis, Vector3 rightAxis)
			{
				...
				var fixedBendRotation = ProceduralRotation * Quaternion.FromToRotation(up, upAxis) *
										Quaternion.FromToRotation(right, rightAxis);
				var fixedRefRotation = ReferenceRotation * Quaternion.FromToRotation(up, upAxis) *
										Quaternion.FromToRotation(right, rightAxis);
