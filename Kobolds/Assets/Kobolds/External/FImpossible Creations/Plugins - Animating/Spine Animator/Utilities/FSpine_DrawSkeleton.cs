...
namespace FIMSpace.FSpine
{
	...
	public class FSpine_DrawSkeleton : MonoBehaviour
	{
		...
		[Range(0f, 1f)]
		public float OrientationsRayLength;
		...
		private int recurrention;
		private int recurrentionSegment;
		...
		public void DrawBonesRecurent(Transform start)
		{
			...
			{
				var newCol = Color.HSVToRGB(0.5f + Mathf.Sin(recurrentionSegment / ColorBias) / 2f, 0.7f, 0.8f);
				newCol.a = Alpha;
				Gizmos.color = newCol;
			}

			if (Selection.Contains(start.gameObject))
			{
