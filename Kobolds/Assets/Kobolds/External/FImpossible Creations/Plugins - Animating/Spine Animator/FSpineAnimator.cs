...
namespace FIMSpace.FSpine
{
	...
	///     Component name will remain named as FSpineAnimator to avoid component break on different filename/in file name in
	///     case of updating plugin.
	/// </summary>
	...
	public partial class FSpineAnimator : MonoBehaviour, IDropHandler, IFHierarchyIcon
	{
...
#region Debug "`" disable key for editor only
			...
			if (OptimizeWithMesh != null)
				if (OptimizeWithMesh.isVisible == false)
				{
					updateSpineAnimator = false;
					return;
				}
...
#region Head bones Variables
		...
		[Tooltip(
			"Main character object - by default it is game object to which Spinal Animator is attached.\n\nYou can use it to control spine of character from different game object.")]
		public Transform BaseTransform;

		/// <summary>
		///     ForwardReference is no more used, it remains here for compatibility import to new version. Use BaseTransform
		///     now
		/// </summary>
		public Transform ForwardReference;

		[Obsolete(
			"Use SpineAnimatorAmount instead, but remember that it works in reversed way -> SpineAnimatorAmount 1 = BlendToOriginal 0  and  SpineAnimatorAmount 0 = BlendToOriginal 1")]
		public float BlendToOriginal
		{
			get => 1f - SpineAnimatorAmount;
			set => SpineAnimatorAmount = 1f - value;
		}


		[Tooltip(
			"If your spine lead bone is in beggining of your hierarchy chain then toggle it.\n\nComponent's gizmos can help you out to define which bone should be leading (check head gizmo when you switch this toggle).")]
		public bool LastBoneLeading = true;

		public bool EndBoneIsHead
		{
			get => LastBoneLeading;
			set => LastBoneLeading = EndBoneIsHead;
		}

		[Tooltip(
			"Sometimes spine chain can face in different direction than desired or you want your characters to move backward with spine motion.")]
		...
		[Tooltip(
			"Connecting lead bone position to given transform, useful when it is tail and you already animating spine with other Spinal Animator component.")]
		...
		[Tooltip(
			"List of bone positioning/rotation fixers if using paws positioning with IK controlls disconnected out of arms/legs in the hierarchy")]
		public List<SpineAnimator_FixIKControlledBones> BonesFixers = new();

		[Tooltip(
			"Useful when you use few spinal animators and want to rely on animated position and rotation by other spinal animator.")]
		...
		[Tooltip(
			"Often when you drop model to scene, it's initial pose is much different than animations, which causes problems, this toggle solves it at start.")]
		public bool StartAfterTPose = true;

		[Tooltip(
			"If you want spinal animator to stop computing when choosed mesh is not visible in any camera view (editor's scene camera is detecting it too)")]
		...
		[Tooltip(
			"In some cases you need to use chain corrections, it will cost a bit more in performance, not much but always.")]
		public bool UseCorrections;

		[Tooltip(
			"Sometimes offsetting model's pivot position gives better results using spinal animator, offset forward axis so front legs are in centrum and see the difference (generating additional transform inside hierarchy)")]
		public Vector3 MainPivotOffset = new(0f, 0f, 0f);
...
#region Animation settings and limitations
		...
		[Range(0f, 1f)]
		[Tooltip(
			"If animation of changing segments rotation should be smoothed - making it more soft, but don't overuse it!")]
		public float RotSmoother;

		[Range(0f, 1f)]
		[Tooltip(
			"We stretching segments to bigger value than bones are by default to create some extra effect which looks good but sometimes it can stretch to much if you using position smoothing, you can adjust it here.")]
		public float MaxStretching = 1f;

		[Tooltip(
			"Making algorithm referencing back to static rotation if value = 0f | at 1 motion have more range and is more slithery.")]
		[Range(0f, 1f)]
		...
		[Tooltip(
			"Useful when your creature jumps on moving platform, so when platform moves spine is not reacting, by default world space is used (null).")]
		public Transform MotionSpace;

		[Tooltip(
			"Fade rotations to sides or rotation up/down with this parameter - can be helpful for character jump handling")]
		...
		[Tooltip(
			"<! Most models can not need this !> Offset for bones rotations, thanks to that animation is able to rotate to segments in a correct way, like from center of mass.")]
		public Vector3 SegmentsPivotOffset = new(0f, 0f, 0f);
		...
		public bool FlattenReferencePose;
...
#region Physics Experimental Stuff
		...
		[Tooltip(
			"If disabled Colliders can be offsetted a bit in wrong way - check pink spheres in scene view (playmode, with true positions disabled colliders are fitting to stiff reference pose) - but it gives more stable collision projection! But to avoid stuttery you can increase position smoothing.")]
		...
		[SerializeField] [HideInInspector] private bool _CheckedPivot;
