		...
		public void CheckIfCanUpdate()
		{
			...
			if (UnscaledDelta)
			{
				if (!AnimatePhysics)
				{
					...
					if (Time.timeScale == 0f) smoothDelta = 0f;
					else smoothDelta = Time.smoothDeltaTime / Time.timeScale;
				}
				else
				{
...
#region Performance Measure END
		...
		private void Motion_MotionInfluence()
		{
			...
			if (InfluenceAxes != Vector3.one)
			{
				...
				var localOffset = CompensationTransform.InverseTransformDirection(influenceOffset);

				localOffset = Vector3.Scale(
					localOffset, new Vector3(1f - InfluenceAxes.x, 1f - InfluenceAxes.y, 1f - InfluenceAxes.z));
