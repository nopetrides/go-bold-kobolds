...
namespace FIMSpace.BonesStimulation
{
	public partial class BonesStimulator
	{
		...
		{
			var newValue = Mathf.SmoothDamp(
				StimulatorAmount, targetAmount, ref sd_amount, duration, Mathf.Infinity, Time.deltaTime);
		...
		private IEnumerator IEUser_FadeStimulator(
			float targetBlend, float duration, float delay, bool disableComponent = false)
		{
			if (delay > 0f) yield return new WaitForSeconds(delay);

			if (duration > 0f)
			{
				var preAmount = StimulatorAmount;
