...
namespace FIMSpace.FEditor
{
	public class FChannelInserter : EditorWindow
	{
		...
		private void OnGUI()
		{
			...
			From = (Texture2D) EditorGUILayout.ObjectField(
				From == null ? "Get Channel From:" : "Get Channel From: " + From.name, From, typeof(Texture2D), false);
			...
			if (From != null && texture != null)
			{
				...
				EditorGUI.DrawPreviewTexture(
					rect, From, null, ScaleMode.ScaleToFit, 1f, 0, EChannelToWriteChannel(ChannelFrom));
				EditorGUILayout.LabelField("Replacing this: ");
				rect = GUILayoutUtility.GetRect(40, 40);
				EditorGUI.DrawPreviewTexture(
					rect, texture, null, ScaleMode.ScaleToFit, 1f, 0, EChannelToWriteChannel(ApplyTo));
				EditorGUILayout.EndHorizontal();
				EditorGUIUtility.labelWidth = 0;
			}
			...
			if (From != null)
			{
				...
				if (From.width != texture.width || From.height != texture.height)
				{
					...
					EditorGUILayout.HelpBox(
						"Dimensions of the textures are not equal. Algorithm will generate copy of texture and scale it to fit target texture.",
						MessageType.Info);
				}

				if (GUILayout.Button(
						(ApplyTo == EChannelSelect.A ? "(Requires A channel) " : "") + "Insert '" + ChannelFrom +
						"' to '" + ApplyTo + "' channel of " + texture.name))
				{
					if (dimensionsDiffer)
						FTextureEditorToolsMethods.ScaleTextureFile(
							From, tfrom, new Vector2(texture.width, texture.height));
				...
				{
					if (dimensionsDiffer)
						tfrom = FTextureEditorToolsMethods.GenerateScaledTexture2DReference(
							From, new Vector2(texture.width, texture.height), 4, true);

					var duplicated = FTextureEditorToolsMethods.DuplicateAsPNG(
						texture, "-PNG", true, ApplyTo == EChannelSelect.A);
		...
		{
			var window = (FChannelInserter) GetWindow(typeof(FChannelInserter));
		...
		private ColorWriteMask EChannelToWriteChannel(EChannelSelect channel)
		{
			switch (channel)
			{
				case EChannelSelect.R: return ColorWriteMask.Red;
				case EChannelSelect.G: return ColorWriteMask.Green;
				case EChannelSelect.B: return ColorWriteMask.Blue;
				case EChannelSelect.A: return ColorWriteMask.Alpha;
			}
		...
		{
			var srcImporter = FTextureEditorToolsMethods.GetTextureAsset(source);
			var srcInfo = FTextureEditorToolsMethods.GetTextureInfo(srcImporter, source);

			var tgtImporter = FTextureEditorToolsMethods.GetTextureAsset(target);
			...
			try
			{
				...
				var newPixels = target.GetPixels32();
			...
			catch (Exception exc)
			{
				...
				EditorUtility.ClearProgressBar();
				Debug.LogError("[Fimpo Image Tools Something went wrong when channeling textures! " + exc);
			}
		}
		...
		public Color32 SwapChannel(Color32 source, Color32 target, EChannelSelect from, EChannelSelect to)
		{
			...
			switch (from)
			{
				...
				case EChannelSelect.RGB:
					tgt = (byte) Mathf.Min((source.r + source.g + source.b) / 3, byte.MaxValue); break;
			}
