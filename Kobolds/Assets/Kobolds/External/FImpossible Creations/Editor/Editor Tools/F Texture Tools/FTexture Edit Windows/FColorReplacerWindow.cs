...
namespace FIMSpace.FTextureTools
{
	...
	{
		public Texture2D TexturizeWith;

		private EColorMode ColorMode = EColorMode.HueShift;
		private float EffectBlend = 1f;
		private float ExtraMatching;

		private float HueMatching = 0.75f;

		private float HueOffset = 0.5f;
		private EMaskExport maskExport = EMaskExport.None;

		private Color MaskingColor = Color.white;
		private float SaturationMatching;
		private float SaturationOffset;
		private bool TexturizeIsReadable;
		private Texture2D texturizeMemo;
		private float TexturizeTiling = 1f;

		private Color ToReplaceColor = Color.red;
		private Color ToReplaceHSV;
		private float ValueMatching;
		private float ValueOffset;

		public static void Init()
		{
			var window = (FColorReplacerWindow) GetWindow(typeof(FColorReplacerWindow));
			window.titleContent = new GUIContent(
				"Color Replacer", FTextureToolsGUIUtilities.FindIcon("SPR_ColorReplace"),
				"Replace certain color on your texture into other");
		...
		protected override void OnGUICustom()
		{
			...
			var highRange = HueMatching > 1f ? 3f : 1.00001f;
			HueMatching = EditorGUILayout.Slider(
				new GUIContent("Hue Matching:"), HueMatching, 0f, highRange, GUILayout.MinWidth(360));
			EditorGUIUtility.labelWidth = 45;

			SaturationMatching = EditorGUILayout.Slider(
				new GUIContent("Sat:"), SaturationMatching, 0f, 1f, GUILayout.MinWidth(200));
			ValueMatching = EditorGUILayout.Slider(
				new GUIContent("Val:"), ValueMatching, 0f, 1f, GUILayout.MinWidth(200));
			EditorGUIUtility.labelWidth = 54;
			ExtraMatching = EditorGUILayout.Slider(
				new GUIContent("Extra:", "Matching factor will be reduced if pixel saturation or value is low."),
				ExtraMatching, 0f, 1f, GUILayout.MinWidth(140));
			...
			if (ColorMode == EColorMode.HueShift)
			{
				...
				SaturationOffset = EditorGUILayout.Slider(
					new GUIContent("Sat:"), SaturationOffset, -1f, 1f, GUILayout.MinWidth(200));
				ValueOffset = EditorGUILayout.Slider(
					new GUIContent("Val:"), ValueOffset, -1f, 1f, GUILayout.MinWidth(200));
				EditorGUIUtility.labelWidth = 0;
				GUILayout.EndHorizontal();
			}
			else if (ColorMode == EColorMode.ColorMasking)
			{
				...
				if (GUILayout.Button(
						new GUIContent(
							" Export", FTextureToolsGUIUtilities.FindIcon("SPR_GrayscaleS"),
							"Export mask as grayscale .png file"), GUILayout.MaxWidth(70)))
				{
					var newFile = GenerateExtraFile();
					if (newFile != null)
					{
				...
				if (GUILayout.Button(
						new GUIContent(
							" Export", FTextureToolsGUIUtilities.FindIcon("SPR_rgbscale"),
							"Export mask as color channel .png file"), GUILayout.MaxWidth(70)))
				{
					var newFile = GenerateExtraFile();
					if (newFile != null)
					{
...
#region Texturize options
				...
				GUILayout.Space(-34);
				TexturizeWith = (Texture2D) EditorGUILayout.ObjectField(
					GUIContent.none, TexturizeWith, typeof(Texture2D), false /*, GUILayout.Height(18)*/);
				...
				if (TexturizeWith != null)
				{
					...
					var path = AssetDatabase.GetAssetPath(TexturizeWith);
					...
					if (tImp is null == false)
					{
						...
						if (GUILayout.Button(
								"Switch readonly for '" + TexturizeWith.name + "' to " + (!tImp.isReadable)))
						{
						...
						if (srcTex)
							if (TexturizeWith.width != srcTex.width)
								EditorGUILayout.HelpBox(
									"Texture sizes differs, preview can look not as precize!", MessageType.None);
					}
...
#region Preparing variables to use down below
			...
			{
				Debug.LogError(
					"[SEAMLESS GENERATOR] Source texture is different scale or target texture! Can't create seamless texture!");
				return;
			}
			...
			if (TexturizeWith != null)
				if (TexturizeIsReadable && ColorMode == EColorMode.Texturize)
				{
					var targetWidth = Mathf.RoundToInt(source.width / TexturizeTiling);
					var targetHeight = Mathf.RoundToInt(source.height / TexturizeTiling);
					if (targetWidth < 1) targetWidth = 1;
					if (targetHeight < 1) targetHeight = 1;

					if (texturizeMemo == null || texturizeMemo.width != targetWidth ||
						texturizeMemo.height != targetHeight)
						texturizeMemo = FTextureEditorToolsMethods.GenerateScaledTexture2DReference(
							TexturizeWith, new Vector2(targetWidth, targetHeight), preview ? 2 : 4);
...
#region Replacing Texture Color
			...
			var dim = new Vector2(source.width, source.height);
			...
			if (ColorMode == EColorMode.HueShift)
				for (var x = 0; x < source.width; x++)
				{
					...
					{
						var px = GetPX(x, y, dim);
						Color sourcePx = sourcePixels[px];
						var blend = CalculateSimilarity(sourcePx);
						...
						var tgtColor = Color.HSVToRGB(
							h, Mathf.Clamp01(s + SaturationOffset * blend), Mathf.Clamp01(v + ValueOffset * blend));
				...
				for (var x = 0; x < source.width; x++)
				{
					for (var y = 0; y < source.height; y++)
					{
						...
						var tgtColor = sourcePx;
						if (texturizeSource && TexturizeIsReadable)
							tgtColor = texturizePixels[
								GetPXLoopSkipEdges(
									randX + x, randY + y, new Vector2(texturizeSource.width, texturizeSource.height))];
			...
			if (!preview)
				EditorUtility.DisplayProgressBar(
					"Replacing Texture Color...", "Applying Color Replacement to Texture... ", 4f / 5f);
		...
		private float CalculateSimilarity(Color color)
		{
			float h, s, v;
			Color.RGBToHSV(color, out h, out s, out v);

			var hueDifference = Mathf.Min(Mathf.Abs(ToReplaceHSV.r - h), 1 - Mathf.Abs(ToReplaceHSV.r - h));
			var hueSimilarity = Mathf.Pow(1f - hueDifference, HueMatching * 32f);
			var finalSimilarity = hueSimilarity;

			var satDifference = Mathf.Min(Mathf.Abs(ToReplaceHSV.g - s), 1 - Mathf.Abs(ToReplaceHSV.g - s));
			var satSimilarity = Mathf.Pow(1f - satDifference, SaturationMatching * 16f);
			...
			var valDifference = Mathf.Min(Mathf.Abs(ToReplaceHSV.b - v), 1 - Mathf.Abs(ToReplaceHSV.b - v));
			var valSimilarity = Mathf.Pow(1f - valDifference, ValueMatching * 8f);
			...
			if (ExtraMatching > 0f)
			{
				var satThresh = Mathf.Lerp(0.05f, 0.4f, ExtraMatching);
				var valThresh = Mathf.Lerp(0.05f, 0.2f, ExtraMatching);
				var reduction = Mathf.InverseLerp(0f, satThresh, s);
