...
namespace FIMSpace.FTextureTools
{
	public class FSeamlessWindow : FTextureProcessWindow
	{
		...
		public bool StampWithOtherTexture;
		public Texture2D StampWith;
		...
		private readonly FEStampMode stampMode = FEStampMode.Stamping;
		private float stampNoiseMask = 1.0f;
		private int stampRotate = 1;
		private bool StampWithReadable;
		...
		{
			var window = (FSeamlessWindow) GetWindow(typeof(FSeamlessWindow));
			window.titleContent = new GUIContent(
				"Seamless Generator", FTextureToolsGUIUtilities.FindIcon("SPR_SeamlessGenSmall"),
				"Stamp sides of the texture to make it tile");
		...
		protected override void OnGUICustom()
		{
			...
			GUILayout.Space(4);
			seed = EditorGUILayout.IntSlider(
				new GUIContent("  Seed:", FTextureToolsGUIUtilities.FindIcon("FRandomIcon")), seed, -50, 50);
			...
			if (stampMode != FEStampMode.NoStamping)
			{
				...
				if (stampNoiseMask > 1.35f)
					EditorGUILayout.HelpBox("Stamp Noise greater than 1 can reveal seams, beware!", MessageType.None);
				...
				if (stampRotate > 1f)
					EditorGUILayout.HelpBox("Rotation greater than 1 can create non precise pixels!", MessageType.None);
				toLoop = (FESeamlessAxis) EditorGUILayout.EnumPopup(new GUIContent("Dimensions to loop"), toLoop);

				GUILayout.Space(4);

				EditorGUILayout.BeginHorizontal();
				StampWithOtherTexture = EditorGUILayout.Toggle(
					new GUIContent(
						"Use Other Texture:",
						"Using other texture as stamps source. Can be useful for stamping textures with stripes."),
					StampWithOtherTexture);

				if (StampWithOtherTexture)
					StampWith = (Texture2D) EditorGUILayout.ObjectField(
						GUIContent.none, StampWith, typeof(Texture2D), false /*, GUILayout.Height(18)*/);
				...
				if (StampWithOtherTexture)
				{
					if (StampWith != null)
					{
						var path = AssetDatabase.GetAssetPath(StampWith);
						...
						if (tImp is null == false)
						{
							...
							if (GUILayout.Button(
									"Switch readonly for '" + StampWith.name + "' to " + (!tImp.isReadable)))
							{
							...
							if (srcTex)
								if (StampWith.width != srcTex.width)
...
#region Preparing variables to use down below
			...
			var doX = toLoop != FESeamlessAxis.X;
			var doY = toLoop != FESeamlessAxis.Y;
...
#region Stamping Texture
			...
			var stampRadiusX = source.width * Mathf.LerpUnclamped(0.05f, .3f, stamperRadius);
			var stampRadiusY = source.height * Mathf.LerpUnclamped(0.05f, .3f, stamperRadius);

			var stampsOffsetX = stampRadiusX * Mathf.LerpUnclamped(1.45f, 0.45f, stampDensity);
			var stampsOffsetY = stampRadiusY * Mathf.LerpUnclamped(1.45f, 0.45f, stampDensity);

			var stampWith = source;
			var stampWithPixels = sourcePixels;

			if (StampWith != null)
				if (StampWithReadable)
				{
					if (preview == false)
					{
						if (StampWith.width != target.width || StampWith.height != target.height)
						{
							stampWith = FTextureEditorToolsMethods.GenerateScaledTexture2DReference(
								StampWith, new Vector2(target.width, target.height));
			...
			if (doX)
			{
				var stampsCountX = (int) (source.width / stampsOffsetX);
				for (var x = 0; x <= stampsCountX; x++)
				{
					var stamp = GetStamp(stampWith, stampWithPixels, (int) stampRadiusX);
					var pastePos = new Vector2(0, 0);
			...
			if (doY)
			{
				var stampsCountY = (int) (source.width / stampsOffsetY);
				for (var y = 0; y <= stampsCountY; y++)
				{
					var stamp = GetStamp(stampWith, stampWithPixels, (int) stampRadiusY);
					var pastePos = new Vector2(0, 0);
			...
			if (!preview)
				EditorUtility.DisplayProgressBar(
					"Generating Seamless Texture...", "Applying Seamless Texture... ", 3.85f / 5f);
...
#region Texture Small Operations
		...
		{
			var randD = -0.2 + rand.NextDouble() * 1.2;
			if (randomize > 0)
			{
				var tRad = (int) ((float) radius * (1f + randD * (randomize * 1f)));
				if (radius < source.width && radius < source.height) radius = tRad;
			}

			var width = radius * 2;
			var stampPixels = new Color32[width * width];
			...
			var stampDim = new Vector2(width, width);
			var dimensions = GetDimensions(source);

			var randomOff = (float) rand.NextDouble() * radius * 512f;
			...
			for (var x = 0; x < width; x++)
			{
				for (var y = 0; y < width; y++)
				{
					var xx = -radius + x;
					var yy = -radius + y;
					var i = GetPX(x, y, stampDim);
					stampPixels[i] = sourcePixels[GetPX((int) origin.x + xx, (int) origin.y + yy, dimensions)];

					var distance = Vector2.Distance(new Vector2(xx, yy), Vector2.zero);
					var fadeMul = distance / (radius * 0.95f);
					stampPixels[i].a = Convert.ToByte(Mathf.Min(255, Mathf.Lerp(255 + hardness * 215, 0, fadeMul)));
					...
					if (stampNoiseMask > 0f)
						if (stampPixels[i].a < 235 + hardness * 15 + stampNoiseMask * 10)
						{
							var noise = Mathf.PerlinNoise(
								(float) x / radius * 3f + randomOff, (float) y / radius * 3f + randomOff);

							var spreadAlpha = Mathf.Lerp(1f, 0f, Mathf.InverseLerp(255, 0, stampPixels[i].a));

							var noiseMask = stampNoiseMask;
							if (stampNoiseMask > 1f)
							{
								var tA = Mathf.Lerp(
									255 + Mathf.LerpUnclamped(hardness * 215, 0, noiseMask - 1f), 0, fadeMul);
								spreadAlpha = Mathf.Lerp(1f, 0f, Mathf.InverseLerp(255, 0, tA));
								noiseMask -= 1f;
							}

							var noiseAlpha = Mathf.LerpUnclamped(1f, noise, noiseMask * 0.95f);
			...
			// Rotating
			if (stampRotate >= 1f) RotateImage(stampPixels, width, width, RandomRange(0, stampRotate));
		...
		{
			for (var x = 0; x < toPasteDim.x; x++)
			{
				for (var y = 0; y < toPasteDim.y; y++)
				{
					var index = GetPXLoop(
						(int) (origin.x - toPasteDim.x / 2 + x), (int) (origin.y - toPasteDim.y / 2 + y), targetDim);
					var toP = GetPX(x, y, toPasteDim);
