...
namespace FIMSpace.FEditor
{
	public static class FResizeTools_MenuItems
	{
			...
			try
			{
				for (var i = 0; i < Selection.objects.Length; i++)
				{
					...
					EditorUtility.DisplayProgressBar(
						"Scaling textures...", "Scaling texture " + texture.name, i / (float) Selection.objects.Length);

					if (texture != null)
						FTextureEditorToolsMethods.ScaleTextureFile(
							texture, texture,
							new Vector2(
								FTex_Methods.FindNearestPowOf2(texture.width),
								FTex_Methods.FindNearestPowOf2(texture.height)));
				}
				...
				for (var i = 0; i < Selection.objects.Length; i++)
				{
					...
					if (texture != null)
						FTextureEditorToolsMethods.ScaleTextureFile(
							texture, texture,
							new Vector2(
								FTex_Methods.FindLowerPowOf2(texture.width),
								FTex_Methods.FindLowerPowOf2(texture.height)));
				}
				...
				for (var i = 0; i < Selection.objects.Length; i++)
				{
					...
					if (texture != null)
						FTextureEditorToolsMethods.ScaleTextureFile(
							texture, texture,
							new Vector2(
								FTex_Methods.FindHigherPowOf2(texture.width),
								FTex_Methods.FindHigherPowOf2(texture.height)));
				}
		...
		public static bool CheckResizeTextureAllSelected()
		{
			...
			for (var i = 0; i < Selection.objects.Length; i++) // We need just one file to be texture to return true
			{
				var tex = AssetImporter.GetAtPath(AssetDatabase.GetAssetPath(Selection.objects[i]));
