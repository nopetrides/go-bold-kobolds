...
namespace FIMSpace.FEditor
{
	public class FQuickResizeWindow : EditorWindow
	{
				...
				try
				{
					for (var i = 0; i < textures.Count; i++)
					{
						EditorUtility.DisplayProgressBar(
							"Scaling textures...", "Scaling texture " + textures[i].name, i / (float) textures.Count);
		...
		{
			var window = (FQuickResizeWindow) GetWindow(typeof(FQuickResizeWindow));
