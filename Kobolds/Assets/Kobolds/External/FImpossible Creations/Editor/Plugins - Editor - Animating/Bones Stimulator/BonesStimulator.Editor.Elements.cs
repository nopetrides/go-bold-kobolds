...
namespace FIMSpace.BonesStimulation
{
	public partial class BonesStimulator_Editor
	{
		...
		private void El_Distance(SerializedProperty dprop)
		{
			...
			else
			{
				...
				else
				{
					...
					if (Camera.main) camTr = Camera.main.transform;
					var fieldTr = (Transform) EditorGUILayout.ObjectField(
						new GUIContent(
							"  ",
							"If you want measure fade out distance from other object than main camera put it here"),
						camTr, typeof(Transform), true);
			...
			if (Application.isPlaying)
				if (Get.FadeOutDistance > 0f)
				{
					...
					EditorGUILayout.LabelField(
						"Current Distance: " + Math.Round(Get.CurrentFadeDistance, 2) + " Blend: " +
						Mathf.Round(Get.GetEffectBlendWeight() * 100) + "%");
					GUI.color = c;
				}
		}
		...
		{
			if (Get.IncludedColliders == null) Get.IncludedColliders = new List<Collider>();
			GUILayout.Space(1f);
			GUI.color = new Color(0.85f, 1f, 0.85f, 1f);
			EditorGUILayout.BeginHorizontal(FGUI_Resources.HeaderBoxStyleH);
			var f = FGUI_Resources.GetFoldSimbol(drawInclud);
			var inclC = Get.IncludedColliders.Count;
			GUI.color = c;

			GUILayout.Label(new GUIContent(" "), GUILayout.Width(1));
			var inclColFoldTitle = "";

			inclColFoldTitle = Lang("Collide With") + " (" + (inclC == 0 ? "0 !!!" : inclC.ToString()) + ")";

			if (GUILayout.Button(
					new GUIContent(" " + f + "  " + inclColFoldTitle, FGUI_Resources.TexBehaviourIcon),
					FGUI_Resources.FoldStyle, GUILayout.Height(24))) drawInclud = !drawInclud;
			...
			if (drawInclud)
			{
				...
				else
				{
					...
					for (var i = 0; i < Get.IncludedColliders.Count; i++)
					{
						EditorGUILayout.BeginHorizontal();
						...
						if (Get.IncludedColliders[i] != null)
						{
							...
							Get.IncludedColliders[i] = (Collider) EditorGUILayout.ObjectField(
								Get.IncludedColliders[i], typeof(Collider), true);
							if (Get.IncludedColliders[i].gameObject)
								if (!Get.IncludedColliders[i].gameObject.activeInHierarchy)
									GUI.color = c;
						}
				...
				if (GUILayout.Button(
						...
						FGUI_Resources.ButtonStyle, GUILayout.Height(18)))
				...
				if (Get.IncludedColliders.Count > 0)
					EditorGUILayout.HelpBox(
						"You can disable collider components on the objects - Bones Stimulator will still detect collision. If you deactivate the Game Object with collider - Bones Stimulator will not detect collision with it.",
						MessageType.Info);
							...
							if (draggedObject)
							{
								...
								for (var ci = 0; ci < coll.Length; ci++)
								{
									...
									foreach (var stim in SelectedStimulators())
									{
										...
										if (stim.IncludedColliders == null)
											stim.IncludedColliders = new List<Collider>();
		...
		{
			var stims = new List<BonesStimulator>();

			foreach (var o in Selection.transforms)
			{
				var stim = o.GetComponent<BonesStimulator>();
				if (stim)
					if (!stims.Contains(stim))
						stims.Add(stim);
			}
