...
{
	private static bool drawMain = true;
	private static bool drawAnimationOptions = true;
	private static bool drawQuickCorrection;
	private static bool drawAdvancedCorrection;
	private static bool drawDebug;
	private static readonly bool drawPreciseAutoCorr = false;
	...
	private void DrawOldGUI()
	{
		...
		EditorGUILayout.BeginVertical(FEditor_Styles.Style(Color.white.ChangeColorAlpha(0.25f)));
		...
		if (drawMain)
		{
			...
			if (Get.SpineBones == null || Get.SpineBones.Count < 1)
			{
				...
				EditorGUILayout.HelpBox(
					"Put here two marginal bones from hierarchy and click 'Get' to create spine chain of section you want to animate with spinal animator",
					MessageType.Info);
			...
			if (Get.SpineBones == null || Get.SpineBones.Count < 1)
			{
				var smallStyle = new GUIStyle(EditorStyles.miniLabel) {fontStyle = FontStyle.Italic, fontSize = 9};
				GUI.color = new Color(1f, 1f, 1f, 0.7f);
				EditorGUILayout.BeginHorizontal();
				GUILayout.Label(
					new GUIContent(
						"             Enter for tooltip",
						"If you rigging quadroped or other animal, start bone should be pelvis bone with back legs and tail inside the hierarchy"),
					smallStyle);
				GUILayout.FlexibleSpace();
				GUILayout.Label(
					new GUIContent(
						"Enter for tooltip                       ",
						"If you rigging quadroped or other animal, end bone should be chest bone / neck bone or head bone, depends of your needs and model structure"),
					smallStyle);
			...
			if (Get.SpineBones != null)
			{
				...
				{
					if (startBone != Get.SpineBones[0].transform ||
						endBone != Get.SpineBones[Get.SpineBones.Count - 1].transform) wrong = 3;
				}
			}
			else
			{
			...
			startBone = (Transform) EditorGUILayout.ObjectField(
				new GUIContent(
					"Start", "Put here first bone in hierarchy depth for automatically get chain of bones to end one"),
				startBone, typeof(Transform), true);
			endBone = (Transform) EditorGUILayout.ObjectField(
				new GUIContent(
					"End", "Put here last bone in hierarchy depth for automatically get chain of bones from start one"),
				endBone, typeof(Transform), true);
			...
			if (GUILayout.Button(
					new GUIContent(
						"L",
						"Automatically get last bone in hierarchy - it depends of children placement, then sometimes last bone can be found wrong, whne you have arms/legs bones inside, if they're higher, algorithm will go through them"),
					GUILayout.MaxWidth(24), GUILayout.MaxHeight(14))) GetLastBoneInHierarchy();
			...
			if (startBone != null && endBone != null)
			{
				...
				if (Get.SpineBones != null)
					if (Get.SpineBones.Count > 0)
					{
						if (startBone != Get.SpineBones[0].transform ||
							endBone != Get.SpineBones[Get.SpineBones.Count - 1].transform) wrong = 3;
						else GUI.color = preCol.ChangeColorAlpha(0.7f);
					}
			}
			...
			if (Get.SpineBones == null || Get.SpineBones.Count < 1)
			{
				...
				GUILayout.Label(
					new GUIContent(
						"        Pelvis bone with Legs",
						"If you rigging quadroped or other animal, start bone should be pelvis bone with back legs and tail inside the hierarchy"),
					smallStyle);
				GUILayout.FlexibleSpace();
				GUILayout.Label(
					new GUIContent(
						"Chest/Neck/Head bone             ",
						"If you rigging quadroped or other animal, end bone should be chest bone / neck bone or head bone, depends of your needs and model structure"),
					smallStyle);
				EditorGUILayout.EndHorizontal();
				GUILayout.Space(1f);
			...
			drawSpineTransforms = EditorGUILayout.Foldout(
				drawSpineTransforms, new GUIContent("Spine Transforms", "Spine chain transforms"), true,
			...
			if (drawSpineTransforms)
			{
				...
				for (var i = 0; i < Get.SpineBones.Count; i++)
				{
					...
					EditorGUILayout.ObjectField(
						"Spine Bone [" + i + "]", Get.SpineBones[i].transform, typeof(Transform), true);
...
#region Animation Options
		...
		if (drawAnimationOptions)
		{
			...
			if (Get.UseCollisions)
				...
				{
					if (Get.PosSmoother < 0.075f) GUI.color = new Color(0.9f, 0.5f, 0.5f);
					else if (Get.PosSmoother < 0.225f)
						GUI.color = Color.Lerp(
							new Color(0.9f, 0.6f, 0.6f), preCc, Mathf.InverseLerp(0.075f, 0.225f, Get.PosSmoother));
				}
...
#region Basic Correction
		...
		if (drawPreciseAutoCorr)
		{
			...
			headBone = (Transform) EditorGUILayout.ObjectField(
				new GUIContent(
					"Head bone",
					"Head bone or some bone before, it's important to be in front of spine and not included in spinal animator's chain"),
				headBone, typeof(Transform), true);
			GUI.color = preCol;
...
#region Debug Options
		...
		if (drawDebug)
		{
			...
			EditorGUILayout.HelpBox(
				"When 'DrawDebug' is toggled, you can use button '~' to instantly deactivate SpineAnimator's motion for time you hold this button (not on build)",
				MessageType.None);
			GUILayout.EndHorizontal();
...
#region Physical Experimental Stuff
	...
	protected static bool drawCollisionParams;
	...
	private void DrawPhysicsStuff(FSpineAnimator spine)
	{
		...
		if (spine.UseCollisions)
		{
			...
			{
				EditorGUILayout.HelpBox(
					"Collision support is experimental and not working fully correct yet.", MessageType.Info);
				...
				if (ActiveEditorTracker.sharedTracker.isLocked) GUI.color = new Color(0.44f, 0.44f, 0.44f, 0.8f);
				else GUI.color = new Color(0.95f, 0.95f, 0.99f, 0.9f);
				if (GUILayout.Button(
						new GUIContent(
							"Lock Inspector for Drag & Drop Colliders",
							"Drag & drop colliders to 'Included Colliders' List from the hierarchy"),
						EditorStyles.toolbarButton))
				{
					var act = Get.gameObject;
			...
			else // In Playmode
			{
				...
				EditorGUILayout.HelpBox(
					"Rescalling in playmode available only in editor not in build", MessageType.Warning);
				EditorGUILayout.PropertyField(sp_CollidersScaleMul, new GUIContent("Scale Multiplier"));
				EditorGUILayout.PropertyField(sp_CollidersScale, new GUIContent("Scale Curve"));
				EditorGUILayout.PropertyField(sp_CollidersAutoCurve, new GUIContent("Auto Curve"));
				FEditor_Styles.DrawUILine(new Color(0.6f, 0.6f, 0.6f, 0.4f));
