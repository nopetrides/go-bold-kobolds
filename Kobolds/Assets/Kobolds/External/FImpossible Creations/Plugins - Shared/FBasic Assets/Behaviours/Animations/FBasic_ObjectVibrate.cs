...
namespace FIMSpace.Basics
{
	...
	public class FBasic_ObjectVibrate : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			var intensityLog = Mathf.Max(0f, Mathf.Log(intensity) / 5f);
			...
			var smoothDelta = Mathf.Lerp(1f, Time.deltaTime * 0.5f, SmoothTranslation);
		...
		public void ChooseNewSeed()
		{
			...
			for (var i = 0; i < 3; i++) randomOffsets[i] = Random.Range(0.8f, 1.0f);
			for (var i = 3; i < 6; i++) randomOffsets[i] = Random.Range(1.0f, 2.5f);
...
#region Private variables for calculations
		...
		private readonly float[] randomOffsets = new float[6];

		/// <summary>
		///     Position defined at start for animation correctness, you can put this object into other empty to have
		///     mobility
		/// </summary>
