...
namespace FIMSpace.Basics
{
	...
	public class FBasic_PullableDrawer : FBasic_Pullable
	{
		...
		[Tooltip(
			"Reverse pull direction if behind object (objects opened up and down (Y-Axis) should have it at false)")]
		public bool ReverseIfBack = true;

		public bool NoDiffVelo;
			...
			if (!Application.isPlaying)
			{
				...
				if (StartValueY > 0f)
				{
					...
					Gizmos.DrawWireSphere(
						Vector3.Lerp(
							transform.position, transform.position + transform.TransformVector(DrawedPositionOffset),
							StartValueY), 0.2f);
				}
			}
		}
		...
		protected override void UpdateIn()
		{
			...
			var startPosW = transform.position;

			transform.localPosition = Vector3.Lerp(
				transform.localPosition, initialLocalPosition + transform.TransformVector(targetPositionOffset),
				Time.deltaTime * 7f * translationIncreaser);
			var postPosW = transform.position;

			var diff = Vector3.Distance(
				transform.localPosition, initialLocalPosition + transform.TransformVector(targetPositionOffset));
			translationIncreaser = Mathf.Lerp(
				translationIncreaser, Mathf.Lerp(1f, 3f, Mathf.InverseLerp(0.001f, 0.3f, diff)), Time.deltaTime * 8f);

			if (diff < 0.005f && velocity < 0.005f) animationFinished = true;
			...
			if (Holding)
			{
				...
				if (NoDiffVelo)
					...
					diffVel = diff * Mathf.Sign(
						(initialLocalPosition - transform.localPosition).magnitude - targetPositionOffset.magnitude);
				else
					//diffVel = diff * Mathf.Sign(SumVector(transform.position - startPos));
					diffVel = diff * Mathf.Sign(
						SumVector(
							transform.InverseTransformVector(postPosW) - transform.InverseTransformVector(startPosW)));
		...
		protected override void StartHolding()
		{
			if (ReverseIfBack)
				lookDot = Vector3.Dot(
					transform.TransformVector(DrawedPositionOffset).normalized, Camera.main.transform.forward);
