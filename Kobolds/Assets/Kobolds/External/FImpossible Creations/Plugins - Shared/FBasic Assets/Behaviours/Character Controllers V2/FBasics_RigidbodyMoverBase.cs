...
namespace FIMSpace.Basics
{
	...
	public abstract class FBasics_RigidbodyMoverBase : MonoBehaviour
	{
		...
		{
			var velocityMemory = rigbody.linearVelocity;
			...
			rigbody.angularVelocity = (smoothedRotation * Quaternion.Inverse(transform.rotation)).QToAngularVelocity() *
									rotateSpeed * 10f;
		}
			...
			if (!isGrounded)
				if (collision != null)
					if (collision.contacts.Length > 0)
						...
						{
							var dot = Vector3.Dot(transform.up, collision.contacts[i].normal);
		...
		protected virtual void UpdateMotor()
		{
			...
			var targetMoveDir = transform.forward;
			var targetRot = transform.eulerAngles; // target rotation

			var accelerateTime = 0.1f;
			var rotateSpeed = 10f;

			// Calculating smooth acceleration value to be used in next fixed update frame
			smoothedAcceleration = Vector3.SmoothDamp(
				smoothedAcceleration, targetMoveDir, ref veloHelper, accelerateTime, Mathf.Infinity, Time.deltaTime);

			// Calculating smooth rotation to be applied in fixes update
			smoothedRotation = Quaternion.Lerp(
				rigbody.rotation, Quaternion.Euler(targetRot), Time.deltaTime * rotateSpeed);
		}
