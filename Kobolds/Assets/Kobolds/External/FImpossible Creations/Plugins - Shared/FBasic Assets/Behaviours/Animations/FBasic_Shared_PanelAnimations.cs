	...
	public class FBasic_Shared_PanelAnimations : MonoBehaviour
	{
		...
		private IEnumerator ClickAniamtion()
		{
			...
			while (time < AnimationTime * 0.6f)
			{
				...
				buttonTransform.localPosition = Vector3.LerpUnclamped(
					buttonInitPosition, buttonInitPosition - Vector3.up * 0.05f,
					FEasing.EaseOutElastic(0f, 1f, progress, EaseExtraValue));
			...
			while (time < AnimationTime / 4f)
			{
				...
				var progress = time / (AnimationTime / 4f);

				buttonTransform.localPosition = Vector3.LerpUnclamped(
					currentPos, buttonInitPosition, FEasing.EaseInOutCubic(0f, 1f, progress));
