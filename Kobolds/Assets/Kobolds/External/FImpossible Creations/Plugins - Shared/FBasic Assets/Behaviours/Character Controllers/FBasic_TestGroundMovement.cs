...
#region Methods
		...
		{
			if (RelativeLookUp) return Vector3.Lerp(transform.up, Vector3.up, RelativeLookUpBias);
		...
		internal RaycastHit CastRay()
		{
			RaycastHit outHit;
			Physics.Raycast(
				transform.position + GetUpVector() * RaycastHeightOffset, -GetUpVector(), out outHit,
...
#region Methods
		...
		protected virtual void HandleTransforming()
		{
			...
			if (MoveForward)
			{
				if (!fittingEnabled)
				{
					RotationYAxis = Mathf.LerpAngle(
						RotationYAxis, Camera.main.transform.eulerAngles.y + RotationOffset,
