...
namespace FIMSpace.Basics
{
	...
	{
		public static List<FBasic_InteractionAreaBase> EnteredInteractions = new();
		public static FBasic_InteractionAreaBase LastEnteredInteraction;

		/// <summary>
		///     If you are pulling doors or something and it goes out of trigger range, this variable will allow component to
		///     update still, after setting to null object will stop updateing
		/// </summary>
		public static FBasic_InteractionAreaBase LockedInteraction;

		[Tooltip(
			"If character need to look at this object with camera direction, useful for FPS games. Value = 1 Camera can look anywhere interaction always popup, Value = 0.25 camera must look almost directly on object for interaction popup")]
		[Range(0.15f, 1f)]
		public float LookAtRange = 1f;

		[Tooltip(
			"We can add rigidbody for example to objects which are using this component's transform as additional child object fpr pbject's features, OnTriggerEnter will be sent only to this rigidbody instead of parent's rigidbody")]
		public bool AddRigidbody;

		/// <summary>
		///     When you derives from this classes and doing animations in UpdateIn() method, object exits player range and
		///     you want still to update until animation finish (for example door hinge animation)
		/// </summary>
		protected bool? conditionalExit;
			...
			if (AddRigidbody)
				if (!rigidBody)
				{
					...
					transform.localPosition +=
						Vector3.right *
						0.0001f; // It's Unity Bug? Or something? Object needs to be moved in any local position offset for trigger collider to work correctly if in parent is another collider which is inside this trigger (even layers are ignored) pretty annoying thing
				}
		}
		...
		{
			if (other.tag == "Player") EnteredFlag = true;
		...
		{
			if (other.tag == "Player") EnteredFlag = false;
		...
		protected virtual Collider GetTrigger()
		{
			if (!triggerArea)
			{
				...
				if (!triggerArea)
				{
					var triggerSphere = gameObject.AddComponent<SphereCollider>();
			...
			while (true)
			{
				...
				if (LockedInteraction == null)
				{
					...
					else
					{
						...
						var screenPoint = Camera.main.WorldToViewportPoint(
							transform.position + transform.TransformVector(toLookPositionOffset));

						if (screenPoint.z > 0 && screenPoint.x > 0 && screenPoint.x < 1 && screenPoint.y > 0 &&
							screenPoint.y < 1)
						{
							var xFactor = screenPoint.x;
							xFactor = Mathf.Abs(0.5f - xFactor);
							var yFactor = screenPoint.y;
							yFactor = Mathf.Abs(0.5f - yFactor);
							...
							if (VisibleFactor < LookAtRange)
							{
								...
								{
									var mostFocused = EnteredInteractions[0].VisibleFactor;
									var mostFocusedI = 0;

									for (var i = 1; i < EnteredInteractions.Count; i++)
