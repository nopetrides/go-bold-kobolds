...
{
	/// <summary>
	///     FM: Main component to controll scripts responsible for character movement, animations etc. using Rigidbody
	///     component
	///     Ready to be inherited in any way.
	///     I don't use character controller because it is expensive when it goes to use for example 50 of them
	///     (CharacterController.Move() is expensive, just make test for 200 CharControllers and then 200 rigidbodies,
	///     difference is huge)
	/// </summary>
	...
	public class FBasic_RigidbodyMovement : FBasic_CharacterHorizontalBase
	{
		...
		/// <summary>
		///     When character starts to float in very small height by rigidbody bump, we raycast from foots to check if we
		///     should stand on ground anyway
		/// </summary>
		...
		protected override void Start()
		{
			...
			if (ChracterLayerMask == 0) ChracterLayerMask = ~(1 << LayerMask.NameToLayer("Water"));
		}
		...
		protected override void FixedUpdate()
		{
			if (Grounded)
			{
				...
				var accForw = accelerationForward;
				var accRight = accelerationRight;
				...
				var newVelocity = new Vector3(
					lastTargetVelocityForward.x * accForw, CharacterRigidbody.linearVelocity.y,
					lastTargetVelocityForward.z * accForw);
				newVelocity += new Vector3(
					lastTargetVelocityRight.x * accRight, 0f, lastTargetVelocityRight.z * accRight);
		...
		/// <summary>
		///     We raycast to ground and placing character on ground when he starts to float for a little bit from rigidbody
		///     velocity
		/// </summary>
		protected void CheckGroundPlacement()
		{
			if (!Jumped)
				if (!Grounded)
				{
					...
					var groundRay = new Ray(transform.position + Vector3.up * 0.2f, Vector3.down);

					if (Physics.Raycast(
							groundRay, out groundHit, SkinHeight + 0.2f, ChracterLayerMask,
							QueryTriggerInteraction.Ignore))
					{
						// Calculating difference in y for bottom of collider and game object to place it on ground correctly
						var colliderToTransformHeight = capsuleCollider.bounds.center.y -
														capsuleCollider.bounds.extents.y - transform.position.y;
						Grounded = true;
						transform.position = new Vector3(
							transform.position.x, groundHit.point.y - colliderToTransformHeight, transform.position.z);
						CharacterRigidbody.linearVelocity = new Vector3(
							CharacterRigidbody.linearVelocity.x, fakeYVelocity, CharacterRigidbody.linearVelocity.z);
		...
		protected override void RotationCalculations()
		{
			if (Grounded)
			{
				...
				else if (horizontalValue != 0f) targetDirection = inputDirection;
		...
		private void CheckIfFacingWall(Collision collision)
		{
			...
			if (accelerationForward != 0f)
				...
				{
					var verticalSign = 1f;
					if (!onlyForward) verticalSign *= Mathf.Sign(verticalValue);

					if (Vector3.Dot(
							collision.contacts[i].point - transform.position, transform.forward * verticalSign) > 0.25f)
					{
						FacingWall = true;
						collisionVertical = i;
						break;
			...
			if (accelerationRight != 0f)
				for (var i = 0; i < collision.contacts.Length; i++)
				{
					...
					if (Vector3.Dot(
							...
							transform.right * Mathf.Sign(horizontalValue)) > 0.25f)
					{
						SideingWall = true;
						break;
					}
