...
namespace FIMSpace.Basics
{
	...
	public class FBasic_DuplicateObjects : MonoBehaviour
	{
		...
		public Vector3 DuplicatesCount = new(3, 1, 3);

		[Tooltip("How far from each other should be created copies")]
		public Vector3 Offsets = new(3f, 0f, 3f);

		public Vector3 Randomize = new(0f, 0f, 0f);
		public Vector3 RandomRotate = new(0f, 0f, 0f);
		public Vector3 RandomScale = new(0f, 0f, 0f);
		public int Seed;
		...
		private void OnDrawGizmos()
		{
			...
			if (DuplicationOrigin == FEDuplicateOrigin.FromComponent) originPosition = transform.position;
			else originPosition = ToDuplicate.transform.position;
			...
			if (DuplicationType == FEDuplicateDirection.GoIterative)
				...
				for (var x = 0; x < DuplicatesCount.x; x++)
					for (var y = 0; y < DuplicatesCount.y; y++)
						for (var z = 0; z < DuplicatesCount.z; z++)
						{
							var offset = originPosition;
				...
				for (var x = 0; x < DuplicatesCount.x; x++)
					for (var y = 0; y < DuplicatesCount.y; y++)
						for (var z = 0; z < DuplicatesCount.z; z++)
						{
							...
							var newObject = Instantiate(ToDuplicate);
							newObject.transform.position = offset + GetRandomVector();
							newObject.transform.rotation *= Quaternion.Euler(
								Random.Range(-RandomRotate.x, RandomRotate.x),
								Random.Range(-RandomRotate.y, RandomRotate.y),
								Random.Range(-RandomRotate.z, RandomRotate.z));

							var scale = newObject.transform.localScale + new Vector3(
							...
							if (PlaceOnGround)
							{
								RaycastHit hit;
								Physics.Raycast(
									newObject.transform.position + Vector3.up * 100f, Vector3.down, out hit, 200f);
								if (hit.transform) newObject.transform.position = hit.point;
							}
						}
			else if (DuplicationType == FEDuplicateDirection.GoFromCenter)
				for (var x = 0; x < DuplicatesCount.x; x++)
		...
		private Vector3 GetRandomVector()
		{
			...
			return new Vector3(
				Random.Range(-Randomize.x, Randomize.x), Random.Range(-Randomize.y, Randomize.y),
		...
		[CustomEditor(typeof(FBasic_DuplicateObjects))]
		public class FBasic_DuplicateObjectsEditor : Editor
		{
			public override void OnInspectorGUI()
			{
				var targetScript = (FBasic_DuplicateObjects) target;
