...
namespace FIMSpace.Basics
{
	...
	/// </summary>
	[Serializable] // Serializable for debug mode
	public class FBasic_FheelekAnimator
	{
		...
		internal void Animate(float acc)
		{
			...
			if (AnimationHolder)
				if (waitForIdle)
				{
					var nextInfo = animator.GetNextAnimatorStateInfo(locomotionLayer);
					if (!nextInfo.IsName(lastAnimation))
					{
						...
						{
							var info = animator.GetCurrentAnimatorStateInfo(locomotionLayer);
				...
				if (!controller.Grounded)
					if (controller.CharacterRigidbody.linearVelocity.y < 0f)
						if (Physics.Raycast(
								...
								0.1f - controller.CharacterRigidbody.linearVelocity.y * Time.fixedDeltaTime))
							Landing();
				...
				// If we are holding animation (not looped one like landing) we checking it progress
			{
				if (!animator.IsInTransition(locomotionLayer))
				{
					var stateInfo = animator.GetCurrentAnimatorStateInfo(locomotionLayer);

					if (stateInfo.normalizedTime > 0.7f) AnimationHolder = false;
				}
			}
		...
		/// <summary>
		///     Crossfades to target animation and locks from launching other animations until state machine not turn back or be
		...
		{
			animator.LerpFloatValue(parameter, value, 5f);
		}
	}
}
