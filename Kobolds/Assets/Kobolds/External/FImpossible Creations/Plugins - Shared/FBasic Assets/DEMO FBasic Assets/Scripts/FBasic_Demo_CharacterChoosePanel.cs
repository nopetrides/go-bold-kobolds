...
namespace FIMSpace.Basics.Demo
{
	...
	public class FBasic_Demo_CharacterChoosePanel : MonoBehaviour
	{
		...
		{
			if (!mainController)
				mainController = FindObjectOfType<FBasic_FheelekController>().GetComponent<FBasic_CharacterInputKeys>();
			...
			if (TargetController != null)
			{
				...
				if (TargetController.enabled) currentController = TargetController;
				else currentController = mainController;
			}
			else
			{
				if (!tppCam.enabled)
