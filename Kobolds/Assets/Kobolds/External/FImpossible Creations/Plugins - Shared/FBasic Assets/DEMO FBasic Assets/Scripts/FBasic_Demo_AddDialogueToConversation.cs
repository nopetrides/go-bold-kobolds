...
namespace FIMSpace.FBasics
{
	public class FBasic_Demo_AddDialogueToConversation : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			var targetDialogue = new FBasic_DialogueBase(DialogueTitle);
			targetDialogue.Replys = new List<FBasic_ReplyBase>();

			for (var i = 0; i < Replys.Count; i++)
			{
				var reply = new FBasic_ReplyBase();
				reply.Text = Replys[i];

				if (PlayerReply[i]) reply.TalkActor = FBasic_ReplyBase.FReplyTalker.Player;
				else reply.TalkActor = FBasic_ReplyBase.FReplyTalker.NPC;
		...
		private void OnValidate()
		{
			if (Replys.Count != PlayerReply.Count)
			{
				...
				for (var i = 0; i < Replys.Count; i++) PlayerReply.Add(i % 2 == 0);
			}
		}
	}
}
