...
namespace FIMSpace.Basics
{
	public class FBasic_Note : MonoBehaviour
	{
		...
		[HideInInspector]
		public string Note = "Write here custom description message for inspector view guide for users.";
	...
	public class FBasic_NoteEditor : Editor
	{
		private static GUIStyle header;
		private static GUIStyle padding;
		private static GUIStyle displayText;
		private FBasic_Note _get;

		public FBasic_Note Get
		{
			get
			{
				if (_get == null) _get = (FBasic_Note) target;
				return _get;
			}
		}
...
#region Refresh Styles
			...
			if (GUILayout.Button(
					EditorGUIUtility.IconContent("CollabEdit Icon"), EditorStyles.label, GUILayout.Width(22),
					GUILayout.Height(18))) Get.EditMode = !Get.EditMode;
			...
			{
				var gc_note = new GUIContent(Get.Note);
