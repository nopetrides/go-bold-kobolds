...
namespace FIMSpace
{
	...
	public abstract class FImp_ColliderData_Base
	{
		...
		{
			var s = collider as SphereCollider;

			if (s)
				return new FImp_ColliderData_Sphere(s);
			var c = collider as CapsuleCollider;
			if (c)
				return new FImp_ColliderData_Capsule(c);
			var b = collider as BoxCollider;
			if (b)
				return new FImp_ColliderData_Box(b);
			var m = collider as MeshCollider;
			if (m)
				return new FImp_ColliderData_Mesh(m);
			var t = collider as TerrainCollider;
			if (t)
				return new FImp_ColliderData_Terrain(t);
			var ch = collider as CharacterController;
			if (ch)
		...
		{
			var s = collider as CircleCollider2D;
			...
			if (c)
				return new FImp_ColliderData_Capsule(c);
			var b = collider as BoxCollider2D;
			if (b)
				return new FImp_ColliderData_Box(b);
			var m = collider as PolygonCollider2D;
			if (m)
				return new FImp_ColliderData_Mesh(m);
			//else
			//{
		...
		{
			if (Transform.gameObject.isStatic) IsStatic = true;
			else IsStatic = false;
		}
