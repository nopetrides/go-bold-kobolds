...
namespace FIMSpace
{
	public class FImp_ColliderData_CharacterCapsule : FImp_ColliderData_Base
	{
		...
		public override void RefreshColliderData()
		{
			...
			if (diff) GetCapsuleHeadsPositions(Capsule, ref Top, ref Bottom, Direction, radius, scaleFactor);
...
#region Capsule Calculations Helpers
		...
		protected static void CalculateCapsuleParameters(
			CharacterController capsule, ref Vector3 direction, ref float trueRadius, ref float scalerFactor)
		{
		...
		protected static void GetCapsuleHeadsPositions(
			CharacterController capsule, ref Vector3 upper, ref Vector3 bottom, Vector3 direction, float radius,
			float scalerFactor)
		{
