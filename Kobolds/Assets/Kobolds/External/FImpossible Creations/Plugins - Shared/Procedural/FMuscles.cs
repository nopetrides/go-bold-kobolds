...
namespace FIMSpace.FTools
{
	public abstract class FMuscle_Motor
	{
		...
		public void Update(
			float delta, float current, float desired, float acceleration, float accelerationLimit, float damping,
			float brakePower)
		{
			...
			dynamicAcceleration = Mathf.Clamp(
				dynamicAcceleration, -accelerationLimit, accelerationLimit); // Limit acceleration
	...
	public class FMuscle_Vector3
	{
		...
		public void Update(
			float delta, Vector3 desired, float acceleration, float accelerationLimit, float damping, float brakePower)
		{
		...
		/// </summary>
		public IEnumerator PushImpulseCoroutine(
			Vector3 power, float duration, bool fadeOutPower = false, float delay = 0f)
		{
	...
	public class FMuscle_Quaternion
	{
		...
		public void Update(
			float delta, Quaternion desired, float acceleration, float accelerationLimit, float damping,
			float brakePower)
		{
		...
		public static Quaternion EnsureQuaternionContinuity(Quaternion latestRot, Quaternion targetRot)
		{
			...
			var midQFlipped = new Quaternion(
				Mathf.LerpUnclamped(latestRot.x, flipped.x, 0.5f), Mathf.LerpUnclamped(latestRot.y, flipped.y, 0.5f),
				Mathf.LerpUnclamped(latestRot.z, flipped.z, 0.5f), Mathf.LerpUnclamped(latestRot.w, flipped.w, 0.5f));
		...
		/// </summary>
		public IEnumerator PushImpulseCoroutine(
			Quaternion power, float duration, bool fadeOutPower = false, float delay = 0f)
		{
			if (delay > 0f) yield return new WaitForSeconds(delay);
