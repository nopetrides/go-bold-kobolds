...
namespace Unity.Physics.Authoring
{
	...
	internal class PhysicsBodyAuthoringBaker : BasePhysicsBaker<PhysicsBodyAuthoring>
	{
		...
		public override void Bake(PhysicsBodyAuthoring authoring)
		{
			...
			AddComponent(
				entity, new PhysicsBodyAuthoringData
				{
			...
			{
				AddComponent(
					entity, new PhysicsCompoundData
					{
			...
			AddComponent(
				entity, authoring.MotionType == BodyMotionType.Dynamic ?
			...
			if (authoring.MotionType == BodyMotionType.Dynamic)
			{
				...
				if (authoring.GravityFactor != 1)
					AddComponent(
						entity, new PhysicsGravityFactor
						{
			...
			if (hasSmoothing)
			{
				...
				if (authoring.Smoothing == BodySmoothing.Interpolation)
					AddComponent(
						entity, new PhysicsGraphicalInterpolationBuffer
						{
		...
		public void OnUpdate(ref SystemState state)
		{
			...
			foreach (var (physicsMass, bodyData, entity) in
				...
				physicsMass.ValueRW = CreatePhysicsMass(
					entityManager, entity, bodyData.ValueRO, MassProperties.UnitSphere);
			...
			foreach (var (physicsMass, bodyData, collider, entity) in
					SystemAPI.Query<RefRW<PhysicsMass>, RefRO<PhysicsBodyAuthoringData>, RefRO<PhysicsCollider>>()
						...
						.WithOptions(EntityQueryOptions.IncludePrefab | EntityQueryOptions.IncludeDisabledEntities))
				physicsMass.ValueRW = CreatePhysicsMass(
					entityManager, entity, bodyData.ValueRO,
					collider.ValueRO.MassProperties, true);
		}

		private PhysicsMass CreatePhysicsMass(
			EntityManager entityManager, in Entity entity,
			in PhysicsBodyAuthoringData inBodyData, in MassProperties inMassProperties, in bool hasCollider = false)
		{
			...
			if (inBodyData.OverrideDefaultMassDistribution)
			{
				...
				if (hasCollider)
					...
					massProperties.AngularExpansionFactor += math.length(
						massProperties.MassDistribution.Transform.pos -
