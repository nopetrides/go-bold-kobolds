...
namespace Unity.Physics.Editor
{
	...
	internal class PhysicsShapeAuthoringEditor : BaseEditor
	{
		...
		protected override void OnEnable()
		{
			...
			m_NumImplicitStatic = targets.Cast<PhysicsShapeAuthoring>().Count(shape =>
				shape.GetPrimaryBody() == shape.gameObject
			...
			switch (shape.ShapeType)
			{
				...
				case ShapeType.Capsule:
					...
					bounds = TransformBounds(
						bounds, float4x4.TRS(capsuleGeometry.Center, capsuleGeometry.Orientation, 1f));
					break;
				...
				case ShapeType.Cylinder:
					...
					bounds = TransformBounds(
						bounds, float4x4.TRS(cylinderGeometry.Center, cylinderGeometry.Orientation, 1f));
					break;
				...
				switch (shape.ShapeType)
				{
						...
						EditorGUI.BeginChangeCheck();
					{
						using (new Handles.DrawingScope(
									math.mul(
										Handles.matrix, float4x4.TRS(boxGeometry.Center, boxGeometry.Orientation, 1f))))
						{
							s_Box.DrawHandle();
						}
					}
					...
					{
						using (new Handles.DrawingScope(
									math.mul(Handles.matrix, float4x4.TRS(sphereGeometry.Center, orientation, 1f))))
						{
							s_Sphere.DrawHandle();
						}
					}
						...
						EditorGUI.BeginChangeCheck();
					{
						...
						var normal = math.cross(math.normalizesafe(forward), math.normalizesafe(right))
									* 0.5f * math.lerp(
										math.length(right) * size2.x, math.length(forward) * size2.y, 0.5f);
				...
				switch (shape.ShapeType)
				{
					...
					case ShapeType.Cylinder:
						...
						if (math.abs(cylinder.BevelRadius - cylinder.Radius) < k_Epsilon)
							m_ShapeSuggestions.Add(
								math.abs(cylinder.Height - 2f * cylinder.Radius) < k_Epsilon ?
		...
		private static class Styles
		{
			...
			public static readonly GUIStyle ButtonDropDown = new(EditorStyles.popup)
		...
		private class PreviewMeshData : IDisposable
		{
			...
			private JobHandle ScheduleConvexHullPreview(
				PhysicsShapeAuthoring shape, NativeArray<BlobAssetReference<Collider>> output)
			{
			...
			private JobHandle ScheduleMeshPreview(
				PhysicsShapeAuthoring shape, NativeArray<BlobAssetReference<Collider>> output)
			{
