...
namespace Unity.Physics.Editor
{
	...
	internal class PhysicsMaterialPropertiesDrawer : BaseDrawer
	{
		...
		public override float GetPropertyHeight(SerializedProperty property, GUIContent label)
		{
			...
			FindToggleAndValueProperties(
				property, templateValueProperty, "m_CollisionResponse", out _, out var collisionResponse);
		...
		protected override void DoGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			FindToggleAndValueProperties(
				property, templateValue, "m_CollisionResponse", out var collisionResponseDropDown,
				out var collisionResponse);
			position.height = EditorGUIUtility.singleLineHeight;
			DisplayOverridableProperty(
				position, Content.CollisionResponseLabel, collisionResponseDropDown, collisionResponse,
				templateAssigned);
			...
			if (collisionFilterGroup.isExpanded)
			{
				...
				FindToggleAndValueProperties(
					property, templateValue, "m_BelongsToCategories", out toggle, out var belongsTo);
				position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
				position.height = EditorGUIUtility.singleLineHeight;
				DisplayOverridableProperty(position, Content.BelongsToLabel, toggle, belongsTo, templateAssigned);

				FindToggleAndValueProperties(
					property, templateValue, "m_CollidesWithCategories", out toggle, out var collidesWith);
				position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
				position.height = EditorGUIUtility.singleLineHeight;
			...
			if (advancedGroup.isExpanded)
			{
				...
				FindToggleAndValueProperties(
					property, templateValue, "m_CustomMaterialTags", out toggle, out var customFlags);
				position.y = position.yMax + EditorGUIUtility.standardVerticalSpacing;
				position.height = EditorGUIUtility.singleLineHeight;
