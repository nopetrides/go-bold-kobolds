	...
	internal class PhysicsSphereBoundsHandle : SphereBoundsHandle
	{
		protected override void DrawWireframe()
		{
			...
			if (radius > 0)
			{
				...
				var backfacedColor = Handles.color * new Color(
					1f, 1f, 1f, PhysicsBoundsHandleUtility.kBackfaceAlphaMultiplier);
				var discVisible = new[]
				{
					y && z,
					x && z,
				...
				if (insideAmount < 1)
				{
					if (math.abs(sqrDistCameraToCenter) >= kEpsilon)
					{
						...
						for (var i = 0; i < 3; i++)
						{
							...
							angleBetweenDiscAndNormal = math.PI * 0.5f - math.min(
								angleBetweenDiscAndNormal, math.PI - angleBetweenDiscAndNormal);
							...
							if (g < 1)
							{
								...
								{
									Handles.DrawWireArc(
										center, discOrientation, vectorToPointOnHorizon, horizonArcLength, radius);
								}

								using (new Handles.DrawingScope(backfacedColor))
								{
									Handles.DrawWireArc(
										center, discOrientation, vectorToPointOnHorizon, horizonArcLength - 360,
										radius);
								}
							}
							else
