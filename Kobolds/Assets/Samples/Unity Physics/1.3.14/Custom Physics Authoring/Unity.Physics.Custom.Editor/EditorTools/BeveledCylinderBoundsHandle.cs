	...
	internal class BeveledCylinderBoundsHandle : PrimitiveBoundsHandle
	{
		...
		protected override void DrawWireframe()
		{
			using (new Handles.DrawingScope(Handles.matrix))
			{
				...
				var topBackFaced = PhysicsBoundsHandleUtility.IsBackfaced(
					top, -tangent, binormal, axes, isCameraInsideBox);
				var bottomBackFaced = PhysicsBoundsHandleUtility.IsBackfaced(
					bottom, tangent, binormal, axes, isCameraInsideBox);
				...
				var sideways1 = new float3(
					math.cos(t + kHalfPI - halfAngleStep), math.sin(t + kHalfPI - halfAngleStep), 0f);
				...
				for (var i = 0; i < m_SideCount; ++i)
				{
					...
					if (bevelGreaterThanZero)
					{
						...
						var currSideBackFaced = PhysicsBoundsHandleUtility.IsBackfaced(
							currSideMidPoint, up, sideways2, axes, isCameraInsideBox);
					...
					else
					{
						...
						var prevSideBackFaced = PhysicsBoundsHandleUtility.IsBackfaced(
							prevMidPoint, up, sideways1, axes, isCameraInsideBox);

						var currMidPoint = ctr + (top1 + top2 + bottom1 + bottom2) * 0.25f + startOffset;
						var currSideBackFaced = PhysicsBoundsHandleUtility.IsBackfaced(
							currMidPoint, up, sideways2, axes, isCameraInsideBox);
