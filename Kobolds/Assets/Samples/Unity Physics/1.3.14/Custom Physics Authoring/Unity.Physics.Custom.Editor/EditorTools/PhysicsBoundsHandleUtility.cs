...
namespace Unity.Physics.Editor
{
	internal static class PhysicsBoundsHandleUtility
	{
		...
		public static bool IsBackfaced(
			float3 localPos, float3 localTangent, float3 localBinormal, Axes axes, bool isCameraInsideBox)
		{
		...
		public static void DrawFace(
			float3 center, float3 size, float cornerRadius, int normalAxis, Axes axes, bool isCameraInsideBox)
		{
			// 0 = 0 1 2
			// 1 = 1 2 0
			// 2 = 2 0 1
		...
		public static void CalculateCornerHorizon(
			float3 cornerPosition, quaternion orientation, float3 cameraCenter, float3 cameraForward, bool cameraOrtho,
			float radius, out Corner corner)
		{
		...
		public static void CalculateCornerHorizon(
			float3 cornerPosition, float3x3 axes, float3 cornerNormal, float3 cameraCenter, float3 cameraForward,
			bool cameraOrtho, float radius, out Corner corner)
		{
		...
		public static void DrawCorner(Corner corner, bool3 showAxis)
		{
			...
			{
				var angleLength = Vector3.SignedAngle(
				...
				for (int A = 1, B = 2, C = 0; C < 3; A = B, B = C, C++)
					...
					else if (corner.splitAxis[0][C])
					{
						if (showAxis[C])
						{
							...
							Handles.DrawWireArc(
								origin, normals[C], intersections[0], corner.angle[A] - angleLength, radius);
						}
						...
						//if (corner.splitAxis[1][C])
					{
						if (showAxis[C])
						{
							...
							Handles.color = color1;
							Handles.DrawWireArc(
								origin, normals[C], intersections[1], corner.angle[A] - angleLength, radius);
						}
