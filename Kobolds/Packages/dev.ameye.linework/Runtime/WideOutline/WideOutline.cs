...
namespace Linework.WideOutline
{
	...
	public class WideOutline : ScriptableRendererFeature
	{
		...
		public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
		{
			...
			var render = wideOutlinePass.Setup(
				ref settings, ref maskMaterial, ref silhouetteMaterial, ref silhouetteInstancedMaterial,
				ref outlineMaterial, renderingData.cameraData.renderScale);
...
#pragma warning disable 618, 672
		...
		{
			if (settings == null ||
				(renderingData.cameraData.cameraType == CameraType.SceneView && !settings.ShowInSceneView)) return;
...
#pragma warning restore 618, 672
		...
		{
			if (maskMaterial == null) maskMaterial = CoreUtils.CreateEngineMaterial(shaders.mask);

			if (silhouetteMaterial == null) silhouetteMaterial = CoreUtils.CreateEngineMaterial(shaders.silhouette);
			...
			if (outlineMaterial == null) outlineMaterial = CoreUtils.CreateEngineMaterial(shaders.outline);

			return maskMaterial != null && silhouetteMaterial != null && silhouetteInstancedMaterial != null &&
		...
		private class WideOutlinePass : ScriptableRenderPass
		{
			...
			public bool Setup(
				ref WideOutlineSettings wideOutlineSettings, ref Material maskMaterial, ref Material silhouetteMaterial,
				ref Material silhouetteInstancedMaterial, ref Material compositeMaterial, float renderScale)
			{
				...
				foreach (var outline in settings.Outlines)
					if (outline.silhouetteMaterial == null || outline.silhouetteMaterialInstanced == null ||
						outline.informationMaterial == null || outline.informationMaterialInstanced == null)
						outline.AssignMaterials(silhouetteBase, silhouetteInstancedBase);

				foreach (var outline in settings.Outlines)
				{
					if (!outline.IsActive()) continue;

					var silhouette = outline.gpuInstancing ?
						outline.silhouetteMaterialInstanced :
						outline.silhouetteMaterial;
					var information = outline.gpuInstancing ?
						outline.informationMaterialInstanced :
						outline.informationMaterial;

					silhouette.SetColor(CommonShaderPropertyId.OutlineColor, outline.color);
					if (outline.occlusion == WideOutlineOcclusion.AsMask)
						silhouette.SetColor(CommonShaderPropertyId.OutlineColor, Color.clear);
					...
					if (settings.widthControl == WidthControl.PerOutline)
						information.EnableKeyword(ShaderFeature.InformationBuffer);
					else information.DisableKeyword(ShaderFeature.InformationBuffer);
					if (outline.width > maxwidth) maxwidth = outline.width;
					information.SetVector(
						CommonShaderPropertyId.Information, new Vector4(outline.width / 100.0f, 0.0f, 0.0f, 0.0f));
				}
				...
				if (settings.widthControl == WidthControl.PerOutline)
					composite.EnableKeyword(ShaderFeature.InformationBuffer);
			...
			{
				internal readonly List<(RendererListHandle handle, bool vertexAnimated)>
					InformationRendererListHandles = new();
			...
			public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
			{
				...
				CreateRenderGraphTextures(
					renderGraph, resourceData, out var silhouetteHandle, out var silhouetteDepthHandle,
					out var informationHandle, out var pingHandle, out var pongHandle);
				if (!silhouetteHandle.IsValid() || !silhouetteDepthHandle.IsValid() || !informationHandle.IsValid() ||
					!pingHandle.IsValid() || !pongHandle.IsValid()) return;
				...
				using (var builder = renderGraph.AddRasterRenderPass<PassData>(ShaderPassName.Mask, out var passData))
				{
					...
					{
						foreach (var handle in data.MaskRendererListHandles) context.cmd.DrawRendererList(handle);
					});
				}
				...
				using (var builder = renderGraph.AddRasterRenderPass<PassData>(
							ShaderPassName.Silhouette, out var passData))
				{
					builder.SetRenderAttachment(silhouetteHandle, 0);
					builder.SetRenderAttachmentDepth(
						settings.customDepthBuffer ? silhouetteDepthHandle : resourceData.activeDepthTexture);

					builder.SetGlobalTextureAfterPass(silhouetteHandle, ShaderPropertyId.SilhouetteBuffer);
					if (settings.customDepthBuffer)
						builder.SetGlobalTextureAfterPass(
							silhouetteDepthHandle, ShaderPropertyId.SilhouetteDepthBuffer);
					...
					builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
					{
						...
						{
							if (handle.vertexAnimated) context.cmd.EnableKeyword(Keyword.OutlineColor);

							context.cmd.DrawRendererList(handle.handle);

							if (handle.vertexAnimated) context.cmd.DisableKeyword(Keyword.OutlineColor);
						}
					});
				}
				...
				{
					using var builder = renderGraph.AddRasterRenderPass<PassData>(
						ShaderPassName.Information, out var passData);
				...
				using (var builder = renderGraph.AddUnsafePass<PassData>(ShaderPassName.Flood, out _))
				{
					...
					builder.SetRenderFunc((PassData _, UnsafeGraphContext context) =>
					{
						...
						Blitter.BlitCameraTexture(
							cmd, silhouetteHandle, pingHandle, RenderBufferLoadAction.DontCare,
							RenderBufferStoreAction.Store, composite, ShaderPass.FloodInit);

						var width = (settings.widthControl == WidthControl.Shared ? settings.sharedWidth : maxwidth) *
						...
						for (var i = numberOfMips - 1; i >= 0; i--)
						{
							...
							Blitter.BlitCameraTexture(
								cmd, pingHandle, pongHandle, RenderBufferLoadAction.DontCare,
								RenderBufferStoreAction.Store, composite, ShaderPass.FloodJump);
							cmd.SetGlobalVector(ShaderPropertyId.AxisWidthId, new Vector2(0f, stepWidth));
							Blitter.BlitCameraTexture(
								cmd, pongHandle, pingHandle, RenderBufferLoadAction.DontCare,
								RenderBufferStoreAction.Store, composite, ShaderPass.FloodJump);
						}
					});
				}
				...
				{
					builder.UseTexture(
						informationHandle); // FIXME: needed only for scene view? Need information handle to survive until this pass.
					...
					builder.SetRenderFunc((PassData _, RasterGraphContext context) =>
					{
						switch (settings.materialType)
						{
							case MaterialType.Basic:
								Blitter.BlitTexture(
									context.cmd, pingHandle, Vector2.one, composite, ShaderPass.Outline);
								break;
			...
			private void InitMaskRendererLists(
				RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
				...
				foreach (var outline in settings.Outlines)
				{
					...
					var drawingSettings = RenderingUtils.CreateDrawingSettings(
						RenderUtils.DefaultShaderTagIds, renderingData, cameraData, lightData, sortingCriteria);
					...
					var filteringSettings = new FilteringSettings(
						renderQueueRange, outline.layerMask, outline.RenderingLayer);
					...
					RenderUtils.CreateRendererListWithRenderStateBlock(
						renderGraph, ref renderingData.cullResults, drawingSettings, filteringSettings,
						renderStateBlock,
						ref handle);
			...
			private void InitSilhouetteRendererLists(
				RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
				...
				foreach (var outline in settings.Outlines)
				{
					...
					var drawingSettings = RenderingUtils.CreateDrawingSettings(
						RenderUtils.DefaultShaderTagIds, universalRenderingData, cameraData, lightData,
						sortingCriteria);
					if (!outline.vertexAnimation)
					{
						drawingSettings.overrideMaterial = outline.gpuInstancing ?
							outline.silhouetteMaterialInstanced :
							outline.silhouetteMaterial;
					...
					var stencilState = StencilState.defaultValue;
					stencilState.enabled = true;
					stencilState.SetCompareFunction(
						outline.occlusion == WideOutlineOcclusion.WhenOccluded ?
					...
					var handle = new RendererListHandle();
					RenderUtils.CreateRendererListWithRenderStateBlock(
						renderGraph, ref universalRenderingData.cullResults, drawingSettings, filteringSettings,
						renderStateBlock, ref handle);
			...
			private void InitInformationRendererList(
				RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
			...
			private static void CreateRenderGraphTextures(
				RenderGraph renderGraph, UniversalResourceData resourceData,
				...
				out TextureHandle pongHandle)
			{
				...
				baseDescriptor.colorFormat =
					GraphicsFormat.R8G8B8A8_UNorm; // TODO: Changed to format somewhere in Unity 6 cycle?
				...
				baseDescriptor.colorFormat =
					GraphicsFormat.R16_SNorm; // TODO: Changed to format somewhere in Unity 6 cycle?
				...
				baseDescriptor.colorFormat =
					SystemInfo.IsFormatSupported(GraphicsFormat.R16G16_SNorm, GraphicsFormatUsage.Render) ?
...
#pragma warning disable 618, 672
			...
			public void CreateHandles(RenderingData renderingData)
			{
				...
				RenderingUtils.ReAllocateIfNeeded(
					ref silhouetteRTHandle, descriptor, FilterMode.Point, TextureWrapMode.Clamp,
					name: Buffer.Silhouette);
				...
				RenderingUtils.ReAllocateIfNeeded(
					ref silhouetteDepthRTHandle, silhouetteDepthDescriptor, FilterMode.Point, TextureWrapMode.Clamp,
					name: Buffer.SilhouetteDepth);

				// Ping pong buffers.
				var pingPongDescriptor = renderingData.cameraData.cameraTargetDescriptor;
				pingPongDescriptor.graphicsFormat =
					SystemInfo.IsFormatSupported(GraphicsFormat.R16G16_SNorm, FormatUsage.Render) ?
						GraphicsFormat.R16G16_SNorm :
						GraphicsFormat.R32G32_SFloat;
				pingPongDescriptor.depthBufferBits = (int) DepthBits.None;
				pingPongDescriptor.msaaSamples = 1;
				RenderingUtils.ReAllocateIfNeeded(
					ref pingRTHandle, pingPongDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: Buffer.Ping);
				RenderingUtils.ReAllocateIfNeeded(
					ref pongRTHandle, pingPongDescriptor, FilterMode.Point, TextureWrapMode.Clamp, name: Buffer.Pong);
			}
			...
			public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
			{
				...
				using (new ProfilingScope(maskCmd, maskSampler))
				{
					...
					foreach (var outline in settings.Outlines)
					{
						...
						var drawingSettings = RenderingUtils.CreateDrawingSettings(
							RenderUtils.DefaultShaderTagIds, ref renderingData, sortingCriteria);
						drawingSettings.overrideMaterial = mask;
				...
				using (new ProfilingScope(silhouetteCmd, silhouetteSampler))
				{
					CoreUtils.SetRenderTarget(
						silhouetteCmd, silhouetteRTHandle,
						settings.customDepthBuffer ?
							silhouetteDepthRTHandle :
							renderingData.cameraData.renderer.cameraDepthTargetHandle);
				...
				if (settings.customDepthBuffer)
					silhouetteCmd.SetGlobalTexture(
						ShaderPropertyId.SilhouetteDepthBuffer, silhouetteDepthRTHandle.nameID);
				...
				using (new ProfilingScope(floodCmd, floodSampler))
				{
					...
					Blitter.BlitCameraTexture(
						floodCmd, silhouetteRTHandle, pingRTHandle, composite, ShaderPass.FloodInit);
					...
					for (var passIndex = numberOfMips - 1; passIndex >= 0; passIndex--)
					{
						...
						Blitter.BlitCameraTexture(
							floodCmd, pingRTHandle, pongRTHandle, composite, ShaderPass.FloodJump);
						floodCmd.SetGlobalVector(ShaderPropertyId.AxisWidthId, new Vector2(0f, stepWidth));
						Blitter.BlitCameraTexture(
							floodCmd, pongRTHandle, pingRTHandle, composite, ShaderPass.FloodJump);
					}
				}
				...
				using (new ProfilingScope(outlineCmd, outlineSampler))
				{
					...
					CoreUtils.SetRenderTarget(
						outlineCmd, renderingData.cameraData.renderer.cameraColorTargetHandle,
						settings.customDepthBuffer ?
							silhouetteDepthRTHandle :
