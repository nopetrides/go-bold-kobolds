...
namespace Linework.FastOutline
{
	...
	public class FastOutline : ScriptableRendererFeature
	{
		...
		public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
		{
			...
			var render = fastOutlinePass.Setup(
				ref settings, ref maskMaterial, ref outlineMaterial, ref outlineInstancedMaterial, ref clearMaterial);
...
#pragma warning restore 618, 672
		...
		private bool CreateMaterials()
		{
			...
			if (clearMaterial == null) clearMaterial = CoreUtils.CreateEngineMaterial(shaders.clear);
		...
		private class FastOutlinePass : ScriptableRenderPass
		{
			...
			public bool Setup(
				ref FastOutlineSettings fastOutlineSettings, ref Material maskMaterial, ref Material outlineMaterial,
				ref Material outlineInstancedMaterial, ref Material clearMaterial)
			{
				...
				foreach (var outline in settings.Outlines)
				{
					...
					material.SetColor(
						ShaderPropertyId.OutlineOccludedColor,
						outline.occlusion == Occlusion.WhenOccluded ? outline.color : outline.occludedColor);
					...
					if (outline.scaling == Scaling.ScaleWithDistance)
						material.EnableKeyword(ShaderFeature.ScaleWithDistance);
			...
			{
				return outline.IsActive() && (outline.maskingStrategy == MaskingStrategy.Stencil ||
			...
			public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
			{
				...
				using (var builder = renderGraph.AddRasterRenderPass<PassData>(
							ShaderPassName.Outline, out var passData))
				{
					builder.SetRenderAttachment(resourceData.activeColorTexture, 0);
					builder.SetRenderAttachmentDepth(resourceData.activeDepthTexture);
					...
					builder.SetRenderFunc((PassData data, RasterGraphContext context) =>
					{
						foreach (var handle in data.OutlineRendererListHandles) context.cmd.DrawRendererList(handle);
					});
				}
			...
			private void InitMaskRendererList(
				RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
				...
				RenderUtils.CreateRendererListWithRenderStateBlock(
					renderGraph, ref renderingData.cullResults, drawingSettings, filteringSettings, renderStateBlock,
					ref passData.MaskRendererListHandle);
			}

			private void InitOutlineRendererLists(
				RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
				...
				foreach (var outline in settings.Outlines)
				{
					...
					drawingSettings.overrideMaterial =
						outline.gpuInstancing ? outline.materialInstanced : outline.material;
			...
			public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
			{
				...
				using (new ProfilingScope(maskCmd, maskSampler))
				{
					...
					var layerMask = settings.Outlines
						.Where(ShouldRenderStencilMask)
						.Aggregate(0, (current, outline) => current | outline.layerMask.value);
					var renderQueueRange =
						RenderQueueRange.all; // FIXME: This does not take into account the setting of the outline.
					var filteringSettings = new FilteringSettings(renderQueueRange, layerMask, layer);
					var drawingSettings = RenderingUtils.CreateDrawingSettings(
				...
				using (new ProfilingScope(clearStencilCmd, outlineSampler))
				{
					...
					CoreUtils.SetRenderTarget(
						clearStencilCmd, renderingData.cameraData.renderer.cameraColorTargetHandle,
						cameraDepthRTHandle); // if using cameraColorRTHandle this does not render in scene view when rendering after post processing with post processing enabled
