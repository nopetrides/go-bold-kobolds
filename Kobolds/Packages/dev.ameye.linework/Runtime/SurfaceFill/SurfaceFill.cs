...
#pragma warning restore 618, 672
		...
		private bool CreateMaterials()
		{
			...
			if (fillMaterial == null) fillMaterial = CoreUtils.CreateEngineMaterial(shaders.fill);
		...
		private class SurfaceFillPass : ScriptableRenderPass
		{
			...
			public bool Setup(
				ref SurfaceFillSettings surfaceFillSettings, ref Material maskMaterial, ref Material fillMaterial)
			{
				...
				foreach (var fill in settings.Fills)
				{
					...
					if (fill.materialType == MaterialType.Custom && fill.customMaterial != null)
					{
						...
						fill.customMaterial.SetInt(
							CommonShaderPropertyId.FullScreenColorBlendModeDestination, dstBlend);
					}
					...
					fill.material.SetColor(
						ShaderPropertyId.SecondaryColor,
						fill.pattern == Pattern.Glow ? Color.clear : fill.secondaryColor);
					...
					if (fill.pattern == Pattern.Texture)
						fill.material.SetFloat(ShaderPropertyId.Rotation, fill.rotation * 0.5f);
					...
					fill.material.SetFloat(
						CommonShaderPropertyId.FullScreenStencilComparison, (float) CompareFunction.Equal);
				...
				using (var builder = renderGraph.AddRasterRenderPass<PassData>(ShaderPassName.Fill, out _))
				{
					...
					builder.SetRenderFunc((PassData _, RasterGraphContext context) =>
					{
						...
						foreach (var fill in settings.Fills)
						{
							...
							{
								fill.material.SetFloat(
									CommonShaderPropertyId.FullScreenStencilPass, (float) StencilOp.Zero);
								fill.material.SetFloat(
									CommonShaderPropertyId.FullScreenStencilFail, (float) StencilOp.Zero);
				...
				RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
				...
				foreach (var fill in settings.Fills)
				{
					...
					var filteringSettings = new FilteringSettings(
						renderQueueRange, fill.layerMask, fill.RenderingLayer);
				...
				using (new ProfilingScope(maskCmd, fillSampler))
				{
					...
					foreach (var fill in settings.Fills)
					{
						...
						CoreUtils.SetRenderTarget(
							fillCmd, renderingData.cameraData.renderer.cameraColorTargetHandle,
