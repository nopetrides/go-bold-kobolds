    ...
    SubShader
    {
        ...
        Pass // 0: BLUR
        {
            ...
            half4 frag(Varyings IN) : SV_TARGET
            {
                ...
                value += SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp,
                                          IN.texcoord + float2(_offset, _offset) * texelSize) * 0.2;
                value += SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp,
                                                                IN.texcoord + float2(_offset, -_offset) * texelSize) *
                    0.2;
                value += SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp,
                                                     IN.texcoord + float2(-_offset, _offset) * texelSize) * 0.2;
                value += SAMPLE_TEXTURE2D(_BlitTexture, sampler_LinearClamp,
                                                                      IN.texcoord + float2(-_offset, -_offset) *
                                                                      texelSize) * 0.2;
