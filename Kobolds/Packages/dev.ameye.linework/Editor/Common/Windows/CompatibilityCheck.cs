...
namespace Linework.Editor.Common.Windows
{
	public class CompatibilityCheck : EditorWindow
	{
		...
		private void CreateGUI()
		{
			...
			var versionLabel = new Label("1.4.6 • March 2025")
			{
				style =
				{
					fontSize = 12,
			...
			reviewLink.AddManipulator(
				new Clickable(() =>
					Application.OpenURL("https://assetstore.unity.com/packages/vfx/shaders/linework-294140#reviews")));
			...
			{
				foreach (var checkItem in checks) RunCheck(checkItem, listView);
			})
			{
		...
		private CheckResult CheckUnityVersion()
		{
			...
			if (unityVersion.StartsWith("6000.1"))
			{
				if (alpha || beta)
					return new CheckResult(
						ResultEnum.Info, unityVersion,
						"Linework is compatible with Unity 6.1. However, alpha/beta versions of Unity may be unstable.");
			...
			if (unityVersion.StartsWith("6000"))
			{
				if (alpha || beta)
					return new CheckResult(
						ResultEnum.Info, unityVersion,
						"Linework is compatible with Unity 6.0. However, alpha/beta versions of Unity may be unstable.");
			...
			if (unityVersion.StartsWith("2022.3"))
			{
				if (alpha || beta)
					return new CheckResult(
						ResultEnum.Info, unityVersion,
						"Linework is compatible with Unity 2022.3. However, alpha/beta versions of Unity may not be stable.");
				return new CheckResult(
					ResultEnum.Info, unityVersion,
			...
			if (unityVersion.StartsWith("2023"))
				return new CheckResult(
					ResultEnum.Fail, unityVersion,
					$"Linework is not compatible with Unity {unityVersion}. Please upgrade to Unity 6 which is the LTS version for the 2023 cycle.");
			return new CheckResult(
				ResultEnum.Fail, unityVersion, $"Linework is not compatible with Unity {unityVersion}.");
		}
		...
		private CheckResult CheckURPVersion()
		{
			...
			while (!_urpSearchRequest.IsCompleted) Thread.Sleep(100);

			if (_urpSearchRequest.Status != StatusCode.Success)
				return new CheckResult(
					ResultEnum.Fail, $"Error: {_urpSearchRequest.Error.message}", "An error occurred.");

			var package = _urpSearchRequest.Result[0];
			var version = package.version;
			return new CheckResult(
				version.StartsWith("14") || version.StartsWith("17") ? ResultEnum.Pass : ResultEnum.Fail, version,
				"Linework is compatible with URP 14 and URP 17.");
		}
		...
		private CheckResult CheckTargetPlatform()
		{
			...
			return target switch
			{
				BuildTarget.StandaloneWindows or BuildTarget.StandaloneWindows64 => new CheckResult(
					ResultEnum.Pass, "Windows", "Linework is compatible with Windows."),
				BuildTarget.StandaloneOSX => new CheckResult(
					ResultEnum.Pass, "macOS", "Linework is compatible with macOS."),
				BuildTarget.iOS => new CheckResult(
					ResultEnum.Warning, "iOS",
		...
		private CheckResult CheckActivePipeline()
		{
			if (GraphicsSettings.currentRenderPipeline == null)
				return new CheckResult(ResultEnum.Fail, "Not found", NoActiveRendererFoundDescription);
			var type = GraphicsSettings.currentRenderPipeline.GetType().ToString();
			if (type.Contains("HDRenderPipelineAsset"))
				return new CheckResult(
					ResultEnum.Fail, "High Definition",
					"Linework is not compatible with the High Definition Render Pipeline.");

			if (type.Contains("UniversalRenderPipelineAsset"))
				return new CheckResult(
					ResultEnum.Pass, "Universal", "Linework is compatible with the Universal Render Pipeline.");
			if (type.Contains("LightweightRenderPipelineAsset"))
				return new CheckResult(
					ResultEnum.Fail, "Light Weight",
					"Linework is not compatible with the Light Weight Render Pipeline");
			return new CheckResult(
				ResultEnum.Fail, "Custom", "Linework is not compatible with any Custom Render Pipeline.");
		}
		...
		private CheckResult CheckActiveRenderer()
		{
			...
			if (!type.Contains("UniversalRenderPipelineAsset"))
				return new CheckResult(
					ResultEnum.Fail, "Universal pipeline not found", "Universal pipeline not found.");
			var pipeline = (UniversalRenderPipelineAsset) GraphicsSettings.currentRenderPipeline;
			var propertyInfo = pipeline.GetType().GetField(
			...
			return rendererData switch
			{
				Renderer2DData => new CheckResult(
					ResultEnum.Fail, "2D",
					"Linework is not compatible with the 2D Renderer. Support will be added in the future."),
				UniversalRendererData => new CheckResult(
					ResultEnum.Pass, "Universal", "Linework compatible with the Universal Renderer."),
		...
		private CheckResult CheckRenderGraph()
		{
			...
			return usingRenderGraph ?
				...
				new CheckResult(
					ResultEnum.Info, "Compatibility Mode",
		...
		private CheckResult CheckRenderingPath()
		{
			...
			var type = GraphicsSettings.currentRenderPipeline.GetType().ToString();
			if (!type.Contains("UniversalRenderPipelineAsset"))
				return new CheckResult(ResultEnum.Fail, "URP renderer not found", "URP renderer not found.");
			var pipeline = (UniversalRenderPipelineAsset) GraphicsSettings.currentRenderPipeline;
			var propertyInfo = pipeline.GetType().GetField(
				"m_RendererDataList", BindingFlags.Instance | BindingFlags.NonPublic);
			var rendererData = ((ScriptableRendererData[]) propertyInfo?.GetValue(pipeline))?[0];
				...
				case UniversalRendererData data:
				{
					...
					return renderingMode switch
					{
						RenderingMode.Forward => new CheckResult(
							ResultEnum.Pass, "Forward", "Linework is compatible with the Forward rendering path."),
						RenderingMode.ForwardPlus => new CheckResult(
							ResultEnum.Pass, "Forward+", "Linework is compatible with the Forward+ rendering path."),
						RenderingMode.Deferred => new CheckResult(
							ResultEnum.Fail, "Deferred",
		...
		private CheckResult CheckGraphicsAPI()
		{
			return SystemInfo.graphicsDeviceType switch
			{
				GraphicsDeviceType.Metal => new CheckResult(
					ResultEnum.Pass, "Metal", "Linework is compatible with Metal."),
				GraphicsDeviceType.Vulkan => new CheckResult(
					ResultEnum.Pass, "Vulkan", "Linework is compatible with Vulkan."),
				GraphicsDeviceType.Direct3D11 => new CheckResult(
					ResultEnum.Pass, "Direct3D11", "Linework is compatible with Direct3D11."),
				GraphicsDeviceType.OpenGLCore => new CheckResult(
					ResultEnum.Fail, "OpenGLCore", "Linework is not compatible with OpenGLCore."),
				_ => new CheckResult(
					ResultEnum.Warning, SystemInfo.graphicsDeviceType.ToString(),
		...
		private CheckResult CheckActiveRenderPipelineAsset()
		{
			...
			if (activeRenderPipelineAsset == null)
				return new CheckResult(ResultEnum.Fail, "Not found", NoActiveRendererFoundDescription);
			return new CheckResult(
				ResultEnum.Info, activeRenderPipelineAsset.name,
