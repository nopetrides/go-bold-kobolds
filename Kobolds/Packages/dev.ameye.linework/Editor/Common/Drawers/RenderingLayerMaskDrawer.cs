	...
	public sealed class RenderingLayerMaskDrawer : PropertyDrawer
	{
		...
		public override VisualElement CreatePropertyGUI(SerializedProperty property)
		{
			...
			var maskField = new MaskField(
				attr.ShowLabel ? property.displayName : string.Empty, renderingLayerMaskNames, property.intValue);
		...
		public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
		{
			...
			if (maskNames.Length < maskCount && maskCount <= 32)
			{
				...
				for (var i = 0; i < maskCount; ++i)
					newRenderingLayerMaskNames[i] = i < maskNames.Length ? maskNames[i] : $"Unused Layer {i}";
			...
			if (EditorGUI.EndChangeCheck()) property.uintValue = (uint) renderingLayer;
