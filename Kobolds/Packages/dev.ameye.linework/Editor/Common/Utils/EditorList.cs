...
namespace Linework.Editor.Common.Utils
{
	public class EditorList<T> where T : ScriptableObject
	{
		...
		public EditorList(
			UnityEditor.Editor targetEditor, SerializedProperty items, Action onChangedCallback, string addItemText,
			string noItemsText, string maxItemsText = "",
			int maxItems = 0)
		{
			Items = items;
			...
			if (item != null)
			{
				...
				var displayContent = CoreEditorUtils.DrawHeaderToggle(
					EditorGUIUtility.TrTextContent(
						ObjectNames.GetInspectorTitle(item).Replace($" ({typeof(T).Name})", ""), "An outline."),
					itemProperty, activeProperty,
		...
		private void AddItem()
		{
			...
			AssetDatabase.TryGetGUIDAndLocalFileIdentifier(component, out _, out _);
			...
			if (EditorUtility.IsPersistent(targetEditor.target)) onChangedCallback?.Invoke();
			targetEditor.serializedObject.ApplyModifiedProperties();
		}
		...
		{
			for (var i = editors.Count - 1; i >= 0; --i) Object.DestroyImmediate(editors[i]);
			editors.Clear();
		}
	}
}
