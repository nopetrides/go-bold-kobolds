	...
	public static class SmoothNormalsBaker
	{
		public static Vector2[] ComputeSmoothedNormals(Mesh mesh)
		{
			...
			var bakeNormalJob = new BakeNormalJob(
				verticesNativeArray, normalsNativeArray, tangentsNativeArray, smoothedNormalsMap, bakedNormals);
		...
		private struct BakeNormalJob : IJobParallelFor
		{
			...
			private static Vector2 OctahedronNormal(Vector3 resultNormal)
			{
				var absVec = new Vector3(
					Mathf.Abs(resultNormal.x), Mathf.Abs(resultNormal.y), Mathf.Abs(resultNormal.z));
