	...
	public class OutlineEditor : UnityEditor.Editor
	{
		...
		public override void OnInspectorGUI()
		{
			...
			if (alphaCutout.boolValue)
			{
				...
				alphaCutoutUVTransform.vector4Value = EditorGUILayout.Vector4Field(
					EditorUtils.CommonStyles.AlphaCutoutUVTransform, alphaCutoutUVTransform.vector4Value);
				EditorGUI.indentLevel--;
			}
			...
			if (gpuInstancing.boolValue)
				EditorGUILayout.HelpBox(
					"GPU instancing breaks the SRP Batcher. See the documentation for details.", MessageType.Warning);
			EditorGUILayout.PropertyField(vertexAnimation, EditorUtils.CommonStyles.VertexAnimation);
			if (vertexAnimation.boolValue)
				EditorGUILayout.HelpBox(
					"With vertex animation enabled, the outline color should be set by your object's shader. See the documentation for details.",
					MessageType.Warning);
			EditorGUILayout.Space();
			...
			{
				EditorGUILayout.HelpBox(
					"The mask mode is used to mask out the other outlines where they are not needed.",
					MessageType.Info);
			}
